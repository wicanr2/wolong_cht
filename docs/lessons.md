...
| 寫掃描條件去問「有沒有人做某件事」，而答案是 0 處 | [過濾器自己有洞：清除端的立即值是補數](#filter-has-a-hole) | 3 | tool |
...
| 日期 | 犯在哪 | 收據 |
|---|---|---|
| 2026-09-11 | 找兵記錄 `+0x01` 的開場設定端，掃「運算元位移 ＝ 1 的寫入」得到 27 筆、每一筆都真的在寫那個欄位——**唯獨少了真正的那一筆**，因為它是 `mov word ptr es:[di], 180h`（位移算 0）。我因此一度寫下「沒有一支在開場設它」 | `docs/spec/200` §5 |
...
### 「永遠不可能」要標明前提，否則它讀起來像物理定律

<a id="impossible-without-its-premise"></a>**什麼時候想起**：要寫下或引用「這個永遠對不上／不可能全等／必然有殘差」這種結論時

**要做的**：把**前提**寫進同一句話。「兩邊的亂數不同源」是**當時 fixture 的性質**，不是規則的性質；句子沒帶限定詞，下一個人（包括自己）就會把它當成不可跨越的界線，照著它把可修的缺口記成結構性殘差。⭐ **引用這種斷言之前先問「它是在什麼條件下成立的」**——條件常常早就變了。

| 日期 | 犯在哪 | 收據 |
|---|---|---|
| 2026-09-11 | `docs/spec/133` §3.6 寫「Y 是亂數，逐兵座標永遠不可能全等」，我照它把畫面對拍的 `sb-minimap` 128／432 px 記成結構性殘差。實際上 `-battle-exact` ＋ `-rng-state` **早在三天前就存在**（`docs/playtest/114` 逐槽核對過 96 槽全同）——**推翻它的東西比它本身還早** | `docs/playtest/121` §4、`CONTEXT.md` §6 |
