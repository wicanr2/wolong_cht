...
| 水平自相符（位移 32）| **95.8%**（相鄰位移 2 只有 78.6%、位移 16 是 67.5%）|
| 垂直自相符（位移 32）| **89.7%**（另一張截圖的視窗左緣，位移 16 是 48.8%）|

**兩軸都在 32 px 出現尖峰 ⇒ 它是 32×32 的圖塊平鋪。** 圖案本身含一條
四列高的棋盤格 dithering 帶，那是它看起來「不像規則底紋」的原因。
...
| 找法 | 結果 |
|---|---|
| 把樣本轉成調色盤索引，逐 byte 掃 `workplace/orig/dosv/` **全部 69 檔** | 4bpp planar（寬 8／16／32）與 1bpp（寬 8／16／32）都沒有命中，最高 86.9% |
