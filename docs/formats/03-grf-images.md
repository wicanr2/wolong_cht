...
| **不假設圖寬**：拿樣本的一列（4 byte）當探針找出所有出現位置，再對每個位置試 stride 4–96 | 0 命中（`dosv` ＋ `pc98` 全部檔案）|
| 同上，再加 **32 種水平相位**（底紋是 32 px 週期，畫在視窗裡的相位取決於視窗 X）| 0 命中。⚠ 中途出現過 12 筆「命中」，是驗證迴圈的 `break` 讓不足的列數算成通過——**探針列太靠近底部時只驗了兩列**。修掉之後歸零 |
...
### 5.5.1 ⭐ 圖案與橫幅的背景是同一套美術

把樣本與段 0（橫幅 640×32）的 **plane 3**（色 8 的那個位元）並排印出來，
最佳位置 (304, 8) 的形狀相符 **90.2%**，其他三個 plane 只有 62–66%。
逐列看就是同一張圖：龍鱗的輪廓、那條四列高的棋盤格帶、細節的走向全部對上。

差異集中在**橫幅上有金龍與標題字的位置**——那些像素在 plane 3 也有位元。
試過「只取色 8」（`plane3 & ~plane0 & ~plane1 & ~plane2`）想把前景濾掉，
反而降到 88.3%：**前景蓋掉的背景還原不回來**。

⇒ **底紋與橫幅背景同源，但視窗底紋另有一份乾淨的資料**，而那一份不在
任何原版檔案裡（下表）。
...
⚠ 那支 bridge 跑的是 **PC-98 版**，位址不能直接搬到 DOS/V（`CLAUDE.md` §7 第 9 條），
只能拿來確認「畫底紋的是哪一支的哪一種手法」。

**remake 這一側先維持純色。** 從實跑截圖把圖案抽出來當素材是可行的，
但那是原版美術，不能進版控（`CLAUDE.md` §9 的資產規則）。
