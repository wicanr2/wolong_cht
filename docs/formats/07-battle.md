...
⭐ 表頭、尾段與 `BATTLE.MDL` 多出來的 13 筆**都是填充**，三處全量掃過
逐 byte 全 0（§2.4）；`GateX` ＝ 0 的六個據點是**關隘**（§2.5）。
...
3,968 ＝ **64 × 62**。⭐ **寬是 64**：繪圖端的定址式是
`di = Y × 64 + X`（下一小節），列跨距 64 就是 X 的值域。
所以地形是 **64 寬 × 62 列**，而整塊 4,096 B 是 **64 × 64**——
多出來的兩列是表頭（Y ＝ 0）與尾段（Y ＝ 63）。

⭐ **兩者都是填充：214 個戰場的那 128 個 byte 全部是 0，逐 byte 零例外**
（2026-09-03 全量掃過，`workplace/parity/battlehdr.py`）。
每個 byte 位置在 214 個戰場之間都只有一種值，而那個值是 `00`。
它們不帶資料，只是把每個戰場湊成整齊的 4 KB；繪圖端的邊界檢查
`cmp di, 1000h` 涵蓋整塊，所以那兩列會被當成圖塊 0 畫出來。

**這是這個檔案的通則**（§2.4）。

#### ⭐ 原版的 Y 座標從表頭那一列算起，比格子的列號大 1

繪圖端**用整塊 4,096 B 的線性索引定址**：`sub_1DC9D` 由鏡頭算出
`di = Y × 64 + X`，`sub_1DD22` 再以 `es:[di]` 取那一格的圖塊值，
而它的邊界檢查是

```asm
cmp     di, 0
jl      short 跳過
cmp     di, 1000h           ; ← 整塊 4,096 B，含表頭與尾段
jge     short 跳過
```

`0x1000` 而不是 `0xF80`，所以 `di = 0` 指的是**表頭的第一個 byte**，
不是第一格地形。也就是說**原版的 Y ＝ 地形列號 ＋ 1**：
`Y = 1` 才是第一列地形，`Y = 62` 是最後一列。

這與 §2.2 的旋轉範圍是同一件事的兩面：`0x40`–`0xFBF` 正好是
「Y ＝ 1 到 62」那 62 列，表頭（Y ＝ 0）與尾段（Y ＝ 63）不動。

**小地圖走的是另一個框**：`0x1F0 + 2 × Y` 那條式子裡的 Y 是**地形列號**
（從 0 起算），與繪圖端差 1。這一條是**實測**（強證據，非靜態）：
remake 拿地形列號畫的小地圖與原版逐像素對齊，往左往右各推 2 px
（＝一列）都明顯更差（9.4% → ≥ 19.5%）。兩個框都要用對，換算表見
[`../spec/57`](../spec/57-tactical-projection.md) §2。
...
### 2.4 ⭐ 通則：表都做成 2 的冪，用不到的槽留 0

三處各自量過，形狀相同（2026-09-03，`workplace/parity/battlehdr.py`）：

| 表 | 整塊大小 | 實際用到 | 其餘 |
|---|---|---|---|
| `BATTLE.MAP` 的索引 | 512 B ＝ **256 筆** × 2 B | 214 筆 | 後 42 筆逐 byte 全 0 |
| 每個戰場 | 4,096 B ＝ **64 × 64** | 64 × 62 格地形 | 第 0 列與第 63 列全 0（§2.1）|
| `BATTLE.MDL` 的表頭 | 4,096 B ＝ **16 筆** × 256 B | 3 筆 | 第 3–15 筆全 0（§3）|

⭐ **知道是填充就不必再找語意。** 三處先前各自掛著一條「內容未解」，
而三條是同一件事：**位移算術用移位比用乘法便宜**——
索引 `shl bx,1`、戰場 `× 0x1000`、圖塊組 `× 0x100`，
表湊成 2 的冪之後這些位移都是一條指令。

### 2.5 ⭐ `GateX` ＝ 0 的六個據點：那是關隘，不是城

索引表第二個 byte 是城門附近的 X（`docs/re/11` §4.5），
`Library.IsSiege` 就拿它是不是 0 當判準。214 個戰場裡 28 個是 0：
**22 個是野戰樣板（192–213，逐個確認全 0）**，另外六個落在據點區——
...
**六個全是關隘或古戰場**，沒有一個是城。所以「`GateX` ＝ 0」的語意是
**這個據點沒有城牆**，打它是野地會戰而不是攻城——
`buildStructures` 掃不到城壁與門的圖塊，一段實體都建不出來，
而那正是對的行為。
...
⭐ **多出來的 13 筆全是 0**（2026-09-03 逐筆掃過）：第 0–2 筆各有
255/256 個非零 byte，第 3–15 筆逐 byte 相同且全 0。
交叉印證在 `BATTLE.MAP` 的索引表——214 筆的**圖塊組欄位值域只有
`{0, 1, 2}`**（各 75／100／39 個戰場），沒有一個戰場指到第 3 筆以後。

所以表頭也是**湊成整塊 4 KB 的填充**，與 §2.1 同一個形狀。
...
| 項目 | 現況 |
|---|---|
| 表頭與尾段執行期會不會被寫 | 靜態全 0 是 confirmed（§2.1）。**沒有窮舉「有沒有人在執行期寫進那 128 B」**——地形緩衝區與佔用圖是兩塊，但沒有逐個寫入端查過 |
| `GateX` ＝ 0 的六個據點戰場 | 已定位是關隘／古戰場（§2.5）。**沒有實跑過一場**——打官渡或赤壁時 `IsSiege` 為假，城壁與門一段都建不出來，那條路沒有驗收紀錄 |
