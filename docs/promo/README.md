...
⭐ **重拍五張靜態圖是便宜的自我檢查。** 2026-08-29 那一輪重拍完只有
`promo-talk.png` 換了雜湊，其餘四張逐位元組相同——而那一輪動的正是
訊息框的肖像（[`../spec/106`](../spec/106-message-box-reporter-portrait.md)）。
**變的張數對得上改動的範圍**，就表示重拍確實吃到了新的程式碼；
五張全都沒變才是要停下來查的訊號。

2026-08-29 那一輪的動態段（張數／不重複張數）：大地圖 330／25、
野戰 390／84、攻城 270／76、四個語系各 130／18・10・20・22、
`newgame`／`form`／`march` 各 90／1（那三段開的是非常駐視窗，原版時鐘停住，
1 是對的）。
