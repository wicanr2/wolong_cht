...
⭐ **讀 DOSBox-X 原始碼**（`rich2-dosbox-src:latest` 的 `/src/dosbox-x`，
`src/ints/mouse.cpp` 的 `Mouse_CursorMoved`）之後知道為什麼：那裡有**兩條路**。

| 條件 | 客體座標怎麼來 |
|---|---|
| 絕對（integration，且游標被鎖住或全螢幕）| `視窗座標 ÷ (輸出寬−1) × max_screen`，`max_screen` 取 `[dos]` 的 `int33 max x/y`（非 0 時），否則取遊戲自己用 `AX=7`／`AX=8` 設的範圍再做一次「接近螢幕寬就當螢幕寬」的整數修圓 |
| **相對（`mouse_emulation=integration` 而游標沒被鎖住時走這一條）** | `mouse.x += 主機位移 × pixelPerMickey × 靈敏度`——**是累加，不是視窗位置的函數** |

擷取腳本跑在 Xvfb 上、沒有鎖游標，所以走的是**相對累加**那一條。
⇒ 同一個視窗座標，在不同的移動順序之下會得到不同的客體座標，
**而那正是「大部分 1:1、少數差 1」的形狀**。

⚠ 所以「視窗座標 → 遊戲座標」在 DOSBox 那條路上**本來就不是一個函數**。
`× 399 ÷ 479` 是這一批取樣點的經驗式，不是通式——
**dosgolem 沒有視窗也沒有累加，送什麼就是什麼**，這一條在新腳本上不存在。
