...
> **2026-08-08 後續：本文的兩個核心判斷都要修正。**
>
> 1. **§6 的「四件事被擋住」整張表在寫下的當下就過時了**——四件都不需要滑鼠
>    （見 [`docs/playtest/05`](05-viewport-and-city-coords.md) §5）。
> 2. **§0 的「滑鼠事件從來沒進去過」不成立。** 事件進得去，
>    但那些實驗有一半跑在**開場動畫裡**（開場長度每次不同，`wait:` 靠不住），
>    另一半送的是絕對定位——而 PC-98 是匯流排滑鼠，只吃相對位移。
>    解法見 [`docs/playtest/06`](06-mouse-solved.md)。
>
> 本文以下維持原樣，作為「推理看起來完全正確但結論是錯的」的紀錄。

**狀態：⛔ 已作廢——本文的兩個核心判斷都被推翻，解法見 `docs/playtest/06`。** 六次嘗試全部失敗之後，用一次免費的截圖比對就找到了：
