...
⚠ **成因還沒定。** 三個假說都被自己的對照推翻了：

| 假說 | 推翻它的 |
|---|---|
| 訊息框掛著的時候不畫游標 | [`92`](92-personnel-assign-parity.md) 的內政官任命同樣掛著訊息框，游標**在**（95 px）|
| 清單重畫那一列時把游標蓋掉 | 兩張的最後兩下都落在同一列上，行為應該一樣 |
| 清單的輸入迴圈（`sub_18412`）不畫游標 | ⭐ **對照 `cur-a`**（只點第一下、停在反白狀態）**確實沒有游標**——但 [`90`](90-formation-second-step.md) 的編成、[`92`](92-personnel-assign-parity.md) 的四張同樣是「清單還在畫面上」而游標**在** |

⭐ `cur-a` 是這一輪加的第三個資料點，做法是把 gen1 的序列砍掉第二下：
**只反白、不決定**。它證明了「畫面上有清單」不是判準——
同一張清單、同一個位置，有沒有游標取決於**當下在等哪一種輸入**，
而那個對應關係還沒對上。

**記成觀察，不記成結論。** 這一格是游標那條線的線索，但還不能拿來推繪製端；
在對上之前，remake 不自繪游標（自繪會讓現在 0 px 的那幾張反而變差）。
