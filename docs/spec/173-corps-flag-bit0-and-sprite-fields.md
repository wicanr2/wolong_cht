...
### 1.1 `+0x00` 位元 0 ＝「這一步是從路段中間走出來的」

⚠ **每次移動判定都會重寫它**，不是設起來就留著。判準是**移動前**的 `+0x0E`：

| 移動前的 `+0x0E` | 位元 0 | 意思 |
|---|---|---|
| `< 800h`（站在據點上）| **清**（`sub_147BB` 的 `00014869`，`and byte ptr [si], 0FEh`）| 這一步是從據點踏出去的 |
| `≥ 800h`（走在邊上）| **設**（`sub_147BB` 三個分支 ＋ `sub_126FF`）| 這一步是從路段中間走出來的 |

`sub_12662` 緊接著靠它分流：沒設 → `loc_126C0` 直接寫座標（出城第一步），
設 → `loc_126C8` 檢查 leg 盡頭。完整的兩半見
[`177`](177-replan-on-leg-not-from-city.md)。

設定端有兩處，都在「軍團拿到／推進路徑指標」的那一刻：
...
清除端只有一處：`sub_147BB` 的 `loc_1482F`（`00014869`，
`and byte ptr [si], 0FEh`）——軍團站在據點上重新選路的那一刻。

`tools/ida_bitflag_users.py` 現在按角色分組（`or`／`bts` 含指定位元 ＝ 設、
`and`／`btr` **不**含 ＝ 清），並在某一組是 0 處時明講。
**清除端的立即值是補數，不含那個位元**——只篩「含」的一側，
清除端會全部落空而輸出看起來正常。
...
⚠ **比的是「現在」與「下一個路徑點」，不是剛走過的那一格。**
`sub_12804` 傳的是 `+0x0A`（步進）本身，所以 `bx ← +0x0C ＋ 步進` 指向
**下一筆**；而座標在 `sub_12708` 裡已經先寫成新的位置了。
所以 `+0x08` 記的是**下一步要往哪走**，比「上一步往哪走」早一拍。

（`sub_127F6` 在 leg 邊界傳的是 **−步進**，取到的是上一筆，
所以它接著 `xor byte ptr [si+8], 1` 把結果翻回同一個約定——
`0↔1`、`2↔3` 剛好是同軸反向。兩個呼叫點的編碼因此一致。）

編碼（`ax ＝ 現在 − 下一筆`，取符號位再 `rol`）：
**0 ＝ 往 X 減、1 ＝ 往 X 增、2 ＝ 往 Y 減、3 ＝ 往 Y 增、4 ＝ 停著**。
...
| `+0x08` 朝向 | 值域與編碼都對（`headingTo` ＝ `sub_12808`、`HeadingStill` ＝ 4），但兩件事錯：**AI 編成走的是另一條路**（`autoFormCorps` 用 `var c Corps`）拿到 Go 零值 0 ＝「往 X 減」；而且算的是**剛走過那一步**的方向，原版算的是**下一個路徑點**，整整晚一拍 | 軍團 50／56 從拍 1,600 起 `04` 對 `00`；軍團 19 在拍 1,900 是 `01` 對 `02` |
...
| 規則層 | `internal/rules/march`：`CellMark.Next` 帶著**下一個路徑點**的座標，讓狀態層算得出原版的朝向（算式本身仍只有 `headingTo` 一份）|
| 差異 | 無（四個欄位都照原版） |
...
| 方式 | 證據 |
|---|---|
| 同局面對拍的軍團表逐 byte | `tools/corps_diff.py`：**五個檢查點（200／700／1,200／1,600／1,900）全部 0 個 byte** |
