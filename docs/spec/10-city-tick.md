...
| 玩家據點求援的喇叭聲（`sub_10CDE`）| ⭐ **量清楚了，但刻意不接**（見下）|

### 5.1 那一聲是什麼（2026-09-06 讀完）

`sub_10CDE` 只是 `mov ax, 101h / call sub_1EB11`，而 `sub_1EB11` 的形狀是：

```asm
sub_1EB11(ax = 0101h):      ; ch ＝ 外圈次數、cl ＝ 內圈次數
    等垂直同步邊緣（sub_1EB5E 讀 VGA 狀態埠 3DAh 的 bit 3）
    in al, 61h / or al, 3 / out 61h, al     ; ★ 喇叭 ON
    等 4 次同步邊緣                          ; ≈ 2 個畫面
    in al, 61h / and al, 0FCh / out 61h, al ; ★ 喇叭 OFF
    cmp cl, 1 / jz 收工
```

兩件事因此定案：

1. **長度 ≈ 2 個垂直畫面**（VGA 400 列模式約 70 Hz ⇒ **≈ 28 ms**），
   而且**節拍是拿垂直同步當時鐘**，不是拿 PIT。
2. ⭐ **音高不是它決定的**——這一支只開關 PPI port `61h` 的 bit 0/1，
   **從頭到尾沒有寫 PIT 通道 2**（埠 `42h`／`43h`），
   而 `KI.EXE` 全庫也找不到任何一處寫它。
   也就是說那一聲的音高**繼承自 BIOS／DOS 留下的除頻值**，
   在不同機器上不一樣。

⚠ **所以 remake 不接**：接了就要替原版挑一個它自己沒有決定的音高，
那不是還原，是替它做決定。這一格記成「已量清楚、刻意不做」，
不是缺口——[`29`](29-audio.md) 的音訊全部有原版可比的來源，這一聲沒有。

⭐ 它**與訊息內容無關**，是訊息框跳出來時的提示音
（[`77`](77-rout-talk-messages.md) §5），所以「接」等於**每一個訊息框都響**。
