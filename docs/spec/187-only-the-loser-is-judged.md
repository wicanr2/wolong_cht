...
同一段裡還有一件事：`test ah, 2 / jz loc_14B41` 的**兩條路都落到
`loc_14B41`**——判守將只是多一步，不是易主的條件。

```asm
        test ah, 2 / jz loc_14B41
        …call sub_1291A…                ; 守方壞滅才判守將
loc_14B41:
        mov  al, [si+1] / pop si / push si
        call sub_14CF3                  ; ★ 攻方贏就易主
```

remake 寫成 `defDead && !attDead && m == combat.Siege`，於是
「攻方贏但守軍只是撤退」那一場白打（同局面拍 5,072：軍團 35
打下據點 88，remake 沒易主）。

### 2.2 `sub_14CF3`：`+0x1A` 收下的是**舊主**

```asm
sub_14CF3:                              ; al ＝ 新主、si ＝ 據點記錄
        mov  bh, al
        xchg bh, [si+1]                 ; ★ [si+1] ← 新主，bh ← 舊主
        mov  [si+1Ah], bh               ; ★ +0x1A ← 舊主
        cmp  bh, 18h / jz loc_14D21     ; 舊主是中立就跳過後續
        call sub_14D63                  ; 遣回內政官
        …
```

⚠ `si` 是**據點記錄**不是軍團：`sub_14ADE` 的 `pop si` 取回的是
`00014AEB` 推的 `di`（戰場據點）。

`+0x1A` 是「這座城本來是誰的」（[`../formats/08`](../formats/08-sinario-save.md)）：
`+0x1A` 指著自己而 `+0x01` 已經不是，就是失土——`sub_13EFD` 靠它把
據點編號填進原主的 `+0x17`（一格佇列），AI 才知道要反攻。
remake 寫成新主，等於**每次易主都把失土紀錄抹掉**。

## 3. 驗證

| | 修正前 | 修正後 |
|---|---:|---:|
| 逐拍取數不一致 | 10 / 5,480 | **0 / 5,480（0.0%）**|
| 第一個分歧 | 5,104 | **沒有取數分歧** |
| 拍 5,176 的據點表 | 103 個 byte／57 座 | **1 個 byte／1 座**（語意未解的 `+0x15`）|
