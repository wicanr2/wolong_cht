...
| ~~逐條的原版擷取~~ | **27 個帶索引的呼叫點全部有原版擷取了**，逐條的結果收在 [`../playtest/93`](../playtest/93-status-bar-sweep.md)。⭐ 最後一條 #7（請求協助的第二步）也拍到了：靠 `parity_save.py --diplomat` 把外交官派好（[`../playtest/102`](../playtest/102-help-second-step.md)，五區只剩 95 px 的原版游標）|
