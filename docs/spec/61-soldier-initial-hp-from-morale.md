...
| 項目 | 現況 |
|---|---|
| ~~`+0x18`（戰力）的算式~~ | **已解**（confirmed，2026-09-02，[`../re/78`](../re/78-soldier-power-from-command.md)）：一般兵是 `((統率 + 適性) × 3 + 兵種係數) ÷ 4`，大將是 `(武力 × 2 + 適性) × 2`；**與士氣無關**。remake 目前把它接成士氣，接回原版的算式見 [`115`](115-soldier-power.md) |
| 大將的開場體力 | 不是軍團士氣——`sub_19B40` 把第 0 號兵蓋成 `max(70, (武力 × 4 + 50) × 士氣 ÷ 100)`（[`../re/78`](../re/78-soldier-power-from-command.md) §4）。remake 兩側都用士氣 |
