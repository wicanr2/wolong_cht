...
| **長跑的速度** | 只能等 | ⭐⭐ **`nodraw` ＋ `speed:0`**：同樣 15 個遊戲日 **7,930 萬道 → 1,030 萬道（7.7×）**，而狀態表 5,502 個欄位逐欄相同。⚠ **兩個要一起關**——繪圖與忙等可互換，單獨關一個幾乎白做（見 §7）|
| **指令花在哪** | 問不到 | **`profile:N[,取樣間隔][,分段]`**：取樣式剖析，印最熱的幾段 |
...
⚠⚠ **但這兩份對「要換的那個變因」不敏感**，所以它們的「相同」不構成
換 base 安全的證據：`siege:` 是直接注入呼叫的，軍團表那一份比的又正好是
不會漂的欄位。實際上新 base 跑到同一個指令數是 **16 時**、舊 base 是
**13 時**（[`../playtest/119`](../playtest/119-rng-pace-comparison.md) §36）——
一行 `clock` 就看得出來。**正對照要挑對變因敏感的樣本。**
...
原版的彈出選單（`sub_193E9` 那一族）**靠滑鼠的移動事件走，不看絕對位置**。
所以 `move:X,Y` 在選單上的意思是「往那個方向移一格」，**Y 的值不決定停在哪一列**：

| verbs | 停在第幾列 |
|---|---:|
| `move:48,296` × 1 | 1 |
| `move:48,232` × 2 | 2 |
| `move:48,264` × 2 | **2**（不是 3）|
| `move:48,200`／`232`／`264`／`296` 各一次 | 4 |

⇒ **移動事件的次數就是列號**。這正是既有腳本寫成
`[move:96,200;steps:100000;]*N;press` 的原因——那個 `N` 就是要選的列，
而 `200` 只是「一個比現在大的 Y」。

⛔ **不要照著畫面量出來的列座標去 `move`。** 進言選單的五列在
畫面 y 72／88／104／120／136，但 `move:48,136` 只會往下一格。
用「重複 N 次」才對得上。

（⚠ 這只適用於**選單**。一覽表的列是熱區，`stap:200,112` 那種絕對座標
照常有效——兩段式仍然要點兩次。）
...
## 7. ⭐⭐ 長跑加速：`nodraw` ＋ `speed:0`

即時制當 oracle，一個遊戲日要五百萬到一千萬道指令。
...
| 輪次 | 最熱的一段 | 是什麼 |
|---|---|---|
| 原樣 | `1D600–1D8FF` **88%** | `sub_1D66A` 把 40×23 顯示清單搬進 VRAM |
| 關掉繪圖之後 | `11DF0–11E0F` **77%** | `sub_11D8E` 尾端的**兩個忙等迴圈** |

那兩個忙等就是說明書講的「速度超過機器速度時設再快也不會更快」：

```asm
mov al, ds:0CFAh              ; 戰略速度設定值（系統選單第四列）
and al, al / jz  .ret         ; 0 ＝ 不等待
.wait1: cmp byte ptr ds:0D2Ch, 0 / jz .wait1
.wait2: cmp ds:0D2Dh, al        / jb .wait2
```

⭐ **繪圖與等待可互換**：每一圈的總時間由計時器決定，繪圖砍掉就多等一會兒。
...
⚠ **`speed:0` 不是外掛**，是遊戲自己的設定（`ds:0CFA`，最高速那一格）。
`nodraw` 是把兩支繪圖底層的入口塞 `retn`——**開了就不能截圖**，
VRAM 停在補丁生效前的樣子。

### 7.1 戰時的倍率是 2×，而且長跑會停在「等玩家」

上表那 7.7× 量在**和平期**。宣戰之後畫面上的東西變多，
同一組設定只剩約 **2×**（開戰期 2 億道指令推 14 個遊戲日）。

⛔ **真正的天花板不是速度。** 196年5月26日 2時，畫面上是

> 「呂布　的兵馬，向 許昌　進攻過來了！！」

許昌是玩家（曹操）的首都——**原版正確地停下來等玩家**，
時鐘從那一刻起一步都不動（再推兩億道指令，日期一樣）。
⭐ 那正是本專案一直拿來對拍的同一場（`siege:35,82`）。

⇒ 要跑更長的無人值守 AI 軌跡，要的不是更快，是
**「把訊息按掉 ＋ 讓那一仗自己打完」**——另一件事。

### 7.2 選劉備那一局的邊界：196 年 5 月 13 日 11 時

同局面逐子刻對拍（`workplace/parity/orig-at-16h-v2.DAT`，4/16 16 時起跑）
在 **5,787 拍 ＝ 5 月 13 日 11 時**停住，畫面是

> 「孫乾　大人，主公有事召見。」

**君主召見軍師**——玩家扮的正是軍師（`CLAUDE.md` §3.2）。
把 `steps:` 從 1.47 億加到 3 億，時鐘一步都不動、據點游標定在 144。

⭐ **推得過去。** 照 [`../playtest/118`](../playtest/118-liubei-90day-audience-response.md) §3
的序列（右鍵推進 → `move` 選列 → `press` → 右鍵推完），時鐘從
5/13 11 時走到 **5/14 12 時**，遊戲繼續跑：

```
steps:147054000                                   ← 跑到召見那一刻
rclick:287,199 ; steps:500000        × 2          ← 推進對話
move:96,220 ; steps:100000           × 3          ← 選第 3 列「絕」
press ; steps:2000000
rclick:287,199 ; steps:500000        × 3          ← 推完君主的回應
```

召見的內容與 `playtest/118` 記的完全一樣（曹操的使者請求協助，
三列：無條件同意／提供資金／絕），只是**時刻不同**——那一份的存檔
從 4/16 13 時起跑、召見在 5/11 17 時；對拍用的 `root-liubei13`
從 16 時起跑、召見在 5/13 11 時。

⛔ **左鍵一次都不會被讀到**（`playtest/118` §2）。`tools/dosgolem.sh`
的步驟表原本沒列 `rclick`／`press`／`profile`，所以「遊戲停住」時
第一個想到的是左鍵——**存在的東西沒寫進說明，跟不存在一樣貴**。

⭐⭐ **加速一定要證明軌跡沒變。** 「快了」而「答案變了」比慢還糟，
而且不會報錯。驗法是現成的：跑到同一個遊戲時刻，三張表逐欄比
（`tools/state_diff.py`）——**5,502 個欄位零差異**。
理由也講得通：忙等只燒時間、繪圖只寫 VRAM，兩者都不在因果鏈上，
而每個遊戲小時的主迴圈圈數是固定的（`sub_11D8E` 每圈推一個子刻、
九個子刻一小時），所以亂數的消耗序列也不變。
