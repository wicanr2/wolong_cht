...
- 出處：[`docs/re/20`](../re/20-ida-re-coverage-audit.md) §5 第四道 gate（逐層差分）、
  [`docs/re/46`](../re/46-strategy-chrome-cell-layer.md)（主畫面的原版幾何）
...
座標出自 [`docs/re/46`](../re/46-strategy-chrome-cell-layer.md)，是從機器碼算的，
不是量的：

| 區 | 矩形（x, y, w, h）| 來源 |
|---|---|---|
| `banner` | 0, 0, 640, 16 | 標題列 |
| `command` | 0, 16, 432, 32 | `sub_1614A`：`cx=0x021B` → 432×32，`bx=0`／`dx=0` |
| `map` | 0, 48, 432, 352 | 指令列之下、右欄之左 |
| `minimap` | 432, 16, 208, 144 | 右欄上段 |
| `faction` | 432, 160, 208, 240 | 右欄下段（君主／首都／軍師／信賴度／資金／預備兵）|

> ⚠ `banner` 的高度與右欄的分界目前**沒有機器碼證據**，是從畫面量的。
> 對拍第一輪就會告訴我們對不對——量錯的話那一區會整片不同。
