...
大地圖上不再顯示；玩家自己的軍團跳 TALK #1F。

**48 個 tick 之後**（`sub_12A7E`，每 tick 減 1）：軍團記錄歸零、
主將 `+0x17` 職務歸 0；原勢力已滅則主將改成在野。
玩家的軍團跳 TALK #23 ＋ 組 `0x198` 的檢討句。

⚠ **這條路徑到不了 #20**（「沒能將{1}捉拿到手」）——那一則要
`sub_1291A` 收到的勢力不等於軍團自己的勢力，而敗走傳的就是軍團自己的。
訊息這一段的完整出處與八變體展開在 [`77`](77-rout-talk-messages.md)。
...
> 現在 `wlsim` 自己載 `MMAP.MAP`（254 條路），走的才是原版的道路。
...
| ~~TALK #1F／#20／#23／#198~~ | ✅ **已接上**（2026-08-23，[`77`](77-rout-talk-messages.md)）。⚠ 同時修正了這一份原本的敘述：**敗走走不到 #20**，而 `#198` 是組編號不是索引 |
