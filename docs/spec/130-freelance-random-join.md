...
| ~~訊息 `0x29` 的畫面對拍~~ | ✅ **不做**（使用者裁定 2026-09-04）：文字已解（#41「{1}加入麾下了。」）、索引出自機器碼、接線有 `Tick` 層的測試釘住，**這樣就算 CONFORMED**。訊息框本身的版面另有 [`../re/66`](../re/66-message-box-geometry.md) 與 [`../playtest/41`](../playtest/41-m7-corrected-text-on-screen.md) 的抽樣 |
