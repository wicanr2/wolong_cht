...
⚠ **這一份不動勝負判定**，只接「階段值」這個輸入。
退卻中每拍遞減的 120 拍倒數（`byte_1D34A`）是另一件事，
已由 [`141`](141-retreat-countdown.md) 接上。
...
| 項目 | 現況 |
|---|---|
| ~~退卻的 120 拍倒數（`byte_1D34A`）~~ | **已接**（[`141`](141-retreat-countdown.md)，2026-09-06）。⚠ 順帶更正：`0x78` 是 **`sub_19A33`（戰鬥初始化）**寫的，`sub_1A8F6` 只寫 `byte_1D349`；先前這一列寫成「`sub_1A8F6` 起」是錯的 |
