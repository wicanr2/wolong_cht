...
| 步 | 常式 | 框 | 講話者 | 肖像 |
|---|---|---|---|---|
| 插圖 | `sub_13D09(al=0)` | **(56, 136) 288×176**（§1.0）| — | `IVENTGRF` 第 0 頁 |
...
### 1.0 ⭐ 插圖的左上角是 (56, 136)，從 VRAM 位址反推

`sub_13D09` 貼圖那一步的參數是**位址**不是格座標：

```asm
sub_13D68:
    dx = 3 / bx = 8 / cx = 0C13h / call sub_10C14   ; (48, 128) 304×192 的底
    ds = cs:word_19876                              ; 解好的圖在哪
    bx = 2A87h / ax = 0B012h / call sub_1FA37       ; ★ 貼圖
```

- `ax = 0B012h`：`ah = 0B0h` ＝ **176 高**、`al = 12h` ＝ 18 個 16 px ＝ **288 寬**
- `bx = 2A87h`：VGA 平面位址。640 px ÷ 8 ＝ 每列 80 byte ⇒
  `0x2A87 ÷ 80 = 136` 餘 `7` ⇒ **y = 136、x = 7 × 8 = 56**

⚠ 先前這一格寫 (64, 144)，兩軸各多 8 px——**格座標與位址兩種單位混在一起**。
`sub_10C14` 那個底是 (48, 128) 304×192，正好比插圖大 8 px 一圈；
用 16 px 粗格去湊就會落在 (64, 144)。
逐像素量出來的位移也是 (−8, −8)（[`../playtest/103`](../playtest/103-advise-scene-parity.md)）。
