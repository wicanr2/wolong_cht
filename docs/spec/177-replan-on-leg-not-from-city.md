...
**狀態：READY。** `sub_147BB` 有兩半：軍團的 `+0x0E` **≥ `800h`**（走在
...
| `internal/state/corpsorder.go` | `arriveDisband` 同上；補兵判準也加 `onCity` |
...
- `tools/parity_ck.sh 2100 2200 2300 2454 2500` 的軍團表回到 0 個 byte。
- 逐拍取數的第一個分歧往後推（現在是拍 2,455）。
- 單元測試：軍團走在邊上時改目標到邊的另一端 ⇒ 步進換號、座標接著動；
  改目標到邊外的據點 ⇒ 先走到較近的那一端。反對照：站在據點上改目標
  仍然走整條重算那一條。
