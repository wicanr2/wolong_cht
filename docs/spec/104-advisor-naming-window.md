...
### 1.1 ⭐ 選字表是**米色底黑字**，六格是黑底白字

兩組屬性都是立即值，**兩個半位元組的順序是「背景︰前景」**
（[`../re/52`](../re/52-slot-select-window.md) §4）：

| 畫什麼 | 常式 | 屬性 | 背景 | 前景 |
|---|---|---|---|---|
| 六格名字 | `sub_19223` → `sub_106FD` | `ax = 0F01h` ⇒ `ah = 0Fh` | 0 黑 | 15 白 |
| 本頁 96 個字 | `sub_1928A` → `loc_1F75E` | `ax = 9000h` ⇒ `ah = 90h` | **9 米色** | **0 黑** |

逐像素量到的是底 `(243,211,146)`、字 `(0,0,0)`
（[`../playtest/105`](../playtest/105-advisor-naming-parity.md)）。

六格的座標也在同一支裡：軍師名 `dx = 160h` ＝ 352、別號 `dx = 1C0h` ＝ 448，
`bx = 0A8h` ＝ 168；標籤各自在六格左緣 −16 ⇒ **336 與 432**。

選字格：`dx = 0D2h` ＝ 210 起、`bx = 0EEh` ＝ 238 起、間距 `0x14` ＝ 20，16 欄 × 6 列。

⚠ **聲母列自己有一條黑底**，範圍 (200, 216, 336, 16)——比熱區
`0x25`（216 起，第一格是空白所以點不到）**往左多 16 px**。
照熱區填會少一格，露出視窗的藍底龍紋。

⚠ **六格底下的游標線是 15 px**：`sub_19223` 的
`sub_1F1A3(si = dx + 0Eh)` ——從 x 畫到 x+14，**含端點**。

⚠ **三個按鈕的文字是置中的**，不是固定內縮：「重來」「繼續」寬 48
⇒ +8，「確定」寬 64 ⇒ **+16**。逐像素量到的文字左緣是 360／416／480。
...
| 對原版 ✅ | [`../playtest/105`](../playtest/105-advisor-naming-parity.md)：dosgolem 走完新遊戲流程（NEW GAME YES → 第一章 → 曹操 → 自定）拍到，抓到兩個缺陷（§2.1）|
...
| ~~選字格的邊界~~ | 本頁 96 個字的**位置**逐像素對過了（[`../playtest/105`](../playtest/105-advisor-naming-parity.md)）；`sub_1928A` 的 `dx = 0D2h`／`bx = 0EEh`／間距 `0x14` 與 remake 一致 |
| 選好字之後的畫面 | [`../playtest/105`](../playtest/105-advisor-naming-parity.md) 停在剛開啟的狀態，**選字、翻頁、換肖像**之後還沒拍 |
| 視窗外的背景 | 原版這一頁後面疊著 NEW GAME 的勢力清單與君主卡；`-open-naming` 直接開視窗，背後只有地圖。要對得走完整條新遊戲流程 |
