...
而 AI 自己很少打得到城壁——**它本來就不會自己破城**
（`CLAUDE.md` §7：說明書第 11 章整章在講破城要換陣形，玩家要自己操作）。
單張截圖抓到的機率太低，不值得用每張 40 秒的 Xvfb 迴圈去碰運氣。

### 4.1 ⭐ 為什麼 AI 打不到城壁：擋住它的是地形，不是城壁

拿據點 82 的攻城圖跑 2,000 幀量過（軍團 81 對 39）：
`hitStructure` 被呼叫 **10,020 次**，實際造成傷害 **0 次**。
逐次記下被擋的格子之後，成因是同一個：
...
⭐ **x 38–45 是抬高的地形，不是城壁。** 這張圖只有五段實體——
一段城壁（x ＝ 27，**開場就是破的**）與四道門（x ＝ 31、34）。
兵撞的是爬不上去的高地，而**地形不會壞**，所以耐久一次都沒有掉。

也就是說「條只亮 21 幀」既不是條的算式錯，也不是攻方停止攻擊：
**沒有東西可以打壞**。要看這條就得讓玩家自己指揮部隊去撞門。
...
| ~~攻方只在第 24–44 幀打城壁~~ | ✅ 查過了，**不是規則層的缺口**：被擋的格子上根本沒有實體（§4.1）|
