...
⚠ **「剛進戰場」不是一個取樣點，是兩個。** 原版進場後
**前 45 個戰術節拍兩軍幾乎不動、而且沒有對白框**，開場對白要到
**節拍 50**（守方 50、攻方 67）才出現（[`../playtest/70`](../playtest/70-dosgolem-tactical-commands.md) §3）。
取樣點要寫**節拍數**，不要寫「開場」。

⚠ **`sb-minimap` 不是穩定區。** 它會**隨時間變**：同一個快照什麼都不點、
只跑兩千萬道指令，那一區就差 488 px（[`../playtest/69`](../playtest/69-dosgolem-tactical-controls.md) §3.1）。
它的 0 px 只在同一個時間點才有意義。
