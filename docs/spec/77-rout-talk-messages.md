...
`#20` 沒有實作——照 §1.1，敗走這條路徑到不了它。
戰鬥脫身那條路目前用的是自畫的一行（`battleLine` 的「部隊壞滅（脫身）」），
接原文是另一件事，記在 §5。
...
| 項目 | 現況 |
|---|---|
| `sub_10CDE` 做什麼 | `#23` 前面那一個呼叫。變數推得出是主將名，但那支本身沒讀 |
| 戰鬥脫身的 `#1F`／`#20` | 同一支 `sub_12977` 也服務戰鬥脫身；remake 那條路現在畫的是自己的句子，沒接原文 |
