...
`sub_12708` 寫完座標就落到 `loc_126F5`（佔用圖 +1）然後 `retn`——
**沒有 `sub_14325`**。分派只發生在下一次「輪到移動」，那時
`cmp bx, [si+14h]` 才相等（`+0x0E` 已經在到站處理 `sub_127A2` 換成
據點編號 × 8）。

remake 原本在 `step` 回 true 之後就順手呼叫 `arriveCorps`，
於是整條 Stage 機早一個移動週期：同局面拍 2,400 的軍團 19
原版 Stage 10、remake 已經 Stage 9。

### 2.2 AI 編成也只設意圖

同一個節拍問題的另一半：`sub_16E8F` 編出軍團之後**只設 `+0x20`**，
`+0x14` 留在腳下的首都。所以新軍團第一次輪到走的是**到站分派**
（`+0x0E == +0x14`），由 `sub_14548` 把意圖落實成行軍目標，
**第二次輪到才踏出第一步**（[`170`](170-ai-target-takes-two-ticks.md)）。

remake 的 `formAICorpsTo` 原本編成當下就 `March`，少掉一個週期——
症狀是新編的兩支軍團比原版多走兩格（同局面拍 2,330）。
...
`tools/parity_ck.sh` 十四個檢查點：

| 檢查點 | 據點表 | 軍團表 |
|---|---|---|
| 200／700／1,200／1,600／1,900 | **0** | **0** |
| 2,120／2,140／2,200／2,300／2,315／2,330／2,340／2,350／2,400 | **0** | **0** |
| 2,430 | **0** | 1 B（軍團 50 的朝向）|
| 2,454 | **0** | 3 B（軍團 19 的 Stage 機）|
| 2,500 | **0** | 14 B（同上）|

⭐ **據點表在每一個檢查點都是 0。** 逐拍取數不一致 210 → **187 / 5,480**，
第一個分歧在拍 2,647。

## 5. 未解

- 拍 2,454 起軍團 19 的 Stage 機仍快一步：原版 10 → 9 → **8**，
