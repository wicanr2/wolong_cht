...
| 每格 4 層上限 | 刻意沒做（§4）|
| ~~`sub_12B3C`~~ | **畫一支軍團**（2026-09-02 讀完）：`or byte ptr [si], 10h` 設**位元 4** → `bl = [si+3] & 3` 與 `al = [si+21h] << 2` 合成索引，查 `es` 那張 4 B／筆的表取三個值 → 由軍團座標 `[si+10h]`／`[si+12h]`（[`../mechanics/20`](../mechanics/20-military.md) §5 的 X／Y）算出貼點，`ah = 3` 呼叫 `loc_1D51F` 合成 → **小地圖開著時**（`byte_198A6` 位元 2）把座標各右移一位加上 `1B6h`／`26h`，用 `ah = 3Fh`（`[si+3] & 1` 成立時改 `0FAh`）呼叫 `sub_15D19` 點一個點。⚠ **不是「擦除舊位置」**——小地圖那一段是明確的畫點 |
| 首都疊圖為什麼不畫 | **機制讀出來了**（2026-09-02）：`sub_1D4C7` 是「往一格推一個疊圖」，**一格最多 4 個**——`bl = [si+1]`（目前疊圖數）`cmp bl, 4 / jnb` 就整個跳過，而且 `[si]` 的 **bit 4 設起來的格子完全不接受疊圖**。顯示清單一格 8 B：`+0` 旗標（bit 4 ＝ 不可疊、bit 5 ＝ 髒）、`+1` 疊圖數、`+3`–`+6` 四個槽；換進去的編號與原本不同才 `or byte ptr [si], 20h` 打髒。⚠ **剩下的是「首都那一格撞到的是哪一條」**——四槽滿了，還是 bit 4 被設起來，要在同一格上量 |
