...
| [`spec/84-multilanguage.md`](../spec/84-multilanguage.md) | **英文清單的欄位塞不下羅馬化人名** | 欄寬是照 3 個全形字排的（人名欄可用 7 個半形格），而羅馬化名字最長 12 個字母。目前靠 `fitCell` 裁切（切在連字號上就去掉連字號、名只剩一兩個字母就只留姓），代價是 `夏侯淵`／`夏侯惇` 都顯示成 `XIAHOU`。真正的解法是為半形語系另排一套欄界——**四個清單家族加起來吃不下 368 p… | 靜態 |
