...
**一個遊戲日 ＝ 23 「時」× 9 子刻 ＝ 207 個 tick。**

⭐ **`0 時`不存在，而且時只到 23。** 進位條件是
`cmp byte ptr ds:0CF3h, 17h` ＋ `jb`——**嚴格小於 23**，所以時只被 `inc`
到 23；時 ＝ 23 那一拍走的是換日那條路，而換日把時歸零之後落到
`loc_11DE0` 又 `inc` 一次，新的一天因此從 **1 時**開始。
實測見 [`../spec/167`](../spec/167-day-is-23-hours.md)：攔四支確認每個子刻
剛好推進一次時鐘，200 個子刻走了 209 格——多出來的 9 格正是那一次換日。
...
已解的三條：`sub_13E11` 是每「時」的軍團輪替（每次處理一支，
`word_10D1C` 每次 `+0x40` 繞到 `0x580` 歸零 ＝ 22 支，
[`45`](45-corps-command-mode.md) §、[`16`](16-idle-clock-event10.md) §4.2）；
`sub_15358` 的九支子程式整份在 [`07`](07-monthly-settlement.md)（經濟公式全解）；
勢力記錄 64 B 的欄位表已補到 `+0x10` 信賴度、`+0x1A`／`+0x1C` 本月支出、
`+0x1D` 士氣基準、`+0x20`／`+0x21` 資金（24 位）、`+0x23` 據點數、
`+0x28` 好戰等級、`+0x2A` 外交官（散在 [`07`](07-monthly-settlement.md)、
[`08`](08-hourly-update.md)、[`59`](59-game-over-exit-codes.md)）。
