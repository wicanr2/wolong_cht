...
`ds:98A5h` 的三條路（`sub_11CD0`）：

| 值 | 行為 |
|---|---|
| `> 1` | `ds:0D2Ch`／`ds:0D2Dh` 歸零、遞減，**直接 `retn`** |
| `== 1` | `ds:0D2Dh` 沒累積到 `0A0h` 就 `retn`；到了才 `sub_20000(ax=2)`、三個變數歸零，再落到正常路 |
| `== 0` | 正常路：`sub_13EFD`（據點每 tick 處理）、`sub_125A3`… |

**前兩條都會跳過 `sub_13EFD`**，也就是那段時間據點不推進。寫入端兩處：

```asm
00011FD8  mov byte ptr ds:98A5h, 0Ch     ; sub_11F7F：滑鼠一動就設 12
000198A5  mov cs:byte_198A5, 8           ; sub_18810 擦掉訊息框之後設 8
```

⚠ **這對即時制的時間模型有意義**（[`../mechanics/15`](../mechanics/15-realtime.md)）：
原版在「玩家剛動滑鼠」與「訊息框剛收掉」之後會停世界十幾個回呼。
remake 目前沒有這一段——**要不要對齊是規則層的決定，不是這份筆記能定的**。

## 6. 未解

| 項目 | 現況 |
|---|---|
| ~~`ds:98A5h` 那個延時器實際在等什麼~~ | **兩個寫入端都找到了**（2026-09-02，見 §5.1）：`sub_11F7F` 在滑鼠一動時設 `0Ch`、`sub_18810` 在擦掉訊息框之後設 `8`。⭐ **倒數期間 `sub_11CD0` 提早 `retn`，跳過 `sub_13EFD`（據點每 tick 處理）**——所以那不只是「等一下再顯示游標」，是**短暫停住世界**。剩下的窄問題：`sub_20000(ax=2)` ＝ 滑鼠層跳表的第 2 號（`sub_2009A`，回傳快取座標並清旗標），為什麼要在這裡叫它 |
