...
loc_11D20:
  ...
  ah = [bx+425Eh] / al = [bx+4241h]  ; 君主的說話型與肖像
  cx = 197h / call sub_18810         ; ★ 組編號，展開 ＝ 414 ＋ 說話型
  al = 2 / call sub_11CB1
```

⭐ **`0x197` 是組編號不是索引。** `TALK.DAT` 索引 ≥ `0x196` 是八格一組
（[`25`](25-message-variants-and-personnel.md) §1），展開成
`0x196 + (0x197 − 0x196) × 8 + 說話型` ＝ **414 ＋ 變體**：

| 變體 | 索引 | 文字 |
|---:|---:|---|
| 0 | 414 | 你做得實在太好了，要好好褒獎你！ |
| 1 | 415 | 嗯，我心滿意足了！今後也好好為我出力吧。 |
| 2 | 416 | 嗯，你的努力使我完成了大願，我向你道謝。 |

說話者是君主（主公型，說話型 0–2），3–7 那五格是空的——
**組內只有前三格有內容，正好對上主公型的值域**。
第一則 `#75` 是「有好消息稟告！！已經再也沒有敵對勢力存在了。」，
`al = 93h` ＝ 不帶肖像。
