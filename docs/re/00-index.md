...
| [`84`](84-popup-row-band-and-world-cursor.md) | ⭐ 彈出選單**不是按絕對座標選列**（`sub_10414` 把游標夾在 y 100–131 的帶，跑出去就上／下移一格）；大地圖上**滑鼠座標就是世界座標**，捲動原點就是鏡頭 |
| [`85`](85-march-target-hit-test.md) | ⭐ **行軍目標怎麼判定**：`sub_1709E` 兩道閘（圖塊 `0CBh`–`0D3h` ＋ 據點表登記的 X／Y 完全相等），所以**一座城只有登記的那一格按得到**；游標的格座標是 `⌊原點÷16⌋＋⌊畫面÷16⌋` 兩次捨去的和；更早一道閘是「游標壓在熱區上就不問」 |
| [`86`](86-march-turnback-at-peace.md) | ⭐ **和平就出不了兵**：`sub_142AB` 不是玩家下令的寫入端，而是**行進中的掉頭判定**——前方那一端的據點屬於別的勢力、非中立、而且我方與它和平，就把目標換成路的另一端。軍團因此在邊界折返，停在上一個自家據點 |
| [`87`](87-opening-deployment.md) | ⭐ **開場擺位**：`sub_19C45` 把每個兵放在戰場邊界那一欄（側 0 `X=1`、側 1 `X=62`），**Y ＝ 亂數 `& 0x1F` ＋ 0x10 ⇒ 16–47**，**不查佔用**所以會重疊；之後他們才走進陣形位置。靜態找不到（沒有交叉參考），是用執行期的記憶體寫入監看抓到的 |
| [`88`](88-mouse-cursor-visibility.md) | ⭐ **游標畫不畫由 `byte_20100` 一個 byte 決定**，不是由「哪一種視窗開著」。原版**一條 `INT 33h AH=1／2／9` 都沒有**，紅箭頭是自己畫上 VRAM 的；`seg002` 是一層相容包裝（`ax` 就是服務號），顯示 23 點、隱藏 31 點。`ipeek:20100:1` 可以直接讀 ⇒ 對拍不必再猜 |
