...
| 項目 | 現況 |
|---|---|
| ~~`loc_1F75E`~~ | **✅ 已解**：字模由 `INT 15h AH=50h` 服務提供，兩條 `call far` 向量在開機時被 `sub_1F720` patch 進去。見 [`29`](29-font-service-int15.md) |
| ~~`sub_1F878`~~ | **✅ 已解**：它只做寬度分類（`cmp ah, 80h`），**不查表**；`ax` 原封不動傳給 blitter 當字碼。查表在字型服務那一側 |
| `sub_1F7A4` | 把 32 B 字模緩衝畫上 VRAM 的實際迴圈，未逐行讀 |
| 屬性的其餘位元 | bit 2 是陰影已證實；`0x9001`／`0x9000` 的 bit 0 差在哪未讀 |
| `word_10D54` | 數字字型段的來源未追。**數字不走 `INT 15h`**（`sub_1069A` 直接從 `ds` 取點陣），所以與 §29 是兩條路 |
