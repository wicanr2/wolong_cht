...
**狀態：入口、尺寸、自動連接與 `MMAP.MCH` 物件圖形入口已定案。**
...
add  ax, 800h   → ds:987Ch      ; 10,240 B  ← 據點景觀圖／存檔槽頭的暫存段
add  ax, 280h
```

（單位是段＝16 byte。）

⚠ **大小不能從相鄰兩個指標的差算出來**——這串指標**有互相重疊的**。
`ds:987Ah` 與 `ds:9876h` 只差 `0x1200` 段，但 `word_19876` 那塊要放
`MMAP.MDL`（32,768 B）＋ `MMAP.MCH`（43,058 B）＝ **75,826 B**，
差值 73,728 B 裝不下；`word_1987A` 其實是**指進 `9876` 那塊裡面**
（`MMAP.MCH` 的 `0xA000`：32,768 ＋ 40,960 ＝ 73,728 ＝ `0x1200` 段，見下）。
所以上面那一欄是各緩衝區**實際要用多大**，不是「下一個指標減這一個」。
...
`word_1987A` 是 `word_19876 + 1200h` 段，亦即 `MMAP.MCH` 內的
`0xA000`。`sub_12533`／`loc_1D51F` 從這裡讀 0x100-byte object metadata
與後續矩陣；完整格式與事件 12 type 1／2 查表見
[`docs/re/14-mmap-mch-objects.md`](14-mmap-mch-objects.md)。
...
**`sub_1E717` 已讀完**（[`08`](08-hourly-update.md) §7.1–§7.3）：
它是 **192 圈的建表迴圈**（＝據點數），`byte_1E47E` 從 192 往下數、
`byte_1E47F` 從 0 往上數，每圈填一筆 8 byte 記錄並把計數器蓋進地圖格。
建出來的就是**行軍用的道路圖**（254 條邊），移植在
`internal/assets/world/roads.go`。

**剩下的入口**：

1. **`sub_1E68C`** —— 自動連接實際怎麼決定替換後的圖塊編號。
2. **`off_159D2` 跳表** —— `byte_198A6` 的每個位元對應一項，
   `bx` 從 2 走到 8（四項）。圖層開關的候選。

## 4. 還沒解的

- `MMAP.MCH` 的 object **type 3**：事件語意、原版 object timer 與畫面
  逐 frame 時序仍未解（type 1／2 的火災／暴動已解，見
  [`14`](14-mmap-mch-objects.md)）。
- `word_19872` 的 98,304 B 畫布怎麼對應到螢幕
  （尺寸與展開後的地圖相同，很可能就是那張 384×256 的格子圖）。

已解的兩條：`MMAP.MAP` 的編碼是 RLE，`sub_1F5E7`，
整份收在 [`../formats/06`](../formats/06-mmap-rle.md)；
`sub_20000` 是 **`int 33h` 滑鼠驅動的分派表**，不是 overlay 載入器
