...
```text
offset +0 ：18 19 1A 1B 1C 18 19 1A
offset +8 ：20 21 22 23 20 21 22 23
offset +16：28 29 2A 2B 28 29 2A 2B
offset +24：00 00 00 00 00 00 00 00
```

⭐ **`bx = type × 8 + 相位`，所以 `bx = 0` 就落在 `985Ah`——那八個 byte 是
type 0，不是 type 1。** 整張表往下對一格：

| 原版 object type | MCH metadata index | 尺寸 | 語意 |
|---:|---|---:|---|
| **0** | `0x18`–`0x1C`，循環重用 | **16×9 tile** | **會飄的雲**（常駐 16 個，[`../spec/146`](../spec/146-map-cloud-objects.md)）|
| 1 | `0x20`–`0x23`，循環重用 | 5×5 tile | 事件 12 火災（`sub_134B1` 的 `ah=1`）|
| 2 | `0x28`–`0x2B`，循環重用 | 5×5 tile | 事件 12 暴動（`ah=2`）|
| 3 | `0x00` | 3×3 tile | 目前沒有可見的產生端 |

> **訂正（2026-09-06）。** 本節初版把 `985Ah` 標成 type 1，於是
> remake 的 `ObjectPatternIndex` 寫成 `(objectType-1)*8+frame`，
> **把火災畫成 16×9 的雲、暴動畫成火災的 5×5 圖**。
> 現場證據：`root-saveb` 跑到 196/4/21 8 時，`peek` 出 16 筆物件
> `+0x0E` 全是 0，而畫面上那朵雲量出來正是 16×9
> （[`../playtest/87`](../playtest/87-map-clouds.md)）。
...
暴風雨範圍，沒有呼叫 `sub_123FF`——**暴風雨不建立物件**。
它改變的是既有那 16 朵雲的活動範圍：`sub_1248A` 把座標夾在
`word_10D22`–`10D28` 內並反向，所以起了暴風雨就把雲關進那 11×11 格
（[`../spec/146`](../spec/146-map-cloud-objects.md) §2）。

⭐ **後 16 筆（段內 `0x2140`／區塊 `0x21C0`）不是執行期生成的，
是存在劇本與存檔裡的**——所以載同一份存檔就會得到同樣的雲。
