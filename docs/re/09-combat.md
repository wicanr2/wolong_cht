...
**狀態：戰略層的戰鬥全解。戰術畫面（`sub_19FA0`）還沒碰。**
...
⭐ **只有玩家的勢力捲進去，才會開戰術畫面。** 其餘一律自動判定。
另外還有兩個例外會退回自動判定：

- `test byte ptr [si], 4` 有設 —— 該軍團這一輪不進畫面
...
| 缺口 | 位置 |
|---|---|
| **戰術戰鬥本體** | `sub_19FA0`，配 `BATTLE.*` 資產。這是下一個大塊 |
