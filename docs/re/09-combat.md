...
進戰術畫面**之前**先跳一則訊息（`cx` 就是 TALK 索引，
[`../spec/105`](../spec/105-encounter-goes-straight-to-battle.md) §4.5）：
野戰 `1Dh`／攻城玩家攻 `1Ch`／攻城玩家守 `1Bh`／空城自動判定後 `1Ah`。
⭐ 野戰那一則的**兩個 `{1}` 是攻守兩個主將**，而且玩家守方那條路
先 `xchg si, di`（`0x14E9F`），所以第一個一律是玩家那一方。
另外還有兩個例外會退回自動判定：
...
**第一，`+0x0E`／`+0x0F`／`+0x10` 不是兵種適性，是場合適性。** 索引它們的是
戰場類別（攻城 0／野戰 1／水戰 2），不是兵種。原本 `docs/formats/08` 把
`+0x0E`–`+0x10` 記成「疑似兵種適性」。
⭐ **三格都到得了**：戰略層的這一支只餵得到 0 與 1，但戰術層的
`sub_19C13` 用 `bl += cs:byte_1D34B` 選欄，而戰場圖塊 ≥ `0xD1` 就是類別 2
——`+0x10` 是**水戰適性**，DOS/V 的規則戰場裡有 84 格落在那個區間
（[`78`](78-soldier-power-from-command.md) §2.1）。
...
所以打輸一場之後士氣必定低於 100，下一場再輸就壞滅——
輸了就得撤回據點養士氣，這是原版的硬性節奏。

⚠ **士氣歸零 ≠ 一定壞滅。** `sub_151B3` 對勝方一樣清零、
`sub_1474A` 也一樣回 STC，但那個 STC 有沒有消費端要看入口：
**攻城（`sub_14ADE`）只判敗方**，勝方的旗標根本不看
（[`../spec/187`](../spec/187-only-the-loser-is-judged.md)）。
野戰（`sub_14A7B`）才是誰壞滅就判誰。

⇒ 說明書 5.5 的「100 を切った状態で**戦闘して負けると**軍団が壊滅」
在攻城那一條是準確的。
...
自動判定裡大將槽保底 1，所以那兩條裡實際上只有士氣 0 會成立；
`[si+29h]` 那一條是給戰術層用的（那裡部隊會被打光）。
⭐ **但壞滅還有第三個入口：`sub_1487B` 找不到退路**（下面那一段）。
...
| 地形係數表的列 2 | 要 `al = 2` 才取得到，**戰略層**沒有呼叫點。⚠ 適性 `+0x10` 已經不是缺口——戰術層的 `sub_19C13` 取得到（[`78`](78-soldier-power-from-command.md) §2.1）|
