...
| 操作提示 | 沒有視覺提示（玩家自己知道要按鍵）| 說服／判決畫面上加一行「Enter 繼續」等提示（`cmd/wlgame/advise.go`）| 原版每一句都停下來等按鍵（`docs/spec/45` §1.1），而現代玩家沒有說明書 | 只加一行字，不改流程；畫面對拍上是 `command` 1,369 ＋ `map` 1,509–1,604 px 的已知殘差（`docs/playtest/121`）|
| 系統選單列數 | 六列 | 多三列：主君編成（`docs/spec/76`）、損害報告（`docs/spec/89`）、戰後結果秒數（`docs/spec/156`）| remake 自己的設定需要出口 | 畫面對拍上是 `map` 約 15,000 px 的已知殘差 |
