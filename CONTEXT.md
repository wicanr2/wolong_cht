...
| **其餘視窗** | ⭐⭐ **武將一覽（編成清單）、財政、數值輸入器三格逐像素 PASS（0 px）**；編成視窗 0.20%＝原版錄影的游標（不可消）（`docs/playtest/42` §4.1）。修掉：欄位對齊（文字欄＋8、數字右靠分隔線右緣）、8×16 原版數字字模、捲軸逐像素重畫（雙層斜角鈕＋3D 凹槽、無滑塊）、編成過濾（`0x176A0` confirmed；軍師也排除）、預設編成騎騎步步弓弓（`sub_16D56`）、收入／支出＝月結快照（`sub_1548F`）| 進言五項選單受擷取輸入模型限制拍不到（`42` §5，右鍵回退探測進行中）；勢力／據點／軍團一覽未比；交戰目標勢力清單已 **PASS 0 px**（`42` §4.2，順帶定案**比例式捲軸滑塊**與勢力清單四格數字欄）；捲軸綠已定案＝**色 13**（四季常數，`docs/spec/38` §1.6）|
