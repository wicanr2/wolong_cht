...
**剩下的**：M6 的內容 parity（見 §2.1，主要是視窗底紋）與 M8／M9 實機驗收。
...
| M6 呈現層 | 主畫面五區逐像素 PASS、戰場九區裡六區 PASS，`field` 0.17%（未歸類的只剩 8 px）。**結局過場也會播了**（`docs/spec/67`）。剩下的是各視窗內部排版的估值（`docs/spec/12` §7）|
