...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **結局的播放** | 規格寫好了（[`spec/67`](docs/spec/67-ending-playback.md)，DRAFT），圖與文字的來源都到位。**遊戲現在贏了只有一個對話框** | 照 `spec/67` §2 的表接：十二幕 ＋ 第一幕的逐字文字 ＋ 十七階淡入淡出。第一幕是 **320 px 寬貼在 x=160**，不是整張 |
| **2** | **倒地動畫** | 純呈現。圖號已解：`0x168`（側 0）／`0x21C`（側 1）＋ 兵種組（大將騎馬 +0、弓 +4、步 +8）＋ 後兩幀再 +2（`sub_1B360`）| 帳已由 [`spec/65`](docs/spec/65-retreated-soldiers-survive.md) 分開，這一項只差畫 |
| **3** | **六份 READY 規格收尾** | `12`（各視窗內部排版仍有估值）、`32`（右鍵提前收掉未接）、`34`（最高速上限是 remake 差異）、`35`（22 勢力選擇視窗是簡化版）、`51`（DAC 算式未全面套用）、`91`（DRAFT）| 每一份的「remake 實作」欄留白處就是缺口 |
| **4** | **Windows／macOS 原生 GUI 實機驗收** | M8 唯一的閘。三平台候選包已產出，但沒有在目標作業系統跑過 | 需要實機或 VM，不是 Docker 能代的 |
| **5** | **Android 完整核心接入** | M9。觸控 shell 原型 APK 已產出並在模擬器跑過，遊戲核心還沒接 | [`docs/mobile/android-plan.md`](docs/mobile/android-plan.md) |
