...
剩下的發行閘只有下表那兩件實機驗收。
**多語系第一期已接通**（使用者裁定啟動，2026-08-26）：簡體
＝`-lang zh-hans`（OpenCC 機轉語系包 1,022 則＋3,130 字級表＋HZK16
字型，`spec/84`）、英文＝flag 與詞表機制（talk 未譯，fallback 繁中）。
第二期＝簡體逐則校訂、英文全量翻譯＋glossary、UI 字串全量抽出。
