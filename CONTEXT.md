...
| **音樂／音效** | **錄音鏈路已驗**（`docs/playtest/25`）：DOSBox 錄得到開場的 15 秒、轉 ogg 再解回來一致。remake 的播放層還沒開始 | `BGM.DAT` 的聲軌事件編碼未解，所以**不能解格式轉檔，只能錄原版**。而逐曲觸發還缺一段 RE：`YNSOUND.COM` 的「播放第 N 曲」command 沒有證據（`docs/spec/29` §3）|
