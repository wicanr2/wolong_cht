...
| **2** | **進言選單不在原版的位置** | 原版的五項選單在 **(0, 64)**（`sub_16224` 的 `dx = 400h`，粗格 → 像素見 `docs/spec/45` §2.1），而且是指令樹的一層；remake 用自己的小視窗畫在 (40, 60) | 四個階層選單的座標都已經解出來（`docs/spec/45` §2.1 那張表），照著擺就好；要不要一併重做指令樹是另一回事 |
