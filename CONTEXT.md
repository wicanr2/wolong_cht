...
- IDA 線性位址 `00017C6E` 的數值輸入器以初值、上限 `0x7530` 與目前值運作；`00017DA5` 追加十進位數字、`00017DC3` 乘 100、`00017DDD` 除 10、`00017DEC` **設成上限**、`00017DF1` 清零，`00017DEA` 結束並保留目前值。這些是已證實的數值變化，不把 PC-98 掃描碼直接外推成跨平台按鍵。（呼叫端傳的是**上限**不是初值，見 §6 與 `docs/spec/78`。）
