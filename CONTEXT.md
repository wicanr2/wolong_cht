...
| **2** | **AI 回家的「逐站前進」沒移植** | 原版一次只走到**下一個自己的據點**，兩個檢查各擋一種情況（`docs/re/65` §8.4）；remake 直接 `March(首都)` 一次算完。**原版的軍團在敵境裡根本走不動**，remake 的會走回去再被敗走判定攔下 | `headHomeResupply` 改成用 `sub_1487B` 的等價：沿路徑找第一個己方據點當目標 |
