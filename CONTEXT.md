...
| 目標 | 現況 | 卡在哪 |
|---|---|---|
| **主畫面** | **外框、版面常數與各視窗內部排版都照原版了**：外框取 `ICONGRF` 段 3（`docs/re/46` §3），四個常駐視窗的矩形出自機器碼（`docs/re/47`），十個顯示清單場景已接九個（`docs/spec/12`–`14`、`22`–`27`），連填色與立體邊的調色盤索引都照原版（`docs/re/48` §2.1）。自繪的只剩勢力色標 | **原版側進不到「四窗全開」的狀態**：主畫面在模擬器上收不到任何點擊（`docs/playtest/23` §4.1），DOS/V 的開新遊戲也停在「確定」。橫幅單獨對拍過，位移掃描落在 (0,0)。⭐ **同狀態對拍已經做得到了**：原版實錄影片裡就是四窗全開的主畫面，七個版面地標全部落在 3 px 內（`docs/playtest/27`）。剩色彩與縮小地圖的內容 |
...
| **3** | **戰場圖塊組的顏色** | 原版城壁是黃褐磚牆、remake 畫成灰色柵欄。假說是 `BATTLE.MDL` 三組選錯（`docs/playtest/27` §7.4），**沒驗** | `BATTLE.MAP` 索引第一欄就是圖塊組編號（`docs/formats/07` §2）；先確認 remake 用的是不是同一組 |
| **4** | **主畫面縮小地圖** | 原版有藍色海域與**多色的勢力據點點**，remake 是單色（`docs/playtest/27` §4）。這是 §2.1 說的「自繪的只剩勢力色標」那一項 | 先找原版縮小地圖的繪製常式 |
| **5** | **M7 文意層對照** | 最大的一塊人工，見下面第 1 項 | `translations/corrections.json` ＋ `docs/reference/02` §14 |
| **6** | **M8 各平台實機驗收** | 交叉建置的候選包還沒在目標 OS 跑過 | 見下面第 2 項 |
...
| M6 呈現層 | 視窗內部的龍紋；主畫面縮小地圖的海與勢力色；底列每格的命令圖示（`docs/spec/33` §5）；戰場圖塊組的顏色 |
