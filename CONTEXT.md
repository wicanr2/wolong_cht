...
| 目標 | 現況 | 卡在哪 |
|---|---|---|
| **主畫面** | UI 外框、命令列、勢力色標**都是自繪的**（`cmd/wlgame/strategyhud.go`），只有縮小地圖用原版素材（`ICONGRF` 段 2）| **`ICONGRF` 段 1 的 UI 語意未解**（`docs/formats/03` §5.3）。七個圖塊的位置與尺寸已定位，但畫面上各代表什麼沒定案。證據等級只到「強推論」，而且來源是**壓縮過的影片**不是執行檔 |
