...
| **野戰（非攻城）** | ⭐ **九區裡七區逐像素相同、地形 0 差**（`docs/playtest/43`）：用改造存檔讓呂布軍在原版裡自然撞上夏侯惇軍，LOAD DATA 讀入、遭遇、開戰。`field` 15% 全是狀態差（對白框台詞與時序、游標），小地圖 40 px | 開場對白的**選句規則**未解（兩邊抽到同組不同句）；⭐ 改存檔搬軍團時**佔用圖快取欄 `+0x1A`/`+0x1C` 要跟著搬**，否則原版讀檔後是幽靈（`43` §3）|
