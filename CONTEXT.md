...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **六個編成位置對戰力有沒有影響** | 自動判定確定不看位置（`sub_15285` 只把六槽相加，`combat.Power` 照抄）。但說明書 11.2 有「陣形の有利不利」專節——**如果陣形真的改變戰力，remake 少了一整條規則** | 讀說明書 11.2 抄進 `docs/mechanics/30-combat.md`，再對 `docs/re/11` 的 16 種陣形表 |
| **2** | **`sub_135ED` 與 `0x15604`：另外兩個動資金的地方** | `tools/ida_money_flow.py` 把錢的收斂點列出來了，這兩個還沒讀。`sub_135ED` **同時呼叫加與減**，很可能是轉帳；`0x15604` 是資金 `+=` 的另一個包裝 | `tools/ida_dump.py` 直接 dump 兩支 |
| **3** | **`sub_131AE` 的事件佇列沒有逐條解** | 每 10 次呼叫吃一筆 4 byte（碼 ＋ 參數）走 `funcs_131E8` 分派。remake 的 `dispatchQueuedEvent` 接了介面，**但分派表裡每個碼各做什麼沒有逐條對** | 攤開 `funcs_131E8`（`tools/ida_dispatch_tables.idc`）|

> 2026-08-16 使用者裁定的三項（行軍指示三選一、戰場圖塊組顏色、視窗底紋）
> 全部結案：第一項接上、第二項驗出是假問題、第三項解出來並畫上去了。
> 進言那條線（文字、畫面、五項指令、逐句節拍）也做完了。
>
> ⚠ **上一版的 worklist 三項全是假缺口。** `sub_13E11` 每「時」做什麼、
> 防災值怎麼成長、行軍費用與預備兵維持費——四樣**早就解出來，
> 而且四樣都已經實作了**（`World.hourly`、`internal/rules/governor`、
> `combat.Upkeep`）。那三項是照 `docs/playtest/31` §3 與
> `docs/mechanics/10`／`40` 的「未解」表抄的，**沒有先 grep `internal/`**。
> **寫進 worklist 之前要對照程式碼，不是只查文件的未解表**——
> 文件的未解表會過期，程式碼不會。現在這三項每一項都對過了。
