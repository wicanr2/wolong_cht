...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **`END_S*.DAT` 的像素格式** | 結局要放的十二張圖**沒有解碼器也沒有規格**——這是 M1 唯一真正的缺口（[`re/70`](docs/re/70-d7end-ending-player.md) §5）。開場 `OPEN_S*` 同一批 | 三個 consumer 的來源位移 `0`／`0xBB8`／`0xE74` 換成 byte 比檔案還大 → 載入時一定解壓過。先找 `sub_11167`（載檔那一支）在做什麼 |
| **2** | **結局的播放** | 順序、版面、節拍與**結局全文**都解了（[`re/70`](docs/re/70-d7end-ending-player.md)），缺的只有圖。**遊戲現在贏了只有一個對話框** | 圖解出來就能照 §3／§4 的節拍接；文字那一段不必等圖 |
