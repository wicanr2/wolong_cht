...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **視窗內部的龍紋底紋**（⚠ 靜態走完了）| 量到是**一條寬 32 的直條、水平完全重複（99.1%）、垂直不重複**，兩色；**圖案與橫幅背景同源**（段 0 的 plane 3 相符 90.2%），但乾淨的那一份不在任何原版檔案裡（`docs/formats/03` §5.5） | 只剩**動態追蹤**：對視窗內部那塊 VRAM 設寫入斷點。⚠ bridge 跑的是 PC-98，位址不能搬到 DOS/V |
