...
前一輪把三件大的做完了：**AI 的行軍決策鏈全解並接上**（`docs/re/65`、
`docs/spec/40`——世界會動了，43 個月的長跑見 `docs/playtest/33`）、
**訊息框的版面對齊原版**（`docs/re/66`、`docs/spec/41`）、
**M8 的五平台交叉建置一次跑完**（含 macOS 的 Ebiten 本體）。

下面按「投報比 ＋ 有沒有現成入口」排。**每一項都寫了下手點**，
不必回頭讀對話。逐畫面的差異盤點在
[`docs/playtest/31`](docs/playtest/31-parity-inventory.md)。

| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **開局主畫面的逐區對拍** | `docs/spec/90` 的管線每一段都有工具、正對照也接進 `check.sh` 了，**但從來沒對原版跑過一次**。現在 remake 的主畫面該有的東西都在（外框、底紋、縮小地圖標記、四窗），是第一次值得逐像素比的時候。開局 196/4/1 兩邊天然同狀態，不必先解決存檔共用 | `tools/dosbox.sh` 跑松崗版到主畫面截圖（密碼頁空白確認就過，`CLAUDE.md` §4.0）→ `-open-window -2` 截 remake → `tools/parity_diff.py` |
| **2** | **戰場的逐格對拍** | 同一個問題的戰術版：`docs/playtest/31` §1 記著「沒做過」。難度比主畫面高——要同一張戰場、同樣的雙方 | 先把主畫面那條管線跑通再說 |
| **3** | **五個開關圖示的來源段** | 主畫面右上角那五個小圖示（刀／旗／地圖／存檔…）remake 是自繪的，原版素材的段與位移沒定位 | `docs/re/48` 的顯示清單裡找貼圖那幾筆的來源位移 |

> 2026-08-16 使用者裁定的三項（行軍指示三選一、戰場圖塊組顏色、視窗底紋）
> 全部結案：第一項接上、第二項驗出是假問題、第三項解出來並畫上去了。
> 進言那條線（文字、畫面、五項指令、逐句節拍）也做完了。
>
> ⚠ **規則層連兩版 worklist 都撲空。** 前一版三項（`sub_13E11`、防災值成長、
> 行軍費用與預備兵維持費）四樣早就實作了；這一版三項（六個編成位置、
> `sub_135ED`／`0x15604`、事件分派表）逐一查下去也**全部已解已實作**——
> 陣形表 16 種 × 48 座標在 `internal/rules/tactical`、`sub_135ED` 的轉帳
> 與釋放俘虜在 `finishQueuedCeasefire`、13 個事件碼在 `events.go`
> `case 1`–`case 13` 一一對上。
>
> **兩次撲空本身就是資料**：規則層與 RE 的缺口已經很薄，
> 剩下的差距在**沒有量過的地方**——所以這一版換成對拍。
> 寫進 worklist 之前一律先 grep `internal/`，文件的未解表會過期。
