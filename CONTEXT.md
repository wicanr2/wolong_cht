...
1. ~~**行軍實作**~~ —— **已完成**（`docs/re/08` §7.5）。道路圖從 `MMAP` 推導：
   節點格 `0xCB`–`0xD3`（正好 192 格）在據點座標 `(X+4, Y)`，
   道路格 `0xB8`–`0xDD` **＋ `0xFF`（橋，量出來的）**，**8-連通**。
   推出 253 條邊，三個正對照全過：85/85 已知鄰接、192/192 連通、
   分支度 ≤ 4。程式在 `internal/assets/world/roads.go`ｘ
   `internal/rules/march`ｘ`state.SetRoads`，實跑 log 印「道路圖：253 條路」。
   **忠實呈現完成**（§7.7）：`roads.go` 現在是 `sub_1E4CE`／`sub_1E57F`／
   `sub_1E81C`／`sub_1E961` 的直接移植，不是等價重寫。
   先前的 BFS 版**邊的集合一樣但路不一樣**——253 條共同邊裡有 109 條
   （43%）長度差超過 2 格，BFS 會抄近路。
   連帶推翻兩個舊斷言：`0xFF` **不是橋**（是城池圖形本身），
   以及「道路要補值才連得起來」（原版在城門格終止，連通性是圖層的事）。
   253 條邊、最長路線 631 格／26 個據點、越界 0、不連續 0。
   以下是當初判斷「不是接起來就好」的量測（留著，因為它擋下了一次白工）：
   ⚠ **不是「接起來」就好**（`docs/re/08` §7.4）。
   據點記錄裡的鄰接量過了：170 條有向邊、全部雙向、**但只有 85 條無向邊、
   108 個連通分量**，最大的分量只有 17 個據點。從長安走不到洛陽。
   那四格只是「中間沒有路節點」的直接相鄰，真正的道路網是
   `sub_1E717` 載入 `MMAP` 時現建的，含路上（192–255）與野外（≥256）節點。
   **前置條件是先解 `MMAP` 的道路格語意**，然後在 Go 裡重建
   `sub_1E717`／`sub_1E81C`。以下是已解的部分：
...
3. ~~`sub_12BD9`~~ —— **已解**（`docs/re/07` §20）。它做兩件事：
   ...
   **按交友度由小到大排**，所以**第一筆就是最敵對的鄰居**。
   ...
   `sub_16A3D` **已解，而且先前記錯了**：它掃的是**自己的據點**
   （`cmp al,[si+1]` 比擁有者），選「類型最小、生產力最高」的那一個 ＝
   **選新首都**。事件 8 因此是**遷都**不是挑侵攻目標。
   四個劇本的初始資料驗過 41/43（兩個例外是作者刻意填的，
   劇本 2 的長阪本來就是戰場）。連帶解出**據點記錄 `+0x16` 低 4 位 ＝
   據點類型**（0 大城／1 中城／2 小城／3 關／4 戰場），
   與部落格說的「大中小城池、戰場、關口」對上。
   **已落到程式**：`internal/rules/capital`（選首都）＋ 兩個觸發點
   （首都易主、每小時 25% 主動遷都），`City.Kind`／`City.Adjacency` 已載入並寫回。
   **君主對軍師建議的採納判斷已解**（`sub_16475`，CLAUDE.md §6 點名的項目）：
   四個反應碼（0 拒絕／1 同意／2 要理由／3 已在交戰）＋ 四個理由的可用條件，
   訊息協定是「基底 ＋ 4 ＋ 反應碼 × 3」。好戰等級在這條鏈上出現三次。
   連帶**訂正 `internal/rules/persuasion` 的三個理由池**——原本照說明書
   分類猜的，三個裡兩個錯；原版選單就在 `TALK.DAT` #102／#166／#230，
   每個都是四個理由 ＋ 撤回。兩個暫定門檻也升級成 confirmed。
   **還缺**：第 ④b 道閘的意圖、`sub_18810` 的 `cx ≥ 0x100`
   （`0x1A4` 指到空訊息；`−0x20` 假說已被四個已驗證索引推翻）、
   以及 `sub_13639`／`sub_135AB`／`sub_16405`。
