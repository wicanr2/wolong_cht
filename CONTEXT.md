...
| 目標 | 現況 | 卡在哪 |
|---|---|---|
| **主畫面** | **外框、縮小地圖與版面常數全部照原版**：外框取 `ICONGRF` 段 3（`docs/re/46` §3），四個常駐視窗的矩形出自機器碼（`docs/re/47`）。自繪的只剩勢力色標與各視窗**內部**的排版 | **原版側進不到「四窗全開」的狀態**：主畫面在模擬器上收不到任何點擊（`docs/playtest/23` §4.1），DOS/V 的開新遊戲也停在「確定」。橫幅單獨對拍過，位移掃描落在 (0,0) |
