...
| **戰術畫面** | ⭐ **側欄七格全部照原版重畫**（`docs/re/60` ＋ `docs/spec/31`）：標題（戰場地名＋雙方君主）、上下兩面將旗＋主將名＋兩條計量條、小地圖部隊點、十六個陣形的選單、底列。**指令面板的列序也訂正了**——原版由上而下是突擊／攻擊／陣形／城壁／守陣／退却（命令碼 2/1/0/3/4）。證據 `docs/images/wlgame-tactical-sidebar.png` | 戰場圖塊組顏色不同（假說未驗）、底列六格在原版是**選部隊**不是下命令（`docs/re/60` §6.2）、同一場戰鬥的逐格對拍未做 |
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **底列六格：原版是選部隊，不是下命令** | `docs/re/60` §6.2：熱區 `0x15`–`0x1A` 切 `byte_1D310` 的位元 0–5，指令面板再把命令送給被選中的隊；**沒選 ＝ 全隊**。remake 目前把底列當成第二套命令列 | 說明書 4.3 的「黃框」就是這件事（`docs/mechanics/30-combat.md`）。要先決定 `tactical.Battle.Order` 怎麼收「哪幾隊」 |
| **2** | **攻方接觸城壁後就不再攻擊** | 量測的副產品（`docs/spec/32` §5）：拿真實攻城圖跑 2,000 幀，城壁只在第 24–44 幀挨打。城壁耐久 500、撞一次掉 1，照理該一直撞。**像是規則層的缺口，沒查過** | `internal/rules/tactical` 的 `hitStructure` 呼叫端；先確認攻方在接觸後的移動決策 |
...
| M6 呈現層 | 視窗內部的龍紋；主畫面縮小地圖的海與勢力色；底列六格要改成選部隊（`docs/re/60` §6.2）；戰場圖塊組的顏色 |
