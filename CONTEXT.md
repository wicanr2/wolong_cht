...
**剩下的**：M6 的內容 parity（見 §2.1）、M7 逐句校訂（1,022 則的文意層）、
M8 實機驗收。**接手順序見 §7.0 開頭那張表**。
...
| **戰術畫面** | ⭐ **側欄七格全部照原版重畫**（`docs/re/60` ＋ `docs/spec/31`）：標題（戰場地名＋雙方君主）、上下兩面將旗＋主將名＋兩條計量條、小地圖部隊點、十六個陣形的選單、底列。**指令面板的列序也訂正了**——原版由上而下是突擊／攻擊／陣形／城壁／守陣／退却（命令碼 2/1/0/3/4）。證據 `docs/images/wlgame-tactical-sidebar.png` | 戰場圖塊組顏色不同（假說未驗）、底列每格的**命令圖示**未接、同一場戰鬥的逐格對拍未做 |
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **⚠ 攻方大多數不前進（攻城打不下來的新卡點）** | 地形模型已經照原版重做（`docs/re/63`、`docs/spec/36`），攻方從「一次都沒碰到城壁」變成「打掉 480 點、最低那段剩 20」，但**攻城還是打不下來**。量到的是：48 個兵裡只有 4 個在撞牆，X 分佈從第 100 幀到全滅一格沒動（`docs/playtest/30` §3）| 查攻擊命令的目標選擇（`sub_1AED2`／`docs/re/11` §5.8）。**這已經不是地形問題** |
| **1b** | **一覽表照原版重做（四張表）** | 原版的表頭字串就在 `KI.EXE` 裡（武將 `0x7758`／據點 `0x7578`／軍團 `0x72FC`），欄位與格式在 `docs/re/27` 全解——**這一項是「照著做」不是「還要解」**。remake 現在的欄位是自訂的（多「評價」、少「勢力／身分」）| `docs/re/27` ＋ `internal/ui/listwin`；差異清單見 `docs/playtest/31` §2 |
| **2** | **底列的命令圖示** | 位置名與待機兵條已接（`docs/spec/33`），缺每格 (54,6) 那張**目前命令的圖示** | `sub_1C673` 讀 `word_10D48` 段的 `命令碼 × 0xC0`。⭐ **那個段的大小已經解出來了**：`sub_100DF` 的配置鏈給它 `0x6C` 個節 ＝ 1,728 byte ＝ **9 筆 × 0xC0**，而 0xC0 byte 在 4bpp 下正好是 **24×16**。剩下的是「誰把資料載進去」|
| **3** | **戰場圖塊組的顏色** | 原版城壁是黃褐磚牆、remake 畫成灰色柵欄。假說是 `BATTLE.MDL` 三組選錯（`docs/playtest/27` §7.4），**沒驗** | `BATTLE.MAP` 索引第一欄就是圖塊組編號（`docs/formats/07` §2）；先確認 remake 用的是不是同一組。⚠ **不是調色盤組**：進戰場不重載色盤（`sub_19946` 只做淡入），而且色盤只有兩組（１６色／液晶，`docs/re/02` §6.2）|
| **4** | **M7 文意層對照** | 最大的一塊人工，見下面第 1 項 | `translations/corrections.json` ＋ `docs/reference/02` §14 |
| **5** | **M8 各平台實機驗收** | 交叉建置的候選包還沒在目標 OS 跑過 | 見下面第 2 項 |
...
| M6 呈現層 | 視窗內部的龍紋；底列每格的命令圖示（`docs/spec/33` §5）；戰場圖塊組的顏色 |
