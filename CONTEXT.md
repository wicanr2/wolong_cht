...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **M8 各平台實機驗收** | 交叉建置的候選包還沒在目標 OS 跑過 | 見下面第 2 項 |
| **2** | **非玩家的 Stage 0–3 決策鏈** | 玩家那一半與 Stage 8–11 已解（`docs/re/64`），AI 那四支還沒逐條讀 | `sub_1439D`／`sub_143AF`／`sub_1440F`／`sub_14466`；連帶的 `+0x21` 語意見 `docs/re/64` §6 |
| **3** | **肖像框的寬度** | remake 用 160 px（出自 `sub_18810`／`sub_1895D` 的常數），但**原版實錄影格上那個框的文字區量到約 275 px**（`docs/playtest/32` §4）| 先確認影格上那個是不是同一種框 |
| **4** | **視窗內部的龍紋底紋**（⚠ 靜態走完了）| 量到是**一條寬 32 的直條、水平完全重複（99.1%）、垂直不重複**，兩色；**圖案與橫幅背景同源**（段 0 的 plane 3 相符 90.2%），但乾淨的那一份不在任何原版檔案裡（`docs/formats/03` §5.5） | 只剩**動態追蹤**：對視窗內部那塊 VRAM 設寫入斷點。⚠ bridge 跑的是 PC-98，位址不能搬到 DOS/V |

> 順序是使用者裁定 2026-08-16：先把行軍指示的三選一串起來（**已完成**），
> 再做戰場圖塊組顏色（**已驗，是假問題**）與視窗底紋
> （**靜態找法走完、沒找到，排到後面等動態追蹤**）。所以現在輪到 M7／M8。
