...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **⚠ 登城機制的最後一塊：兩張地面高度圖** | 量完也讀完大半了（`docs/playtest/28` §5.1）：**remake 的攻方打不進城**。`sub_1B1B1` 已解——擋路的是**單位**不是地形高度、低平面水平跨格**沒有高度差上限**、高平面要高度完全相等，而 `PlaneHigh` 只由純 Z 那一支寫（騎馬被擋就在那裡）。**缺的只剩 `es:[di+0x7000]`／`+0x8000` 那兩張地面高度圖是誰填的** | ⚠ 已排除 `sub_1BB6D`／`sub_1BB3C`（只清到 0x6FFF、只設 bit 7），而且**全庫沒有用 `+0x7000` 位移寫它的指令**。要找的是「`di` 預設成 ≥ 0x7000 之後直接寫」的那種寫法。解出來才動 `tryMove` |
| **2** | **底列的命令圖示** | 位置名與待機兵條已接（`docs/spec/33`），缺每格 (54,6) 那張**目前命令的圖示** | `sub_1C673` 讀 `word_10D48` 段的 `命令碼 × 0xC0`；要先讀 `sub_100DF`／`sub_1006B` 定出那個段的實際基址 |
