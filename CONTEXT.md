...
| 開局鏡頭是「把首都放在畫面正中央」（`cmd/wlgame` 的 `viewCols/2`／`viewRows/2`）| **是首都 −(16, 12)**，水平偏左四格。拿松崗實機的兩個首都各量一次都一樣（`docs/spec/52`、`docs/playtest/37`）| **沒有出處就照直覺補的常數**。「置中」看起來太自然，於是沒有人去找原版怎麼算。差的 64 px 讓地圖區 66.6% 不同，而肉眼看兩張圖「差不多」——**這種差異要靠平移搜尋才分得出是位移還是畫錯** |
...
| `MMAP.MAP` 的 `0xCB`–`0xD3` 是「道路格」，載入時會被換成節點編號（`docs/formats/05` §3 舊版）| **那九個值是據點中心**，只出現在據點記錄座標的右邊四格；`sub_1E4CE`／`sub_1E57F` 只讀地圖、寫的是另一張連接表，**地圖本身沒有被改寫**（`docs/re/67`）| **從「誰掃它」推語意，沒有看「它出現在哪」**。掃它的那一支在建道路圖，於是值被命名成「道路格」——而據點本來就是道路圖的節點。把八個殘差位置換算回地圖格、印出圖塊編號，一次就看出來它們全是據點 |
...
| **2** | **那 4 格從哪裡加的** | 兩處量到 −4 都一致，規則可用了，但機器碼裡還沒找到加它的那一行。**留著會讓下一個碰到鏡頭的人重推一次** | 讀 `sub_1D66A` 的畫面起點（`es=0A0C8h` ＋ `di=0A00h`）與 `sub_1D615` 的索引（`docs/spec/55` §5）|
...
- ~~「機器碼寫 20、實機量到 16」那個謎~~：**鏡頭變數比畫面上 x=0 那一欄小 4**
  （`docs/spec/55` §2，兩個不同鏡頭各量一次都是 −4）。剩下的是
  「那 4 是哪一段加的」，掛在 `docs/spec/55` §5。
