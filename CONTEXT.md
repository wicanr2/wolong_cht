...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **四個視窗開著時的對拍** | 主畫面剩下的三個區（命令列、縮小地圖、自勢力情報）在開局全關，所以這一輪比到的是地圖。要比視窗內容就得先把它們點開 | ⚠ **進到大地圖之後 INT 33 的範圍變成整個世界**（`sub_120D6` 的 `0..0x17FF × 0..0x101F`），螢幕座標 ＝ 原始座標 − 鏡頭原點，所以送點擊的座標算法與選單裡完全不同。先量一次對映（游標在地圖模式被 `ax=2` 藏起來，要另外找探針）|
