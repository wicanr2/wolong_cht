...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **M7 畫面抽樣與排版 parity** | 文意層的變體組已全部掃過（`docs/reference/02` §14.1），剩下的是「把校訂後的文字放上畫面看有沒有溢出」 | `tools/talkdat_selftest.py` 已有 22 格行寬 guard；缺的是抽樣截圖 |
| **2** | **M8 各平台實機驗收** | 交叉建置的候選包還沒在目標 OS 跑過 | 見下面第 2 項 |
| **3** | **非玩家的 Stage 0–3 決策鏈** | 玩家那一半與 Stage 8–11 已解（`docs/re/64`），AI 那四支還沒逐條讀 | `sub_1439D`／`sub_143AF`／`sub_1440F`／`sub_14466`；連帶的 `+0x21` 語意見 `docs/re/64` §6 |
