...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | ⛔ **戰術戰鬥會卡死** | 把兵的開場體力改成軍團士氣（原版就是這樣，[`spec/61`](docs/spec/61-soldier-initial-hp-from-morale.md)）之後，一場戰鬥跑 95 萬 tick 一個 byte 都不動：側 1 的 48 個兵**站在相鄰格**卻打不到側 0 剩下的 2 個。開場剛好 100 時「受傷就退場」把它蓋住了 | `spec/61` §5 有 0.01 秒可重跑的重現。先解這個，`spec/61` 才接得上；接上之後將旗的體力條才會對 |
