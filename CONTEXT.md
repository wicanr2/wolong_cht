...
| **2** | **內政官沒有「因為據點被攻陷而歸來」** | `sub_14CF3` 換旗之後第一件事就是 `sub_14D63`（TALK #68「{2}內政官的{1}大人因為據點被攻陷而歸來了」）。remake 的 `capture` 沒有這一步，據點易主之後舊主的內政官還掛在那座城上 | 讀 `sub_14D63`，對照 `internal/rules/governor` 的任免路徑 |
| **3** | **上框在結果階段的內容** | 影格上君主會在**上框**回話（「好，來人啊…」），而 remake 的結果句走一般通知框。下框（軍師）已經接好了（`docs/spec/42`）| 事件 2／3 的結果句改走上框 ＋ 君主肖像；先確認原版的結果句是不是同一則 TALK |
