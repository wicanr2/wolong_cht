...
> 沒過關的是「畫什麼」——戰術側欄的組成有五項差異、
> 縮小地圖沒有海與勢力色、攻城缺門強度條。見 §2.1。
...
| **戰術畫面** | PARTIAL（`docs/playtest/20`）：幾何、命令面板、底列 glyph、初始相機、32×30 display grid 已接。⭐ 對過原版影片，**幾何落在 3 px 內、六個指令按鈕完全一致**（`docs/playtest/27` §7）| **側欄的內容組成不對**（原版是戰場地名＋雙方君主＋兩面將旗＋兩列圖示，remake 畫成兩塊兵數面板）、戰場圖塊組顏色不同、攻城缺門強度條；`BATTLE.MDL` 的像素格式已解（`docs/formats/07` §8）|
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **戰術側欄的內容組成** | 影片比對已經列出**五項具體差異**（`docs/playtest/27` §7.3）：戰場地名＋雙方君主那一格、兩面將旗、小地圖的部隊點、兩列 8 格圖示、底部 `▶▶` 列。幾何全對，缺的是「畫什麼」 | 側欄視窗與主畫面同一套 `sub_1895D`；用 `tools/ida_callsite_args.py` 查戰術那幾支的 `cx`（矩形）與 TALK 索引。先查 `docs/re/24` 有沒有現成證據 |
| **2** | **攻城的「門強度」條** | 小而完整。城門的圖塊值與實體記錄已解（`docs/formats/07` §7），只缺顯示 | 找讀城門實體記錄（`0x0C00` 起 16 筆）並畫條的那一支 |
...
| M6 呈現層 | 視窗內部的龍紋；**戰術側欄的內容組成**（`docs/playtest/27` §7.3 的五項）、主畫面縮小地圖的海與勢力色、攻城的門強度條 |
