...
| 目標 | 現況 | 卡在哪 |
|---|---|---|
| **主畫面** | **外框與縮小地圖用原版素材**（`ICONGRF` 段 3／段 2，貼法與原版 `sub_10C60`／`sub_10C77` 一致，見 `docs/re/46` §3）。自繪的是**勢力色標與版面常數**，後者來源是壓縮過的參考影片 | 差異點還沒逐項指認。要靠同狀態對拍——原版側的工具已就緒（`docs/playtest/21`），**還沒對過** |
