...
| **戰術畫面** | ⭐ **側欄七格全部照原版重畫**（`docs/re/60` ＋ `docs/spec/31`）：標題（戰場地名＋雙方君主）、上下兩面將旗＋主將名＋兩條計量條、小地圖部隊點、十六個陣形的選單、底列。**指令面板的列序也訂正了**——原版由上而下是突擊／攻擊／陣形／城壁／守陣／退却（命令碼 2/1/0/3/4）。證據 `docs/images/wlgame-tactical-sidebar.png` | 戰場圖塊組顏色不同（假說未驗）、攻城的**門強度條**原版算式已解但 remake 還沒畫、底列六格在原版是**選部隊**不是下命令（`docs/re/60` §6.2）、同一場戰鬥的逐格對拍未做 |
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **攻城的「門強度」條** | 小而完整，而且**原版那一側已經全解**（`docs/re/60` §11）：字在 (264,8)、條在 x 320 起 151 px、長度 ＝ `城門實體 +0x18 >> 4`、色 11／空白 0、只在更小時重畫 | 先寫 `docs/spec/`，再接進 `cmd/wlgame/battle.go`。城門實體記錄在 `0x0C00` 起 16 筆（`docs/formats/07` §7）|
| **2** | **底列六格：原版是選部隊，不是下命令** | `docs/re/60` §6.2：熱區 `0x15`–`0x1A` 切 `byte_1D310` 的位元 0–5，指令面板再把命令送給被選中的隊；**沒選 ＝ 全隊**。remake 目前把底列當成第二套命令列 | 說明書 4.3 的「黃框」就是這件事（`docs/mechanics/30-combat.md`）。要先決定 `tactical.Battle.Order` 怎麼收「哪幾隊」 |
...
| M6 呈現層 | 視窗內部的龍紋；主畫面縮小地圖的海與勢力色；攻城的門強度條（原版算式已解，`docs/re/60` §11）；底列六格要改成選部隊（`docs/re/60` §6.2）；戰場圖塊組的顏色 |
