...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **remake 畫不出奇數的戰術鏡頭** | 戰場對拍的對齊點落在 `camWorldY = 11`，而 remake 畫奇數會跳掉（1024 點差 423，偶數只差 2）。⭐ 原版縮圖點選的公式 `((x − 0x1F0) >> 1) \| 1` **把 Y 強制成奇數**——奇數才是原版的常態（`docs/playtest/40` §4.1）| 讀 `sub_1DD22` 的兩趟交錯（`add di, 40h`／`add si, 20h`）怎麼處理 `(Y−X)` 的奇偶 |
