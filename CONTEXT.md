...
| **戰術畫面** | ⭐ **側欄七格全部照原版重畫**（`docs/re/60` ＋ `docs/spec/31`）：標題（戰場地名＋雙方君主）、上下兩面將旗＋主將名＋兩條計量條、小地圖部隊點、十六個陣形的選單、底列。**指令面板的列序也訂正了**——原版由上而下是突擊／攻擊／陣形／城壁／守陣／退却（命令碼 2/1/0/3/4）。證據 `docs/images/wlgame-tactical-sidebar.png` | 戰場圖塊組顏色不同（假說未驗）、同一場戰鬥的逐格對拍未做。**底列的命令圖示已接**（`docs/spec/33` §1.5）|
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **戰場圖塊組的顏色** | 原版城壁是黃褐磚牆、remake 畫成灰色柵欄。假說是 `BATTLE.MDL` 三組選錯（`docs/playtest/27` §7.4），**沒驗** | `BATTLE.MAP` 索引第一欄就是圖塊組編號（`docs/formats/07` §2）；先確認 remake 用的是不是同一組。⚠ **不是調色盤組**：進戰場不重載色盤（`sub_19946` 只做淡入），而且色盤只有兩組（１６色／液晶，`docs/re/02` §6.2）|
| **2** | **視窗內部的龍紋底紋** | 量到是 **32×32 平鋪、只有兩色**（`docs/formats/03` §5.5），但 69 個原版檔逐 byte 掃過都沒中 | 下一步是 `sub_11AC3`（LOAD DATA 面板那條路徑）與 `sub_1FAC2`／`word_1987C` |
| **3** | **M7 畫面抽樣與排版 parity** | 文意層的變體組已全部掃過（`docs/reference/02` §14.1），剩下的是「把校訂後的文字放上畫面看有沒有溢出」 | `tools/talkdat_selftest.py` 已有 22 格行寬 guard；缺的是抽樣截圖 |
| **4** | **M8 各平台實機驗收** | 交叉建置的候選包還沒在目標 OS 跑過 | 見下面第 2 項 |
| **5** | **非玩家的 Stage 0–3 決策鏈** | 玩家那一半與 Stage 8–11 已解（`docs/re/64`），AI 那四支還沒逐條讀：它們讀據點 `+0x00` 的位元 6／7、據點 `+0x18`、勢力 `+0x18`，還有一次亂數決定計時器 | `sub_1439D`／`sub_143AF`／`sub_1440F`／`sub_14466`；連帶的 `+0x21` 語意見 `docs/re/64` §6 |

> 順序是使用者裁定 2026-08-16：先把行軍指示的三選一串起來（**已完成**），
> 再做戰場圖塊組顏色與視窗底紋。M7／M8 往後排。
...
| M6 呈現層 | 視窗內部的龍紋（已量到是 32×32 平鋪，來源未定位，`docs/formats/03` §5.5）；戰場圖塊組的顏色 |
