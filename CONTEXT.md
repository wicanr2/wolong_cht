...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **攻方接觸城壁後就不再攻擊** | 量測的副產品（`docs/spec/32` §5）：拿真實攻城圖跑 2,000 幀，城壁只在第 24–44 幀挨打。城壁耐久 500、撞一次掉 1，照理該一直撞。**像是規則層的缺口，沒查過** | `internal/rules/tactical` 的 `hitStructure` 呼叫端；先確認攻方在接觸後的移動決策 |
