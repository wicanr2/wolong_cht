...
**剩下的**：沒有逐像素對過的那幾個視窗（一覽表、編成、進言、財政）、
野戰的戰場，以及 M8／M9 的實機驗收。
...
| M6 呈現層 | 主畫面五區逐像素 PASS、戰場九區裡六區 PASS，`field` 0.17%（未歸類的只剩 8 px）。**結局過場也會播了**（`docs/spec/67`）。剩下的是**沒有逐像素對過的那幾個視窗**（一覽表、編成、進言、財政）與**野戰的戰場** |
