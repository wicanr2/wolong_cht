...
| **3** | **「回不了家就潰散」還沒實作** | 原版：Stage ≥ 10 的軍團若回首都的路要穿過非己方據點，就當場走 `sub_1291A`（俘虜／戰死）。規則已解（`docs/re/65` §8，強證據），remake 的路徑不看據點歸屬 | `internal/rules/march` 的路徑加「經過的據點屬不屬於自己」，再在 `arriveDisband`／`headHomeResupply` 用它 |
...
- ~~AI 行軍鏈的實跑觀察~~：跑了 43 個月，勢力 22 → 15、解體 18 次、
  不變量全程成立（`docs/playtest/33`）。順帶修掉 `wlsim` 遇到
  「等玩家決定」會空轉這件事。
