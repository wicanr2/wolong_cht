...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **據點中央的勢力徽記** | 主畫面剩下的差異**全部是這一格**：城池圖案逐像素相同，只有正中央那一格原版是紅底白框、remake 是圖塊原本的橙色（`docs/playtest/37` §4）。七座據點全一樣，而且**曹操與孫策的城看起來同色**，所以先別假設它是勢力色 | 已排除的：`sub_11CC9` 的兩個 overlay 只畫軍團（`sub_12AF4`）與災害（`sub_12533`）；`MMAP.MAP` 的 RLE 尾巴只有 ≤64 B。下一步是把那一格的圖塊編號印出來，再看 `MMAP.MDL` 附近有沒有紅色版本 |
