...
**下一項：把月結 `sub_15358` 的九支子程式讀完。**

理由：經濟公式（收入、募兵、維持費、生產力複利、災害觸發率）
全部在那九支裡，而且它們共用同一張已解出的勢力表（22 × 64 B）。
解完就能寫 `internal/rules/economy`，那是規則層的第二塊。
**不需要 oracle。**
...
2. `internal/rules/economy` — **下一項**，先讀完 `sub_15358` 的九支子程式
