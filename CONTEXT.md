...
#### ⭐ 接手順序（2026-08-25 更新，第九版）

**視窗與野戰這兩條線 2026-08-25 收斂**（細節在 §3 現況一覽與
`playtest/42`／`43`）：五張清單視窗 0 px、據點情報卡版面逐格一致、
單挑子系統全實作（`spec/80` CONFORMED）＋ 台詞逐組抽驗（`re/75`）。
同日把 mechanics 的缺口逐條覆核，**行為級收斂到 5 條機制＋2 條接線**：
災害傷害量、評價的去向、外交回報的信賴度增減、信賴度初始時序、
AI 長程決策對照；接線＝行軍中間節點（原版 `sub_127A2` 已解）與
事件 10 訊息顯示。剩下的發行閘只有下表那兩件實機驗收。
