...
| **⭐ 缺口總表建立** | `docs/re/43`（生成的，已接進 `check.sh`）。把各文件的「未解」小節、表格未解列與散句抽成一張表，共 **126 條**：規則正確性 7、資料保存 28、程式碼理解 82、外部資料 9。末段列出**提到未解卻抽不到任何一列的 22 份文件**——那是抽取器自己的盲區，不印出來的話解析失敗會長得像「那份文件沒有缺口」 |
