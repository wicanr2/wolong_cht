...
| **3** | **`sub_12A7E`：軍團「敗走」狀態的後續** | 回首都的路要穿過別人的地時，軍團的存在旗標被寫成 **8** 並設 `+0x03 = 48`（`docs/re/65` §8）。旗標 8 在 `sub_125A3` 有專屬 handler，**那一支還沒讀**，所以最終結果沒定案——不知道結局就不能實作 | 讀 `sub_12A7E`；`+0x03` 疑似計時器 |
