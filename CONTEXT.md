...
| **2** | **Windows／macOS 原生 GUI 實機驗收** | M8 唯一的閘。三平台候選包已產出，但沒有在目標作業系統跑過。⚠ Linux AppImage 已重打到今天（[`release/02`](docs/release/02-appimage-20260818.md)），另兩個平台還停在 `20260812` | 需要實機或 VM，不是 Docker 能代的 |
| **3** | **Android 完整核心接入** | M9。觸控 shell 原型 APK 已產出並在模擬器跑過，遊戲核心還沒接 | [`docs/mobile/android-plan.md`](docs/mobile/android-plan.md) |
