...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **玩家守城時戰場要轉 180 度** | 戰場對拍的 `field` 區差 87%，成因已定位：`sub_14ED7` 在玩家守城那一支 `or byte_10D35, 0C0h`，bit 6 就是翻轉，而 remake 連 `battlefield.Select` 算出來的 `rotate` 都丟掉（`docs/playtest/40` §4）| 先寫規格：翻轉發生在**載入**（`sub_1CAEB`），城壁移動目標 X 也跟著 `63 − v`（`docs/re/11` §4.5）|
| **2** | **戰場對拍的第二輪** | 第一輪只對到「同一張戰場」，雙方軍團組成不同（`docs/playtest/40` §5）。轉 180 度做完之後才有意義 | 原版側的取得方式已經固定下來：**編成一支軍團然後等 AI 來打**（`docs/playtest/40` §1），不必碰點不到的選單 |
