...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **兩份規格的真缺口** | 六份「未收尾」裡有四份是登記不實（2026-08-21 稽核，明細在 §6.1）。`32` 的**戰場區右鍵熱區層**已補（2026-08-22，熱區 `0x1D` 右鍵收掉門強度條）。真的還缺的是 `35` 的**22 勢力選擇視窗版面**與**熱區 `0x16`（點地圖）原版做什麼** | `cmd/wlgame/battle.go` 沒有 `MouseButtonRight` 分支；`0x16` 要讀 `sub_15A3A` 那一帶 |
