...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **⚠ 攻城的登城（梯子）機制** | 量完了（`docs/playtest/28`）：**remake 的攻方打不進城**。城牆四格厚而可破壞的只有一格，撞穿也開不出通道——原版靠**爬上牆頂**，而爬牆機制還沒 RE。攻方只有約 500 幀（大將每 10 幀掉 1 體力，50 觸發全軍退卻）| `Field.gateX` 在三張抽樣圖上**正好就是可破壞城壁那一格**，假說是「那一格可以無視高度差爬上去」。要讀 `sub_1AF69` 的 Z 分支與 `sub_1B0AF`，**別直接照假說實作** |
