...
已知：事件分派器是 `sub_131AE`（跳表 `funcs_131E8`，代碼低位元組 −1 當索引），
內政官事件是代碼 4（`sub_132A9`），它會呼叫 `sub_139E8`（付款對話）。
**還沒找到「誰把上昇值／防災值／城兵數加回去」**——
grep 過 `+10h`／`+11h`／`+13h` 的所有 add／inc，一個都沒有，
所以效果多半用了別的定址方式或藏在付款流程尾段。→ 下一輪的目標。
