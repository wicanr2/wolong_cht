...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **肖像框的寬度** | remake 用 160 px（出自 `sub_18810`／`sub_1895D` 的常數），但**原版實錄影格上那個框的文字區量到約 275 px**（`docs/playtest/32` §4）| 先確認影格上那個是不是同一種框 |
| **2** | **AI 行軍鏈的實跑觀察** | 機制已接（`docs/spec/40`），但只有單測；**長跑一場看軍團數與待辦佇列的走勢**才知道行為像不像原版 | `cmd/wlsim` 跑固定種子的長局，記錄各勢力軍團數、解體次數、`LostSite`／`ReliefSite` 的命中率 |
| **3** | **視窗內部的龍紋底紋**（⚠ 靜態走完了）| 量到是**一條寬 32 的直條、水平完全重複（99.1%）、垂直不重複**，兩色；**圖案與橫幅背景同源**（段 0 的 plane 3 相符 90.2%），但乾淨的那一份不在任何原版檔案裡（`docs/formats/03` §5.5） | 只剩**動態追蹤**：對視窗內部那塊 VRAM 設寫入斷點。⚠ bridge 跑的是 PC-98，位址不能搬到 DOS/V |
| **4** | **`sub_147BB` 與軍團 `+0x00` 位元 1** | AI 鏈裡唯一還沒讀的環節（`docs/re/65` §8）；remake 現在用「重下一次行軍」代替 | `sub_12662` 的 `0x126A0` 讀它、清掉、呼叫 `sub_147BB` |
