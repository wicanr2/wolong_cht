...
| **音樂／音效** | ✅ **會出聲了**：純 Go 的 OPL3（`internal/audio`）把 `*BGM.DAT` 與 `SOUND.DAT` 渲染成 ogg（`tools/bgm2ogg.sh`），`internal/ui/sound` 播，系統選單第 3 列開關。與原版錄音的包絡互相關 0.704（`docs/playtest/26`）| **哪一首配哪個場景未解**——只有開場那一首有證據，其餘是 remake 的選擇。音色的諧波結構也沒量化比對過 |
