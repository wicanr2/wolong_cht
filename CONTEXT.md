...
| ⭐ 玩家守城時戰場要轉 180 度，圖塊值也要換成鏡射版 | [`spec/56`](docs/spec/56-battlefield-rotation.md) |
...
| **2** | **翻轉之後雙方的佈陣點沒換邊** | 看得到的症狀：原版玩家守城時陣形線在 X=5（`sub_199F3` 的 `word_1D33C` 初值），remake 畫在 X≈58。戰場轉了，佈陣點沒跟著轉（`docs/spec/56` §5）| 讀 `sub_199F3` 之後誰用 `word_1D33C`／`word_1D33E` 佈陣 |
| **3** | **原版的鏡頭中途會自己移** | 初值是 `sub_199F3` 的 (36,14)，原版那一張開打 40 秒後停在 (36,13)。**不知道誰移的**，對拍每次都要重新掃一遍鏡頭 | 找誰設 `byte_1D348`（鏡頭 dirty flag）；已知 `▶▶` 列的 handler 會把鏡頭設成 (128,128)（`docs/re/60` §9）|
