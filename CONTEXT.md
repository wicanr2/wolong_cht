...
| 戰術呈現是逆向覆蓋率最低的一塊（`docs/re/20` 的 25–40%） | **它是覆蓋率最高的模組**：35 支函式只有 2 支未觸及。低的是**移植度**不是**覆蓋度** | 兩個數字量的不是同一件事，而稽核只挑戰術函式評估深度，沒有全量比較。全量普查（`docs/re/21`）才顯出真正的缺口在戰略 UI 與月結／經濟／AI |
...
| M3 反組譯 | 739 支函式中 355 支未觸及，缺口集中在戰略 UI 與月結／經濟／AI（`docs/re/21`）；另有上表的小項 |
| M4 機制文件 | 跟著 M3 的缺口走 |
