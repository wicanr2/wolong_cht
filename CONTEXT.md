...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **戰術鏡頭差一個等角格** | 戰場翻轉接上之後（`docs/spec/56`）`field` 區從 87.8% 降到 46.1%，再平移 (−16,−8) 就降到 21%——**16×8 正好是一格**。小地圖不用平移，所以翻轉中心是對的，差的是鏡頭（`docs/playtest/40` §4.1）| 讀原版守城時的鏡頭初值：`sub_199F3` 設 `word_1D328=0x24`／`word_1D32A=0x0E`，看攻守兩邊是不是同一組 |
| **2** | **戰場對拍的第二輪** | 第一輪只對到「同一張戰場」，雙方軍團組成不同（`docs/playtest/40` §5）| 原版側的取得方式已經固定下來：**編成一支軍團然後等 AI 來打**（`docs/playtest/40` §1），不必碰點不到的選單 |
