...
| **3** | **`sub_147BB` 與軍團 `+0x00` 位元 1** | AI 鏈裡唯一還沒讀的環節（`docs/re/65` §8）；remake 現在用「重下一次行軍」代替 | `sub_12662` 的 `0x126A0` 讀它、清掉、呼叫 `sub_147BB` |
| **4** | **事件場景的兩個講話位置** | 訊息框已對齊原版（`docs/spec/41`），但事件 2／3 的 `IVENTGRF` 場景上那兩個講話框只有機器碼算出的 (8, 88) 與 (136, 296)，**沒有影格覆驗** | 找一張有事件對話的原版影格，照 `docs/re/66` §5 的方式逐項量 |
