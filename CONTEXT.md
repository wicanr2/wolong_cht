...
M6 可玩但畫面不是原版的，音樂完全沒有實作。**

> **使用者的驗收標準是「畫面要跟原版一樣」**（2026-08-14）。
> 照這個標準，主畫面與戰術畫面**都還沒到**——UI 外框是自繪的、
> 戰術版面只到 PARTIAL。音樂與音效**一行都沒寫**。
> 見 §2.1。
...
**剩下的**：M6 的畫面 parity（見 §2.1）、M7 逐句校訂（1,022 則的文意層）、
M8 實機驗收。細節見 §7.0。
...
3. **音訊：原版側已全解，remake 側還沒開工** —— 容器（`docs/re/23`）、
   事件與控制事件（`docs/re/56`）、OPL3 暫存器與音色／音量／速度／`SOUND.DAT`
   （`docs/re/57`）都有出處。缺的是純 Go 的 OPL3 合成核心與 ogg 匯出
   （`docs/spec/29` §3–§4）。曲塊 `+0x04` 那張表仍未解，但**驅動沒有讀它**。
