...
| **1** | **`sub_13E11` 每「時」做什麼** | 它是**勢力層**的每小時 handler（`docs/re/65` §6），而行軍與 AI 的節拍都掛在上面。沒讀完就不知道 remake 的每小時漏了什麼 | `tools/ida_dump.py` 直接 dump；`docs/re/65` §6 已經確認它是勢力層不是軍團層 |
| **2** | **防災值怎麼成長** | 欄位早就解出來（據點記錄 `+0x0C`），但**成長規則未解**，所以天災的長期經濟是空的：remake 的防災值只會被天災扣、不會回來 | 從月結 `sub_15358` 往下找寫 `+0x0C` 的地方（`tools/ida_disp_users.py` 掃位移）|
| **3** | **行軍費用與預備兵維持費** | 說明書 10.6 有專節講行軍費用，`sub_15358` 尾段有維持費單價，兩個都沒讀。**經濟模型少了兩個支出項**，長跑的資金曲線會偏樂觀 | 說明書判讀（`docs/reference/01`）＋ `sub_15358` 尾段 |

> 順序是使用者裁定 2026-08-16：先把行軍指示的三選一串起來（**已完成**），
> 再做戰場圖塊組顏色（**已驗，是假問題**）與視窗底紋
> （**靜態找法走完、沒找到，排到後面等動態追蹤**）。
...
| M6 呈現層 | 視窗內部的龍紋（已量到是寬 32 的直條、水平重複，來源未定位，`docs/formats/03` §5.5）|
