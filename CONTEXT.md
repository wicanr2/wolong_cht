...
| **音樂／音效** | ⭐ **原版側全解了**：事件與控制事件（`docs/re/56`）、音色／音量／速度／`SOUND.DAT`／晶片型號（`docs/re/57`）。**音源是 OPL3，六個聲軌各佔一組 4-operator 通道** | 剩 remake 這一側：純 Go 的 OPL3 核心、離線渲染成 ogg、播放層（`docs/spec/29`，已 READY）|
