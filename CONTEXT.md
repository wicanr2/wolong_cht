...
| **反組譯改用 IDAPython** | `tools/ida_scan.py`＋`tools/ida.sh` 依 `.py` 副檔名自動換 image。全庫掃「對 `[reg+disp]` 寫立即值」133 處，解出軍團 `+0x00` 的位元圖（`docs/re/34`）：位元 1 ＝ 有行軍指令（到站清）、位元 2 ＝ 行軍中 |
...
前一輪把三件大的做完了：**音訊全線接通**（RE ＋ 純 Go OPL3 ＋ ogg ＋
場景對應）、**勝利條件解出來並實作**、**同狀態畫面對拍變成做得到的事**
（拿原版實錄影片，工具 `tools/parity_frame.py`）。
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **玩家勢力的預備兵被抽乾** | 完全不操作的玩家勢力三年內從 800/3200/4800 掉到 0/0/0，而它**不會自己編成軍團**——消耗端不明（`docs/playtest/33` §2.3）| 在 `internal/state` 找所有會減 `Faction.Reserves` 的地方，逐一對機器碼 |
| **2** | **視窗內部的龍紋底紋**（⚠ 靜態走完了）| 量到是**一條寬 32 的直條、水平完全重複（99.1%）、垂直不重複**，兩色；**圖案與橫幅背景同源**（段 0 的 plane 3 相符 90.2%），但乾淨的那一份不在任何原版檔案裡（`docs/formats/03` §5.5） | 只剩**動態追蹤**：對視窗內部那塊 VRAM 設寫入斷點。⚠ bridge 跑的是 PC-98，位址不能搬到 DOS/V |
