...
| **2** | **進言沒有逐句節拍** | 原版三句（上框 → 下框 → 上框）**要按鍵才往下走**，remake 直接顯示最後的狀態。這是唯一還看得出來的進言差異（`docs/spec/45` §3.1、`docs/playtest/35` §3）| 讀 `sub_10241`／`sub_102C2`——`sub_13B08` 每一句前後各呼叫一次，等待邏輯應該在那裡 |
| **3** | **進言選單不在原版的位置** | 原版的五項選單在 **(0, 64)**（`sub_16224` 的 `dx = 400h`，粗格 → 像素見 `docs/spec/45` §2.1），而且是指令樹的一層；remake 用自己的小視窗畫在 (40, 60) | 四個階層選單的座標都已經解出來（`docs/spec/45` §2.1 那張表），照著擺就好；要不要一併重做指令樹是另一回事 |
