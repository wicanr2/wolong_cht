...
| **主畫面（remake 自己的問題）** | ⚠ **兩套視窗實作並存**：`drawNaturalStrategyHUD`（照原版矩形，`g.hud` 控制）與舊的 `g.open[]`＋`windowRect`（自排位置，鍵盤 1–4 開關）**同時在畫**。截圖可見自勢力情報被畫在 (172,50) 而不是 (432,192) | 不是被格式擋住，是 remake 內部的重複實作（`CLAUDE.md` §7 第 6 條）。**這是目前主畫面最大的一項偏差，也是最好修的** |
...
| **音樂／音效** | **完全沒有實作**——`go.mod` 沒有音訊依賴，`internal/` 沒有音訊套件 | **`BGM.DAT` 的聲軌事件編碼完全未解**（`docs/re/23`）：音高、長度、迴圈、包絡都還沒讀。格式沒解，寫不出播放器 |
...
3. **`*BGM.DAT` 的聲軌事件編碼** —— 容器索引（11 曲，兩版都逐 byte 對齊到檔尾）
