...
**主畫面對完了，戰場也對到同一個局面了。** 戰場側欄四區逐像素 PASS
（標題／陣形列／指令面板／`▶▶` 列），將旗兩格 1.3–1.5%；`field` 19.9%
剩下的**不是雙方部隊不一樣，是城壁的面畫得不對**——換成同一個局面之後
那些差異原封不動（[`docs/playtest/40`](docs/playtest/40-tactical-parity.md) §5）。
