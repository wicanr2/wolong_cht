...
| **2** | **據點失守時受影響的軍團沒調頭** | `sub_14CF3` 打完換手之後呼叫 `sub_14DA4`：把那批軍團逐一交給 `sub_1487B` 改成「回家的下一站」，找不到退路的走 `sub_1291A`。remake 的 `capture` 只換旗，軍團留在原地 | `nextHopHome` 已經有了（`docs/spec/46`），缺的是 `capture` 之後掃一遍受影響的軍團 |
