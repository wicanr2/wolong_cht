...
| **音樂／音效** | ✅ 純 Go 的 OPL3（`internal/audio`）把 `*BGM.DAT` 與 `SOUND.DAT` 渲染成 ogg（`tools/bgm2ogg.sh`），`internal/ui/sound` 播。場景對應從機器碼讀出來（`docs/re/58`）：四季配樂、事件曲、攻城分玩家攻守、野戰與地形戰場，全部接上。與原版錄音的包絡互相關 0.704（`docs/playtest/26`）| 11 首裡曲 1 在 DOS/V 的 `KI.EXE` 沒有呼叫端。**音色的諧波結構沒量化比對過** |
