...
| **2** | **進言仍是清單視窗，不是兩個框輪流講話** | 文字那一半已經做完（§3 那張表最上面一列），剩下的是版面：原版是「`IVENTGRF` 插圖 ＋ 上框君主 ＋ 下框軍師」的模態流程（`docs/re/66` §5.2），remake 把整段對話堆在一個清單裡，所以要靠「君主：」「軍師：」標說話者 | 外交事件已經有現成的兩框接線（`enqueueAdvisorTalk`／`messageDialog.lower`／`scene`，`docs/spec/42` §3），把 `drawAdvise` 的 `advisePersuade` 換成同一套；選單五列走 `drawLegacyChoiceBox` |
| **3** | **AI 回家的「逐站前進」沒移植** | 原版一次只走到**下一個自己的據點**，兩個檢查各擋一種情況（`docs/re/65` §8.4）；remake 直接 `March(首都)` 一次算完。**原版的軍團在敵境裡根本走不動**，remake 的會走回去再被敗走判定攔下 | `headHomeResupply` 改成用 `sub_1487B` 的等價：沿路徑找第一個己方據點當目標 |
| **4** | **上框在結果階段的內容** | 影格上君主會在**上框**回話（「好，來人啊…」），而 remake 的結果句走一般通知框。下框（軍師）已經接好了（`docs/spec/42`）| 事件 2／3 的結果句改走上框 ＋ 君主肖像；先確認原版的結果句是不是同一則 TALK |
