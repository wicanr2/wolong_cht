...
#### ⭐ 接手順序（2026-08-17 更新，第三版）

**開局主畫面的逐區對拍做完了，而且過了**（[`docs/playtest/37`](docs/playtest/37-main-screen-parity.md)）：
五區有四區**逐像素相同**，兩局合計 29 萬個像素一個不差。
一路上釘死三件以前沒量過的事——松崗版的白色是 `#F3F3F3`
（[`docs/spec/51`](docs/spec/51-vga-dac-palette-scale.md)）、
開局鏡頭是首都 −(16,12) 而不是置中
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **橫幅日期的數字字模** | 主畫面五區只剩橫幅沒 PASS，而橫幅的差異只有日期數字：原版墨水高 **14 列**，remake 的倚天 ASCII 只有 **9 列**（顏色與位置已經對齊）| 找原版畫數字的那一支（`sub_106F5` 一帶）吃哪一套字模 |
| **2** | **四個視窗開著時的對拍** | 主畫面剩下的三個區（命令列、縮小地圖、自勢力情報）在開局全關，所以這一輪比到的是地圖。要比視窗內容就得先把它們點開 | ⚠ **進到大地圖之後 INT 33 的範圍變成整個世界**（`sub_120D6` 的 `0..0x17FF × 0..0x101F`），螢幕座標 ＝ 原始座標 − 鏡頭原點，所以送點擊的座標算法與選單裡完全不同。先量一次對映（游標在地圖模式被 `ax=2` 藏起來，要另外找探針）|
| **3** | **戰場的逐格對拍** | 同一個問題的戰術版，`docs/playtest/31` §1 記著「沒做過」。管線現在整條都有了，缺的是讓兩邊開同一張戰場 | `-open-siege -siege-node N` 對 remake，原版用存檔定位（`docs/spec/90` §2）|
| **4** | **五個開關圖示的來源段** | 主畫面右上角那五個小圖示 remake 是自繪的，原版素材的段與位移沒定位 | `docs/re/48` 的顯示清單裡找貼圖那幾筆的來源位移 |
| **5** | **橫幅日期的數字字模** | 原版的數字墨水高 14 列，remake 的倚天 ASCII 只有 9 列（`docs/spec/52` §4）。顏色與位置已經對齊，剩字形 | 先找原版畫數字的那一支（`sub_106F5` 一帶），再看它吃哪一套字模 |
...
兩個仍缺的橫向項目：五個開關圖示的來源、
主畫面在模擬器上收不到點擊（`docs/playtest/23` §4.1）。
