...
| **野戰（非攻城）** | ⭐⭐ **九區裡七區 0 差，`field` 0.05%＝原版游標（不可消）、小地圖 0.20%＝時刻**（`docs/playtest/43`）。開場對白照 `sub_1A2E8` 挑戰段實作後**台詞逐字、版面逐像素一致**（框內純黑、無名字列、行距 16、`\1` 名字色 9，`docs/re/74`）| 單挑本體（`byte_1D34B` 閘、回合互嗆迴圈、決著）未實作（`re/74` §5）；⭐ 改存檔搬軍團時**佔用圖快取欄 `+0x1A`/`+0x1C` 要跟著搬**（`43` §3）|
| **其餘視窗** | ⭐⭐ **武將一覽（編成清單）、財政、數值輸入器三格逐像素 PASS（0 px）**；編成視窗 0.20%＝原版錄影的游標（不可消）（`docs/playtest/42` §4.1）。修掉：欄位對齊（文字欄＋8、數字右靠分隔線右緣）、8×16 原版數字字模、捲軸逐像素重畫（雙層斜角鈕＋3D 凹槽、無滑塊）、編成過濾（`0x176A0` confirmed；軍師也排除）、預設編成騎騎步步弓弓（`sub_16D56`）、收入／支出＝月結快照（`sub_1548F`）| 進言五項選單受擷取輸入模型限制拍不到（`42` §5）；勢力／據點／軍團一覽與交戰目標清單未比；捲軸綠是色 13 還是 14 要換季分辨 |
