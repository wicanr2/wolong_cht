...
**主畫面與戰場都對到同一個局面了。** 戰場九區裡**五區逐像素 PASS**
（底列／標題／陣形列／指令面板／`▶▶` 列），小地圖 0.04%（8 px），
⭐ **`field` 從 87.8% 一路降到 0.84%**（176,640 px 裡 1,477 px，第 61 步）。
剩下的是將旗兩格各 1.5%
（[`docs/playtest/40`](docs/playtest/40-tactical-parity.md) §8、§9）。
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | ⛔ **擠在一起就打不到人** | 兵撐得夠久時戰術戰鬥卡死：48 個兵圍著 2 個打，跑 95 萬 tick 一個 byte 不動。成因是**移動的互換**——一支走進目標旁邊那一格，同一幀又被別人換出來，所以輪到它更新時距離永遠是 2（[`spec/61`](docs/spec/61-soldier-initial-hp-from-morale.md) §5.1）。開場體力剛好 100 時「受傷就退場」把它蓋住了 | 對 `sub_1AF69`（移動）與 `sub_1B240`（互換）的先後與旗標條件。解完 `spec/61` 才接得上，將旗的體力條也才會對 |
| **2** | **M3 的 T2 那 18 支** | T3 已歸零（`docs/re/68`），T2 是下一批：只在其他 `docs/` 被提過、沒有 `docs/re/` 筆記，合計 714 bytes | `tools/py.sh tools/re_coverage.py workplace/ida/dosv/census/census.tsv` 的 T2 表 |
| **3** | **`field` 剩下的 0.84%** | 1,477 px 散在各處，優先度低於上面兩項 | 逐塊看，先排除部隊的次像素位置差 |
