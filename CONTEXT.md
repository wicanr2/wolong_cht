...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **據點徽記畫上去** | 主畫面剩下的差異**全部是這一格**。位置與顏色分類已經解出來了（[`docs/re/67`](docs/re/67-city-emblem-on-strategy-map.md)）：徽記在**據點記錄座標右邊四格**，顏色照「自勢力／別的勢力／無所屬」分——無所屬那一格兩邊逐像素相同，所以橙色是圖塊本身、紅與藍是原版畫上去的 | 缺的是**圖形**與**畫它的那一支**。`sub_11CC9` 的兩個 overlay 只畫軍團與災害，所以它不在每幀路徑上；圖形應該在 `word_1D84C` 段（`MMAP.MCH` 物件圖塊，`internal/assets/world/mmapmch.go` 已能解），逐張比對原版那 16×16 |
