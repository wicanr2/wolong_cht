...
#### ⭐ 接手順序（2026-08-17 更新，第二版）

前一輪把主畫面的逐區對拍**真的對原版跑完了**（`docs/playtest/37`）：
地圖區從 99.2% 不同降到 0.5%，孫策那一組的 `faction` 區
**43,264 個像素一個不差**。順帶釘死兩件以前沒量過的事——
松崗版的白色是 `#F3F3F3`（`docs/spec/51`）、開局鏡頭是首都 −(16,12)
而不是置中（`docs/spec/52`）。
...
與 `tools/parity_locate.py`，看差在哪一種用 `tools/patch_zoom.py`。

| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **據點徽記畫上去** | 主畫面剩下的差異**全部是這一格**。位置與顏色分類已經解出來了（[`docs/re/67`](docs/re/67-city-emblem-on-strategy-map.md)）：徽記在**據點記錄座標右邊四格**，顏色照「自勢力／別的勢力／無所屬」分——無所屬那一格兩邊逐像素相同，所以橙色是圖塊本身、紅與藍是原版畫上去的 缺的是**圖形**與**畫它的那一支**。一座據點動到**五格**（中心 ＋ 斜角各差兩格的四面小旗）。**已排除**：`MMAP.MCH` 的 256 張圖塊逐張疊上去沒有一張吻合，64 個物件矩陣也沒有這種形狀（`tools/mch_match.py`、`tools/mch_patterns.py`）。換個方向從程式碼找：誰寫格子的 `[si+3]`–`[si+7]`（`loc_1D51F` 之外）|
