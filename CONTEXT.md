...
| **其餘視窗** | ⭐ **第一輪逐像素數字到手**（`docs/playtest/42`）：財政 **0.40% NEAR**（剩支出欄語意）、數值輸入器 **1.06%**（剩游標位置）——修掉外框、補零、數值顏色（索引 9）、數字字模（8×16 原版字模）四項。武將一覽 21.84%、編成 9.71%，五項實質差異已歸類（不含君主、欄位對齊、捲軸美術、預備兵扣減時點、數字字模）| 一覽表與編成的五項差異待修（`playtest/42` §4）；進言五項選單受擷取輸入模型限制拍不到（§5）；勢力／據點／軍團一覽與交戰目標清單未比 |
