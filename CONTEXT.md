...
| 目標 | 現況 | 卡在哪 |
|---|---|---|
| **主畫面** | **外框、版面常數與各視窗內部排版都照原版了**：外框取 `ICONGRF` 段 3（`docs/re/46` §3），四個常駐視窗的矩形出自機器碼（`docs/re/47`），十個顯示清單場景已接九個（`docs/spec/12`–`14`、`22`–`27`），連填色與立體邊的調色盤索引都照原版（`docs/re/48` §2.1）。自繪的只剩勢力色標 | **原版側進不到「四窗全開」的狀態**：主畫面在模擬器上收不到任何點擊（`docs/playtest/23` §4.1），DOS/V 的開新遊戲也停在「確定」。橫幅單獨對拍過，位移掃描落在 (0,0)。**沒有 same-state 對拍能力，所以「還差多少」只能用「常數出不出自機器碼」來量，不能用像素差** |
