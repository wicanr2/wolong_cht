...
| **3** | **Android：戰場與 SAF 匯入** | M9。⭐ **里程碑 A 通過**：模擬器與桌面在 frame 1／60／600 的指紋完全相同（`2b58e7b58f4796f9`／`5b3585cf005a6ec5`／`9651d8e27d6c9631`，2026-08-20）。主畫面、進言、一覽、軍團編成、存讀檔都可用（[`docs/mobile/android-ux.md`](docs/mobile/android-ux.md)）| 缺兩塊：**戰場**（[`android-ux.md`](docs/mobile/android-ux.md) §5 的版面）與 **SAF 匯入**（現在資料靠 `adb` 推進 app 內部目錄，那是驗收路徑不是玩家路徑）。實機驗收 ⛔ 沒有裝置 |
