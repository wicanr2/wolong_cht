...
**事件分派表已解**（`docs/re/07` §18）：`funcs_131E8` 13 支處理常式，索引 ＝ (代碼−1)×2；內政官要錢是 `sub_132A9`、外交官是 `sub_132E9`、火災與暴動**共用** `sub_134B1`（`0x010C`／`0x020C` 的低位元組都是 12，高位元組是參數不是代碼）。順帶定出**武將表在記憶體的基址 `0x4240`**。仍未解的是 `sub_139E8`（玩家答應之後做什麼）＝ 內政官的實際效果。

**月結已經整條解完。** 尾段八支只剩 `sub_12BD9` 沒讀
（對 22 個勢力各配一塊 0x30 緩衝區，疑似 AI 局勢評估或縮小地圖）。

**內政官的效果已解**（`docs/re/07` §19）：`sub_14194`，掛在
**主迴圈的每 tick 據點更新 `sub_13EFD`** 上——一次一個據點，
192 個輪一圈而一天是 216 tick，所以**每個據點大約每天一次**。

這就是「120 個月 1872 次暴動」的正解：補回上昇值的就是這一支。
AI 的據點走預設分支 `cl=8`，每天 9/16 機率 `+1`，月期望 `+16.9`
對上衰減期望 `−7.5`，**淨值是正的**。

⚠ 反直覺但要照抄：**玩家的據點基準是 5，比 AI 的 8 更差**，
要靠內政官的政治值補回來（政治 15 → `cl=20`）。
不派內政官的玩家據點比 AI 還糟——這讓「人事」指令有實質意義。

**已實作**（`internal/rules/governor` ＋ 6 個測試，接在 `World.tickCity`）。
實測 120 個月的暴動從 **1872 降到 681**（−64%），平均上昇值到 +100、
生產力長到上限。

實測 120 個月的暴動最終 **1872 → 0**。中間卡在 681 是因為我在
`tickCity` 加了一個**原版沒有的** owner 檢查跳過中立據點——
原版 `sub_13EFD` 的 `cmp …, 18h / jz` 只跳過 `sub_13F74`，
**`sub_14194` 是無條件呼叫的**。中立據點於是只有每月衰減沒有回補。

⭐ **抓到它的關鍵是把 AI 與中立分開統計。** 先前只印玩家的數字，
看到「平均上昇值 +100」就以為好了——**症狀出現在統計欄位看不到的那一群身上**。

副作用：`TestTaxTippingPoint` 的平衡點從 22.5% 移到 33.75%
（多了整備的 +11.25/月）。舊測試斷言「25% 會崩潰」，那是**缺了整備那一層
的產物**——這種測試會把模型的缺陷釘成規格，讓補上缺陷看起來像弄壞東西。

**⭐ 命令視窗全綠**：九項指令全部可用
（P 進言／J 人事／F 財政／A 編成／M 行軍／C 軍團／T 據點／G 武將／K 勢力）。
財政的「今月末 vs 來月」兩欄對照把**次月末才生效**這條規則顯示出來，
少了它玩家改完看不出為什麼數字沒動。

**「人事」已實作**（`cmd/wlgame/personnel.go`，按 `J`）：
內政官任命／解任、外交官任命／解任，兩段式選取，
內政官先選據點再選武將、外交官先選勢力再選武將（說明書的順序）。
命令視窗上的灰項少一個。

**下一項：dump `word_19874` 那塊 4,000 B 的連結表**（行軍路網），
以及命令視窗剩下的三格（財政／據點／勢力）。
