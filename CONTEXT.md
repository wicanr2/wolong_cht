...
| 停戰／協力的理由成立條件 | 選單內容已從 `TALK.DAT` #166／#230 定案，但**只有敵對提案的判定式讀出來了**（`sub_16475`）。另外兩個指令的條件還沒對過機器碼 |
...
4. **解出機制之後只寫 `docs/re/`，忘了 `docs/mechanics/`。**
   兩者回答的不是同一個問題：`docs/re/` 是「程式碼在哪」，
   `docs/mechanics/` 是「這個遊戲怎麼運作」。
   2026-08-08 的道路網與行軍就是這樣：解完、實作完、測試完、commit 完，
   機制文件一直沒建，**是 review 才發現的**（已補 `20-military.md`）。
   `CLAUDE.md` §6 是 `[HARD]`，卻是最容易在「解完了很興奮」時漏掉的一條。

---

### 7.1 里程碑現況

以 `CLAUDE.md` §9 的表為準，這裡只記「還沒完成的那幾項在缺什麼」。

| | 缺什麼 |
|---|---|
| M1 資料格式 | `ICONGRF` 段 1；`MMAP.MCH`／`BATTLE.MCH` 的語意 |
| M3 反組譯 | 上表的小項 |
| M4 機制文件 | 停戰／協力的理由判定式 |
