...
| **戰術畫面** | ⭐ **九區裡六區逐像素相同**（`docs/playtest/40`）：底列、標題、我方將旗、陣形列、指令面板、`▶▶` 列。`field` 0.17%（176,640 px 裡 307 px）、小地圖 8 px、對方將旗 44 px。側欄七格照原版重畫（`docs/re/60` ＋ `docs/spec/31`）| `field` 那 307 px 裡 **299 px 原理上消不掉**（旗的揮舞相位、原版錄影裡的滑鼠游標、兩邊的門破在不同 tick），真正未歸類的只剩 8 px。**野戰（非攻城）沒對過**——地形是從大地圖即時長出來的，同狀態更難湊 |
