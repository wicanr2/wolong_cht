...
4. **M3 的覆蓋缺口** —— 全函式普查（`docs/re/21`）量出 739 支裡有大量從未被
   ...
   **那些是會被執行的遊戲程式碼**，不是用不到的程式庫。
   ...
   已解掉的三塊：戰略指令列與熱區分派（`docs/re/22`）、一覽表引擎（`docs/re/26`）、
   一覽表的逐列繪製與欄位對照（`docs/re/27`，T4 前三大的函式就在這裡）。
...
| 項目 | 現況 |
|---|---|
| **軍團 `+0x20` 的語意與存檔分歧** | 三個寫入端兜不成單一名稱（編成寫首都、行軍寫目的地、遷都改寫）。remake 的 `Corps.Home` **行軍時不更新**，所以下完行軍指令再存檔，`+0x20` 與原版不同。**先收斂語意再改**，否則只是換一種錯法。證據見 `docs/re/27` §7.2 |
