...
| 未讀函式從 355 降到 287 是「多讀懂了 68 支」 | **其中大部分只是被寫進 `docs/re/24` 的登記表** | 覆蓋率靠「符號有沒有出現在 `docs/`」算，目錄型文件會把自己算進去。排除目錄後重算是 268 支。**「已登記」不是「已讀懂」，量測工具要能區分這兩者，否則做一份清單就能讓缺口歸零** |
...
4. **M3 的覆蓋缺口** —— 全函式普查（`docs/re/21`）量出 739 支裡有大量從未被
   ...
   **那些是會被執行的遊戲程式碼**，不是用不到的程式庫。
   ...
   268 支未讀函式已全部登記在 `docs/re/24`，附上 TALK 訊息層級的角色證據。
...
| M3 反組譯 | 739 支函式中 284 支未觸及（佔程式碼 28%），缺口集中在戰略 UI、戰術戰鬥與月結／經濟／AI（`docs/re/21`，數字要用排除目錄後的）；另有上表的小項 |
| M4 機制文件 | 跟著 M3 的缺口走 |
