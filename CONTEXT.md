...
**剩下的**：M6 的內容 parity（見 §2.1）、M7 的畫面抽樣與排版 parity
（文意層的八變體組已全部掃過）、M8 實機驗收。
**接手順序見 §7.0 開頭那張表**。
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **視窗內部的龍紋底紋** | 量到是**一條寬 32 的直條、水平完全重複（99.1%）、垂直不重複**，只有兩色（`docs/formats/03` §5.5）。69 個原版檔逐 byte 掃過都沒中 | 下一步是**動態追蹤**：用 `tools/dosboxx_bridge.sh` 對那塊 VRAM 設寫入斷點，看是誰畫的。靜態的找法已經走完 |
| **2** | **M7 畫面抽樣與排版 parity** | 文意層的變體組已全部掃過（`docs/reference/02` §14.1），剩下的是「把校訂後的文字放上畫面看有沒有溢出」 | `tools/talkdat_selftest.py` 已有 22 格行寬 guard；缺的是抽樣截圖 |
| **3** | **M8 各平台實機驗收** | 交叉建置的候選包還沒在目標 OS 跑過 | 見下面第 2 項 |
| **4** | **非玩家的 Stage 0–3 決策鏈** | 玩家那一半與 Stage 8–11 已解（`docs/re/64`），AI 那四支還沒逐條讀：它們讀據點 `+0x00` 的位元 6／7、據點 `+0x18`、勢力 `+0x18`，還有一次亂數決定計時器 | `sub_1439D`／`sub_143AF`／`sub_1440F`／`sub_14466`；連帶的 `+0x21` 語意見 `docs/re/64` §6 |

> 順序是使用者裁定 2026-08-16：先把行軍指示的三選一串起來（**已完成**），
> 再做戰場圖塊組顏色（**已驗，是假問題**）與視窗底紋。M7／M8 往後排。
...
| M7 校訂 | 文意層 1,022 則；60 筆校訂已可套用，#0–#1021 第一輪已讀，**77 個八變體組已以組為單位掃過（沒有要新增的）**，畫面抽樣與排版 parity 未完成 |
| M8 發行 | 實機驗收 |
