...
| **戰術畫面** | PARTIAL（`docs/playtest/20`）：幾何、命令面板、底列 glyph、初始相機、32×30 display grid 已接 | 同狀態戰場、EGA mask 逐位元等價、動畫時序未做；`BATTLE.MDL` 的 61,440 B 像素格式**只有統計證據，還沒把圖畫出來**（`docs/formats/07` §7）|
| **音樂／音效** | ✅ **會出聲了**：純 Go 的 OPL3（`internal/audio`）把 `*BGM.DAT` 與 `SOUND.DAT` 渲染成 ogg（`tools/bgm2ogg.sh`），`internal/ui/sound` 播，系統選單第 3 列開關。與原版錄音的包絡互相關 0.704（`docs/playtest/26`）| ⭐ 場景對應也解了（`docs/re/58`）：四季配樂、事件曲、攻城／野戰。剩曲 1／8／10 在已知路徑不可達，曲 6（事件）remake 還沒接。音色的諧波結構沒量化比對過 |
