...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **Windows／macOS 原生 GUI 實機驗收** | M8 唯一的閘。⭐ **三平台已重新對齊成一致的 `20260821` 批次**（[`release/03`](docs/release/03-three-platform-20260821.md)，含 Android APK），Linux 有 GUI smoke，另兩個平台只驗了檔頭 | 需要實機或 VM，不是 Docker 能代的 |
