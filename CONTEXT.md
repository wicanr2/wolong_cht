...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **視窗內部的龍紋底紋** | 量到是 **32×32 平鋪、只有兩色**（`docs/formats/03` §5.5），但 69 個原版檔逐 byte 掃過都沒中 | 下一步是 `sub_11AC3`（LOAD DATA 面板那條路徑）與 `sub_1FAC2`／`word_1987C` |
...
| M6 呈現層 | 視窗內部的龍紋（已量到是 32×32 平鋪，來源未定位，`docs/formats/03` §5.5）|
