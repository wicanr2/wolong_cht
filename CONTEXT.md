...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **⚠ 登城機制的最後一塊：兩張地面高度圖** | 量完也讀完大半了（`docs/playtest/28` §5.1）：**remake 的攻方打不進城**。`sub_1B1B1` 已解——擋路的是**單位**不是地形高度、低平面水平跨格**沒有高度差上限**、高平面要高度完全相等，而 `PlaneHigh` 只由純 Z 那一支寫（騎馬被擋就在那裡）。**缺的只剩 `es:[di+0x7000]`／`+0x8000` 那兩張地面高度圖是誰填的** | 從 `word_1D2FA` 段的寫入端追；`sub_1BB6D`（展開圖塊堆疊）是第一嫌疑。解出來才動 `tryMove`——現在照低平面那條規則改會翻掉兩條既有測試 |
