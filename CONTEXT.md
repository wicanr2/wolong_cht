...
| **原生存檔格式** | `docs/spec/20` ＋ `internal/savefile`。JSON、不壓縮、沒有原版檔案也載得動（使用者裁定）。**檔案自帶原始區塊那 22,208 B**，所以匯出回原版格式是無損的——round-trip byte-for-byte 有測試守著。順帶修掉一個缺陷：`routes`／`cityCursor`／事件游標先前「不序列化」，存檔再讀回來就掉了，而 `cityCursor` 決定 AI 下一個處理哪個據點 |
