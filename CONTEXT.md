...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **視窗底紋的 96 列排法** | 點陣**找到了**：`ICONGRF.DAT` 最後 128 byte（段 3 `+0x2320`），32 寬 1bpp，逐列對螢幕 94–99%（`docs/formats/03` §5.5）。但螢幕上的垂直週期是 96，而 24／32 列平鋪與各種磚砌都只有 63–83% 找**取用端**——`KI.EXE` 裡已經排除三條路（沒有立即值、`sub_1F888` 的三個呼叫點都不在段尾、`word_10D50` 的五個用法沒有 `si=1980h`）。剩下掃另外七個執行檔，或對那 128 byte 做動態讀取追蹤 |
