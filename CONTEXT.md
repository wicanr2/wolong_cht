...
| **3** | **行軍指示的三選一（戰鬥指揮／委任／解體）** | 機制全解（`docs/re/45`），但呈現層沒有這個選單，所以玩家**還不能委任、也不能解體軍團**。一覽表的「委任」格已經會畫，只是目前只有 AI 編的軍團會亮 | `sub_17FDB` 的流程在 `docs/re/27` §7.1；⚠ 解體的**消費端仍未找到**（`+0x23 = 0x0B` 沒有任何讀取端比對它） |
| **4** | **戰場圖塊組的顏色** | 原版城壁是黃褐磚牆、remake 畫成灰色柵欄。假說是 `BATTLE.MDL` 三組選錯（`docs/playtest/27` §7.4），**沒驗** | `BATTLE.MAP` 索引第一欄就是圖塊組編號（`docs/formats/07` §2）；先確認 remake 用的是不是同一組。⚠ **不是調色盤組**：進戰場不重載色盤（`sub_19946` 只做淡入），而且色盤只有兩組（１６色／液晶，`docs/re/02` §6.2）|
| **5** | **視窗內部的龍紋底紋** | 量到是 **32×32 平鋪、只有兩色**（`docs/formats/03` §5.5），但 69 個原版檔逐 byte 掃過都沒中 | 下一步是 `sub_11AC3`（LOAD DATA 面板那條路徑）與 `sub_1FAC2`／`word_1987C` |
| **6** | **軍團 `+0x23` 狀態機** | 寫入端寫過八種值（0／1／2／3／8／9／`0x0A`／`0x0B`），讀取端十支，語意未解；`sub_13E11` 每「時」拿 `+0x23 × 8 + 0x18` 去和 `+0x21` 比 | 先解 `+0x21`（`docs/re/45` §5）|
