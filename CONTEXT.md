...
**主畫面與戰場都對到同一個局面了。** 戰場側欄四區逐像素 PASS
（標題／陣形列／指令面板／`▶▶` 列），小地圖 0.04%（8 px），
⭐ **`field` 從 87.8% 一路降到 0.84%**（176,640 px 裡 1,477 px，第 61 步）。
剩下最大的一塊是 `bottom` 的 12.70%，而且**與步數無關**
...
| # | 工作 | 為什麼現在做 | 下手點 |
|---:|---|---|---|
| **1** | **`bottom` 那 12.70%** | 這是現在最大的一塊，而且**與步數無關**（0/40/61/75/90 都是 1,950 px）——所以不是兩邊的時刻對不準，是那一區畫的東西不一樣 | 先把 `bottom` 切成「待機兵條」與「底」兩塊分開量，再回頭找原版畫它的那一支 |
