...
#
# 為什麼這一支跑 PC-98：兩版是同一份原始碼的兩次編譯，而**PC-98 那一版的
# 腳本比較齊**，所以玩法規則、AI、資料結構、地圖與戰鬥畫面都驗得起來。
#
# ⚠ **不是因為松崗版過不去。** 松崗 DOS/V 的密碼頁四格留白按「確定」就進
# 開場（docs/playtest/18，CLAUDE.md §4.0）——要驗 DOS/V 的東西就用
# tools/dosv_live_capture.sh 去跑，不要因為看到密碼畫面就改用這一支。
#
