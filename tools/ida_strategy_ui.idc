...
static main()
{
    auto out;
    Wait();
    out = fopen("/work/stratui.txt", "w");
    if (out == 0) { return; }
    fprintf(out, "勢力選擇詳情 ＋ 戰略 UI 缺口\n位址空間：IDA DOS/V linear address\n");
    // 正對照：已知必中的一條，掃不到就代表腳本本身壞了
    fprintf(out, "正對照：0001701A = 「%s」\n", GetDisasm(0x1701A));

    dump_func(out, 0x119CA, "sub_119CA 誰進入勢力選擇");
    dump_func(out, 0x15C14, "sub_15C14 畫一列勢力");
    dump_func(out, 0x15CC6, "sub_15CC6");
    dump_func(out, 0x15DBB, "sub_15DBB 右側詳情");
    dump_func(out, 0x17E4A, "sub_17E4A 戰略 UI 最大未讀");
    dump_func(out, 0x1807B, "sub_1807B");
    dump_func(out, 0x17F90, "sub_17F90 上者的呼叫者");
    fclose(out);
}
