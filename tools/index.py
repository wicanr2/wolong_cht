...
BLOCKED_BY_COPY_PROTECTION = re.compile(
    ...
    r"|(防拷|密碼保護|複製保護|拷貝保護|密碼頁)(下|之下)的"
    # ⭐ 第四種寫法：**擺成括號裡的理由**，而且換一個同義詞。
    # 「不宣稱逐像素驗收完成（原版執行檔仍受複製保護）」——
    # 沒有「擋」這個動詞、沒有「防拷」這個詞，前三條全部比不到。
    # 「受／因／由於」把它標成因果，才是這一條要抓的東西。
    r"|(受|因|由於)[^。\n]{0,10}(防拷|複製保護|拷貝保護|密碼保護)")
...
def check(docs):
    ...
    solved_cell = re.compile(
        r"^\s*(?:\*\*)?(?:✅|已解|已讀|已驗|已證實|已確認|已定案|已實作|已接入)")
    for d in docs:
        inopen = False
