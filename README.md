...
| 規則還原（M4／M5）| **75%** | 政略／行軍／戰術／外交／說服／單挑都以機器碼出處實作，規則層 5 年長跑不變量成立，67 份規格 66 CONFORMED | `mechanics` 的 24 列未解（行為級的約 9 列，見 [`re/43`](docs/re/43-open-questions.md) §2.1）；AI 決策只驗過「跑得起來」，沒有與原版的長程對照 |
...
| **整體** | **80%** | 量過的地方幾乎都對到 0 px、規則層可玩且穩定、保存目標（格式＋文字）接近完備 | 拉低整體的是平台實機驗收（外部條件：缺機器）與 mechanics 的行為級未解 |
