...
| 全專案的未解 | **479 列**（另有 4 列是 DOS／BIOS 平台層，不計入）。⚠ **這個數字比較接近「文件有多少份」**——479 列分布在 193 份文件、平均每份 2.5 列，而每寫一份新文件就帶進約三列自己的未解 | [`re/43`](docs/re/43-open-questions.md) §0 |

#### 那 479 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 479 列，而開局主畫面 **256,000 個像素與原版一個不差**。
