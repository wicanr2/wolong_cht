...
| 反組譯 | 739/739 支有筆記；`docs/re/` 自己標成未解的有 **175 列**（2026-08-21 逐列核完，原本 220）| [`re/21`](docs/re/21-function-census.md)、[`re/43`](docs/re/43-open-questions.md) |
| 全專案的未解 | **492 列**（含 spec 135、re 175、playtest 62、mechanics 42、formats 34…）。⚠ **那是列數不是獨立問題數**——同一個缺口寫在四份文件裡就佔四列 | [`re/43`](docs/re/43-open-questions.md) §1 |

#### 那 492 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 492 列，而開局主畫面 **256,000 個像素與原版一個不差**。
...
|---|---:|---|
| 程式碼理解（`docs/re/`）| 175 | **不擋任何東西能不能玩。** 這是「某支函式／某個變數還沒讀透」的存量——它的價值是**未來的修正從這裡出來**，不是現在有東西壞掉。2026-08-21 逐列核過一次，清掉 45 列已解的登記 |
| 其他（`spec` 135 ＋ `release` 13 ＋ `mobile` 12 ＋ `promo` 2）| 162 | `spec` 那 135 列多半是**已實作項目自己的剩餘細節**。`release`／`mobile` 的 25 列裡**真正擋發行的只有四件事**（見下），其餘是設計取捨與流程小事 |
| 驗收（`docs/playtest/`）| 62 | **不是「做錯了」，是「還沒量」。** 這一類最誠實也最容易被高估——它標的是「我們相信它對，但沒有數字」 |
| 規則正確性（`docs/mechanics/`）| 42 | **唯一會讓遊戲行為與原版不同的一類。** 目前跑得起來且不變量成立（規則層 5 年 60 個月長跑，[`playtest/33`](docs/playtest/33-ai-march-long-run.md)），但這裡每解一條就可能改動平衡 |
| 資料保存（`docs/formats/`）| 34 | 影響的是**文化資產保存的完整度**，不是玩得動玩不動。例如 `ICONGRF` 段 1 `0x0000` 那一塊畫的是什麼 |
| 外部資料（`docs/reference/`）| 17 | 松崗中文版說明書等尚未取得的素材。**拿不到就是拿不到**，不是工作量 |
...
| 擋什麼 | 佔幾列 | 為什麼卡住 |
|---|---:|---|
| Windows／macOS 原生 GUI 實機驗收 | **4** | ⛔ 沒有那兩個平台的機器。交叉建置的產物只驗了檔頭（`release/01`／`02`／`03`／`README-RELEASE` 各寫一次）|
...
**四件事 ＝ 12 列。** 剩下的 13 列是設計取捨（手機小卡放哪些欄位、
戰場要不要能縮放）與流程小事（`verification/` 截圖不進管線），不擋發行。

⚠ **這個數字變小不自動等於進度。** 2026-08-21 的稽核把它從 570 降到 492，
而那 −78 的組成是：**刪掉過期登記**、**修掉抽取工具的精確度問題**，
以及**修好工具之後多浮現的 17 列真缺口**（原本被整列比對「已解」吃掉）。
**沒有任何一列是靠解出新東西減掉的**（逐輪紀錄在 [`WORKLIST.md`](WORKLIST.md)）。

⭐ **同一次修正同時讓數字變小與變大**：抽取器原本用整列比對「已解」跳過列，
於是「`ENDPAL` 那邊已解，開場這邊還沒做」這種**真缺口**被整列吃掉，
`formats/02` 與 `mechanics/50` 因此看起來像沒有缺口。
**「查詢回空」不等於「沒有」**——過濾器自己有洞也長這樣。

⚠⚠ **`docs/re/` 那 175 列已經逐列核過**（2026-08-21），清掉的 45 列多半是
「解掉了，但答案寫在別份文件」：`sub_14C4C` 的地形對映在
[`mechanics/30`](docs/mechanics/30-combat.md) §2、`sub_1748F` 在
[`re/27`](docs/re/27-list-row-fields.md)、`+0x23` 的狀態機在
[`re/64`](docs/re/64-corps-arrival-state-machine.md)。
其餘五個目錄**還沒逐列核**，同樣的形狀應該還在裡面。
⭐ **成因是結構性的**：`docs/re/` 的未解表沒有負責人，
而解掉它的人通常把答案寫在別份文件（規格、機制，甚至只寫在程式註解裡），
沒有任何機制會回頭清這裡。`tools/index.py` 的檢查 ⑩ 只擋得住
「那一列自己承認解了」那一種，而**狀態行寫 READY 的文件可以帶著
任意過期的未解小節**——檢查 ② 開頭就 `continue` 掉了。
