...
| 全專案的未解 | **431 列**（re 175、spec 118、playtest 49、formats 33、mechanics 24、mobile 12、release 11、reference 7、promo 2）。**九個目錄在 2026-08-21 全部逐列核過。** ⚠ **那是列數不是獨立問題數**——同一個缺口寫在四份文件裡就佔四列 | [`re/43`](docs/re/43-open-questions.md) §1 |

#### 那 431 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 431 列，而開局主畫面 **256,000 個像素與原版一個不差**。
...
| 其他（`spec` 118 ＋ `mobile` 12 ＋ `release` 11 ＋ `promo` 2）| 143 | `spec` 那 118 列多半是**已實作項目自己的剩餘細節**。`release`／`mobile` 的 23 列裡**真正擋發行的只有四件事**（見下），其餘是設計取捨與流程小事 |
...
| 外部資料（`docs/reference/`）| 7 | 松崗中文版說明書等尚未取得的素材。**拿不到就是拿不到**，不是工作量 |
...
| 16 KB page size 的實測 | 1 | `.so` 的 LOAD 段已是 `0x4000`，但沒有那種裝置或 AVD 實際載過 |
