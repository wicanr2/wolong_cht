...
| 規則規格 | **173 份**（不含索引與 `TEMPLATE.md`）：**168 CONFORMED**／4 READY／1 DRAFT | [`spec/00`](docs/spec/00-index.md) |
| 反組譯 | 739/739 支有筆記；`docs/re/` 自己標成未解的有 **786 列**（每寫一份新的反組譯筆記就帶進自己的未解表）| [`re/21`](docs/re/21-function-census.md)、[`re/43`](docs/re/43-open-questions.md) |
| 全專案的未解 | **786 列**（另有 6 列是 DOS／BIOS 平台層與編譯器 runtime，不計入）。⚠ **這個數字比較接近「文件有多少份」**——786 列分布在 335 份文件、平均每份 2.4 列，而每寫一份新文件就帶進約三列自己的未解 | [`re/43`](docs/re/43-open-questions.md) §0 |
