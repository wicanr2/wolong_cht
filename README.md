...
**核心規則與可玩垂直切片已接通；完整交付已集中在 `dist-all/`（三平台桌面包、Linux AppImage、Android APK、推廣片），目前是一致的 `wolong-remake-20260822` 批次。Windows／macOS 原生 GUI 與 Android 的實機驗收仍待完成。**
...
| 規則規格 | **64 份**（不含索引與 `TEMPLATE.md`）：**63 CONFORMED**／1 READY（`34-speed-steps`）／0 DRAFT | [`spec/00`](docs/spec/00-index.md) |
| 反組譯 | 739/739 支有筆記；`docs/re/` 自己標成未解的有 **181 列**（2026-08-21 逐列核完時是 175；之後 [`re/71`](docs/re/71-strategy-hotspot-dispatch.md)、[`re/72`](docs/re/72-world-map-display-list.md) 與 [`re/53`](docs/re/53-lord-select-window.md) 各自登記了新缺口）| [`re/21`](docs/re/21-function-census.md)、[`re/43`](docs/re/43-open-questions.md) |
| 全專案的未解 | **453 列**（re 180、spec 129、playtest 51、formats 33、mechanics 24、release 13、mobile 12、reference 6、promo 5）。**九個目錄在 2026-08-21 全部逐列核過**，之後每一輪新寫的文件自己帶進新缺口。⚠ **那是列數不是獨立問題數**——同一個缺口寫在四份文件裡就佔四列 | [`re/43`](docs/re/43-open-questions.md) §1 |

#### 那 453 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 453 列，而開局主畫面 **256,000 個像素與原版一個不差**。
...
|---|---:|---|
| 程式碼理解（`docs/re/`）| 180 | **不擋任何東西能不能玩。** 這是「某支函式／某個變數還沒讀透」的存量——它的價值是**未來的修正從這裡出來**，不是現在有東西壞掉。2026-08-21 逐列核過一次，清掉 45 列已解的登記 |
| 其他（`spec` 129 ＋ `release` 13 ＋ `mobile` 12 ＋ `promo` 5）| 159 | `spec` 那 118 列多半是**已實作項目自己的剩餘細節**。`release`／`mobile` 的 23 列裡**真正擋發行的只有四件事**（見下），其餘是設計取捨與流程小事 |
| 驗收（`docs/playtest/`）| 51 | **不是「做錯了」，是「還沒量」。** 這一類最誠實也最容易被高估——它標的是「我們相信它對，但沒有數字」 |
...
| **沒有音效裝置時遊戲會掛** | 2 | 完整版會自己找到音檔並開音訊，而 Ebiten 沒有可查詢的音訊 API——一般啟動遇到沒有音效卡的機器就結束（[`docs/spec/75`](docs/spec/75-bundled-audio.md) §5）|

**四件事 ＝ 13 列。** 剩下的 12 列是設計取捨（手機小卡放哪些欄位、
...
⚠ **這個數字變小不自動等於進度。** 2026-08-21 的稽核把它從 570 降到 431，
而那 −139 **沒有一列是靠解出新東西減掉的**。組成是三件事：
**刪掉已經解了卻還掛著的登記**、**修掉抽取工具的精確度問題**、
以及**把「刻意的 remake 差異」與「已定案的決定」從缺口表分出去**
（逐輪紀錄在 [`WORKLIST.md`](WORKLIST.md)）。

⚠ **變大也一樣不等於退步。** 從 431 一路到 457，每一列都是**新寫的文件
誠實登記自己的缺口**：`re/71`（熱區分派）、`re/72`（顯示表的第五個槽由誰填）、
`re/53`（自訂軍師名存不存得進 SAVE.DAT）、`spec/72`–`76`（完整版、右鍵、
軍團疊圖、音訊、主君編成各自的未解）。
**解出新東西通常會讓這個數字上升**——看得更清楚的直接後果，
就是看見更多還不清楚的地方。
...
- 完整交付根目錄：[`dist-all`](dist-all)，包含**四平台完整包**（Linux／Windows／macOS／Android）、Linux AppImage、五支推廣片、雜湊與 GUI smoke 截圖。目前是一致的 `wolong-remake-20260823` 批次（[`docs/release/05`](docs/release/05-full-20260823.md)）。
...
- Linux AppImage：[`wolong-remake-linux-amd64-20260823.AppImage`](dist-all/packages/wolong-remake-linux-amd64-20260823.AppImage)。已通過 Linux／Xvfb 固定種子 smoke（含結局過場）。**公開散布的版本仍要由玩家提供合法 DOS/V 資料與中文字型。**
