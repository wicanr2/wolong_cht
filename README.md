...
| ⭐ **攻城戰，兩邊開同一場**（9 區）| **七區 0 px**、`field` **0.10%**（＝原版畫的滑鼠游標與旗的揮舞相位）、小地圖 2.50%。2026-09-05 第一次做得到「同一場」——dosgolem 直接叫原版的 `sub_14ADE` 開一場指定的仗 | [`playtest/72`](docs/playtest/72-same-battle-parity.md) |
...
| 全專案的未解 | **581 列**（另有 6 列是 DOS／BIOS 平台層與編譯器 runtime，不計入）。⚠ **這個數字比較接近「文件有多少份」**——581 列分布在 239 份文件、平均每份 2.4 列，而每寫一份新文件就帶進約三列自己的未解 | [`re/43`](docs/re/43-open-questions.md) §0 |

#### 那 581 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 581 列，而開局主畫面 **256,000 個像素與原版一個不差**。
...
| 畫面一致（M6）| **93%** | 十個局面逐像素：主畫面／系統選單／三視窗／五張清單／財政全 0 px，野戰 0.05%；攻城側欄四區 0 px、戰場區 0.86%（＝兩邊開的不是同一場）。⭐ **2026-09-05 兩邊第一次開同一場仗**：九區裡**七區 0 px**、`field` **0.10%**（＝原版畫的游標與旗的揮舞相位）——先前戰場那一組 0.86% 的成因「兩邊開的不是同一場」拿掉了（[`playtest/72`](docs/playtest/72-same-battle-parity.md)）。原版側的取樣也不再需要 DOSBox，而且取樣點可以寫成**遊戲日期或事件** | 分數只涵蓋量過的局面。⚠ 小地圖那 2.50% 是**新挖出來的差異**（部隊點原版兩排、remake 一排），`field` 看不到它。另外「量過」的門檻變高了：`sb-minimap` 會隨時間變、「剛進戰場」其實是兩個取樣點（[`playtest/70`](docs/playtest/70-dosgolem-tactical-commands.md) §3）|
...
| **整體** | **81%** | 量過的地方幾乎都對到 0 px、規則層可玩且穩定、保存目標（格式＋文字）接近完備。⭐ **原版現在是一個可以問問題的物件**：記憶體讀得到、控制流攔得到、狀態存得起來分岔——這是這一輪最大的結構性改變 | 拉低整體的是平台實機驗收（外部條件：缺機器），以及**行為層對拍才剛開始**——第一次做就抓到一個規則分歧 |
