...
| ⭐ **全軍站定那一刻**（同一場攻城）| 兩邊都在**第 70 拍**站定、**第 75/76 拍**重新開動；那一刻**96 個兵逐槽全等**，八區 **全部 0 px**、`field` **139 px（0.08%）**——而那 139 px 全部是已歸類的不可消項：原版自己畫的滑鼠游標 95、旗的揮舞相位 44 | [`playtest/74`](docs/playtest/74-settled-tick-parity.md)、[`playtest/75`](docs/playtest/75-pathfind-detour.md)、[`playtest/76`](docs/playtest/76-battle-talk-parity.md) |
...
| 規則規格 | **127 份**（不含索引與 `TEMPLATE.md`）：**126 CONFORMED**／1 READY（`83-initial-strategy-pass`）／0 DRAFT | [`spec/00`](docs/spec/00-index.md) |
...
| 全專案的未解 | **625 列**（另有 6 列是 DOS／BIOS 平台層與編譯器 runtime，不計入）。⚠ **這個數字比較接近「文件有多少份」**——625 列分布在 257 份文件、平均每份 2.4 列，而每寫一份新文件就帶進約三列自己的未解 | [`re/43`](docs/re/43-open-questions.md) §0 |

#### 那 625 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 625 列，而開局主畫面 **256,000 個像素與原版一個不差**。
...
| **整體** | **83%** | ⚠ **這一輪不動**：M6 加了 1 分，但同一輪也量出更多**還沒量過**的表面（`sub_18853` 還有 45 個呼叫點、武將／勢力兩格的反白沒有原版擷取）。量過的地方幾乎都對到 0 px、規則層可玩且穩定、保存目標（格式＋文字）接近完備。⭐ **原版現在是一個可以問問題的物件**：記憶體讀得到、控制流攔得到、狀態存得起來分岔——這是這一輪最大的結構性改變 | 拉低整體的是平台實機驗收（外部條件：缺機器），以及**行為層對拍才剛開始**——三輪各抓到一件事（和平掉頭、尋路排序、退卻沒有倒數），而第三輪同時量出一條還沒對的（退卻中補不補兵）|
...
- 完整交付根目錄：[`dist-all`](dist-all)，包含**四平台完整包**（Linux／Windows／macOS／Android）、Linux AppImage、**一支合成推廣片**、雜湊與 GUI smoke 截圖。目前是一致的 `wolong-remake-20260906` 批次（[`docs/release/13`](docs/release/13-full-20260906.md)）。⚠ **「全平台重建」是兩支腳本**：`tools/release_all.sh` 不會重建 APK（Android 是另一條管線，檔名取的是 APK 自己的 mtime），要一起換得再跑 `WOLONG_BUNDLE_DATA=1 tools/android_build.sh`。
...
- Linux AppImage：[`wolong-remake-linux-amd64-20260906.AppImage`](dist-all/packages/wolong-remake-linux-amd64-20260906.AppImage)。已通過 Linux／Xvfb 固定種子 smoke（含結局過場，且**不帶任何資料旗標**就跑得起來），並與原版逐關卡對拍過（[`docs/release/12`](docs/release/12-full-20260902.md) §4，那一批的 AppImage）。**公開散布的版本仍要由玩家提供合法 DOS/V 資料與中文字型。**
...
### 中文用倚天 16×15 點陣字，而且**繁中不必自備**

不用 TTF 縮到 16px（會糊、筆劃比例也不對），走 1990 年代 DOS 中文遊戲
實際用的倚天點陣字。

⭐ **繁中的字型直接從原版資料取**（[`docs/spec/137`](docs/spec/137-builtin-symbol-font.md)）：
`END_S13.DAT` 一份就是「408 格全形符號 ＋ `stdfont.15` 全文」，
`END_S14.DAT` 與 `ascfont.15` byte-for-byte 相同。玩家手上既然已經有原版，
就不必再另外找字庫。⚠ **那 408 格符號與倚天 `SPCFONT.15` 不一樣**——
全形逗號差 (+3, −2) 個像素，逐像素對拍看得出來，所以要用原版那一份。
簡體與日文的字集不在原版資料裡，那兩個語系仍要 `-font` 指到自備的字庫。

**字型檔不隨本專案散布**，一個 byte 都不打包。
