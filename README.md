...
| 規則規格 | **59 份**（不含索引與 `TEMPLATE.md`）：**58 CONFORMED**／1 READY／0 DRAFT | [`spec/00`](docs/spec/00-index.md) |
| 反組譯 | 739/739 支有筆記；`docs/re/` 自己標成未解的有 **177 列**（2026-08-21 逐列核完時是 175，原本 220；之後新寫的 [`re/71`](docs/re/71-strategy-hotspot-dispatch.md) 自己登記了 2 列）| [`re/21`](docs/re/21-function-census.md)、[`re/43`](docs/re/43-open-questions.md) |
| 全專案的未解 | **432 列**（re 177、spec 116、playtest 51、formats 33、mechanics 24、mobile 12、release 11、reference 6、promo 2）。**九個目錄在 2026-08-21 全部逐列核過。** ⚠ **那是列數不是獨立問題數**——同一個缺口寫在四份文件裡就佔四列 | [`re/43`](docs/re/43-open-questions.md) §1 |

#### 那 432 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 432 列，而開局主畫面 **256,000 個像素與原版一個不差**。
...
|---|---:|---|
| 程式碼理解（`docs/re/`）| 177 | **不擋任何東西能不能玩。** 這是「某支函式／某個變數還沒讀透」的存量——它的價值是**未來的修正從這裡出來**，不是現在有東西壞掉。2026-08-21 逐列核過一次，清掉 45 列已解的登記 |
| 其他（`spec` 116 ＋ `mobile` 12 ＋ `release` 11 ＋ `promo` 2）| 141 | `spec` 那 118 列多半是**已實作項目自己的剩餘細節**。`release`／`mobile` 的 23 列裡**真正擋發行的只有四件事**（見下），其餘是設計取捨與流程小事 |
...
| 擋什麼 | 佔幾列 | 為什麼卡住 |
|---|---:|---|
| Windows／macOS 原生 GUI 實機驗收 | **4** | ⛔ 沒有那兩個平台的機器。交叉建置的產物只驗了檔頭 |
| Android 實機驗收 | 3 | ⛔ 沒有裝置。模擬器驗不到觸控手感、真實 GPU、高 DPI 上的點陣字 |
| release signing（keystore 怎麼保管）| 3 | 還沒決定；目前出的是 debug 簽章 |
| 16 KB page size 的實測 | 2 | `.so` 的 LOAD 段已是 `0x4000`，但沒有那種裝置或 AVD 實際載過 |

**四件事 ＝ 12 列。** 剩下的 11 列是設計取捨（手機小卡放哪些欄位、
...
⚠ **變大也一樣不等於退步。** 之後的 +1 是新寫的
[`re/71`](docs/re/71-strategy-hotspot-dispatch.md) 誠實登記了自己的 2 列，
同一輪又解掉 1 列。**解出新東西通常會讓這個數字上升**——
看得更清楚的直接後果，就是看見更多還不清楚的地方。
