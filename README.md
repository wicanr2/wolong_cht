...
| 規則規格 | **147 份**（不含索引與 `TEMPLATE.md`）：**142 CONFORMED**／4 READY／1 DRAFT | [`spec/00`](docs/spec/00-index.md) |
...
| 全專案的未解 | **746 列**（另有 6 列是 DOS／BIOS 平台層與編譯器 runtime，不計入）。⚠ **這個數字比較接近「文件有多少份」**——746 列分布在 312 份文件、平均每份 2.4 列，而每寫一份新文件就帶進約三列自己的未解 | [`re/43`](docs/re/43-open-questions.md) §0 |
