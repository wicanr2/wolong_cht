...
| ⭐ **全軍站定那一刻**（同一場攻城）| 兩邊都在**第 70 拍**站定、**第 75/76 拍**重新開動；那一刻 **`sb-minimap` 只差 8 px（0.04%）**——那一區是唯一看得到全部 96 個兵的，所以它的意思是「整個戰場上兩邊的部隊站在同一批格子上」。七區 0 px；`field` 22% 是**兩個開場對白框**（捷徑路徑推完才武裝喊話，`spec/117`），不是部隊 | [`playtest/74`](docs/playtest/74-settled-tick-parity.md) |
...
| 全專案的未解 | **592 列**（另有 6 列是 DOS／BIOS 平台層與編譯器 runtime，不計入）。⚠ **這個數字比較接近「文件有多少份」**——592 列分布在 243 份文件、平均每份 2.4 列，而每寫一份新文件就帶進約三列自己的未解 | [`re/43`](docs/re/43-open-questions.md) §0 |

#### 那 592 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 592 列，而開局主畫面 **256,000 個像素與原版一個不差**。
