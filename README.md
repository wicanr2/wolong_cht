...
| 反組譯 | 739/739 支有筆記；`docs/re/` 自己標成未解的有 **220 列**，⚠ **其中已知有一部分是過期登記**（見下）| [`re/21`](docs/re/21-function-census.md)、[`re/43`](docs/re/43-open-questions.md) |
| 全專案的未解 | **527 列**（含 re 220、spec 129、playtest 58、mechanics 42、formats 34…）。⚠ **那是列數不是獨立問題數**——索引檔的「現況」欄是別的文件的摘要，同一個缺口會被數兩次（這類 5 列）| [`re/43`](docs/re/43-open-questions.md) §1 |

#### 那 527 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 527 列，而開局主畫面 **256,000 個像素與原版一個不差**。
...
|---|---:|---|
| 程式碼理解（`docs/re/`）| 220 | **不擋任何東西能不能玩。** 這是「某支函式／某個變數還沒讀透」的存量——它的價值是**未來的修正從這裡出來**，不是現在有東西壞掉。⚠ **這一類已知含過期登記**：解掉之後答案常常寫在別份文件（規格／機制），沒人回頭清這裡 |
| 其他（`spec` 129 ＋ `release` 13 ＋ `mobile` 12 ＋ `promo` 2）| 156 | `spec` 那 129 列多半是**已實作項目自己的剩餘細節**。`release`／`mobile` 的 25 列裡**真正擋發行的只有四件事**（見下），其餘是設計取捨與流程小事 |
| 驗收（`docs/playtest/`）| 58 | **不是「做錯了」，是「還沒量」。** 這一類最誠實也最容易被高估——它標的是「我們相信它對，但沒有數字」 |
...
| 資料保存（`docs/formats/`）| 34 | 影響的是**文化資產保存的完整度**，不是玩得動玩不動。例如 `ICONGRF` 段 1 前三塊的 UI 語意 |
...
⚠ **這個數字變小不自動等於進度。** 2026-08-21 的稽核把它從 570 一路降到
527，而那 −43 **完全來自刪掉過期登記與修掉工具的精確度問題**——
期間只新增過 **1 列**真缺口（`spec/60` 的開戰對白側別）。
**沒有任何一列是靠解出新東西減掉的**（逐輪紀錄在 [`WORKLIST.md`](WORKLIST.md)）。

⚠⚠ **而且這個數字本身還沒被逐列核實過。** 已知它偏高：
`docs/re/` 那 220 列裡抽樣就找到過期的（例如「`KYOGRF`／`IVENTGRF`
的尺寸未解」，而那兩個在 [`formats/03`](docs/formats/03-grf-images.md)
是 confirmed 並且渲染驗收過）。
⭐ **成因是結構性的**：`docs/re/` 的未解表沒有負責人，
而解掉它的人通常把答案寫在**別份文件**（規格、機制，甚至只寫在程式註解裡），
