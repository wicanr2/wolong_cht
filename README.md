...
| 規則還原（M4／M5）| **78%** | 政略／行軍／戰術／外交／說服／單挑都以機器碼出處實作，規則層 5 年長跑不變量成立，67 份規格 66 CONFORMED；2026-08-25 逐條覆核後行為級缺口收斂到 **5 條機制＋2 條 remake 接線**（無內政官成長、協力門檻、場合適性、行軍節點、君主陣亡等六條查證定案）| 剩：災害傷害量、評價的去向、外交回報的信賴度增減、信賴度初始時序、行軍中間節點與事件 10 訊息的接線；**AI 決策沒有與原版的長程對照**（最大的一塊）|
...
| **整體** | **80%** | 量過的地方幾乎都對到 0 px、規則層可玩且穩定、保存目標（格式＋文字）接近完備 | 拉低整體的是平台實機驗收（外部條件：缺機器）、mechanics 剩餘的行為級缺口與 AI 長程對照 |
