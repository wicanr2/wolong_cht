...
攻城戰九區裡六區逐像素相同、戰場區 0.17%（其中 299 px 原理上消不掉，
...
| 攻城戰（9 區）| 6 區 **0 px**；`field` 307 / 176,640 ＝ **0.17%**、小地圖 8 px、對方將旗 44 px | [`playtest/40`](docs/playtest/40-tactical-parity.md) |
...
| 軸 | 分數 | 依據 | 扣分在哪 |
|---|---:|---|---|
| 資料格式保存（M1）| **9 / 10** | 全部檔種有 Go 解碼器＋測試，`TALK.DAT`／存檔 byte-for-byte round-trip，過場、音訊、地圖族全解 | `ICONGRF` 段 1 的 UI 語意；`formats` 還有 33 列登記在案的細節 |
| 文字保存與校訂（M2／M7）| **9 / 10** | 1,022 則全保存、60 筆校訂可重跑、兩版 1,022 則逐句對照讀完、排版 parity 全量 0 超寬 | 缺兩版並排的畫面對照這最後一格 |
| 反組譯理解（M3）| **7 / 10** | 739/739 支函式都有筆記、四個分級收斂；單挑這種末端子系統都能讀到逐 tick | 「有筆記」≠「讀懂」——`docs/re/` 自己登記的未解還有約 180 列 |
| 規則還原（M4／M5）| **7.5 / 10** | 政略／行軍／戰術／外交／說服／單挑都以機器碼出處實作，規則層 5 年長跑不變量成立，67 份規格 66 CONFORMED | `mechanics` 還有 24 列會影響行為的未解；AI 決策只驗過「跑得起來」，沒有與原版的長程對照 |
| 畫面一致（M6）| **9 / 10** | 十個局面逐像素：主畫面／系統選單／三視窗／五張清單／財政全 0 px，攻城 0.17%、野戰 0.05%，殘差全數歸類 | 分數只涵蓋量過的局面——沒量過的畫面仍是「照機器碼做＋目視」 |
| 音訊 | **7 / 10** | 格式與場景對應全從機器碼讀出、OPL3 合成與原版錄音人耳比對過 | 諧波結構沒有量化比對；無音效裝置會掛的問題還在 |
| 發行與平台（M8／M9）| **5 / 10** | 四平台包出得來且批次一致、Linux 有 GUI smoke、Android 模擬器指紋與桌面相同、發行閘擋原版資產 | Windows／macOS／Android 都沒有實機驗收；Android 只有 debug 簽章 |
| **整體** | **8 / 10** | 量過的地方幾乎都對到 0 px、規則層可玩且穩定、保存目標（格式＋文字）接近完備 | 拉低整體的是平台實機驗收（外部條件：缺機器）與 mechanics 的行為級未解 |

讀法同上一節：**分數對應「量過的部分對不對」**，不是進度條。
7 分那兩軸的天花板是「未解列數收斂」，5 分那一軸的天花板是實機——
後者不是工作量，是缺裝置。
