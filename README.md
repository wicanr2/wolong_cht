...
| 規則規格 | **126 份**（不含索引與 `TEMPLATE.md`）：**125 CONFORMED**／1 READY（`83-initial-strategy-pass`）／0 DRAFT | [`spec/00`](docs/spec/00-index.md) |
...
| 全專案的未解 | **617 列**（另有 6 列是 DOS／BIOS 平台層與編譯器 runtime，不計入）。⚠ **這個數字比較接近「文件有多少份」**——617 列分布在 254 份文件、平均每份 2.4 列，而每寫一份新文件就帶進約三列自己的未解 | [`re/43`](docs/re/43-open-questions.md) §0 |

#### 那 617 列對 remake 代表什麼

**它不衡量「離做完還有多遠」。** 最直接的證據是這兩個數字同時成立：
未解 617 列，而開局主畫面 **256,000 個像素與原版一個不差**。
...
83% 那一版的依據是「靜態讀完機器碼＋規則層長跑不變量成立＋AI 長程方向一致」；
第一次把原版的**執行期狀態**逐日讀出來對，抓到一條漏掉的規則，於是降到 78%。
接著把同一套作法推到戰術層、逐兵比座標，抓到第二條並補上，
**同一場第 70 拍 96 個兵逐槽全等**，才加回 2 分到 80%。
加分的理由永遠是「量過了」，不是「修好了」。
...
| **行為層（不是畫面）的對拍** | 才剛開始。做過軍團表（8/8 相同）、行軍一條規則、開場布陣（**0/48 相同**）；武將表、據點表、勢力表與 AI 決策都還沒 |
