...
| 文字保存與校訂（M2／M7）| **90%** | 1,022 則全保存、60 筆校訂可重跑、兩版 1,022 則逐句對照讀完、排版 parity 全量 0 超寬；**多語系第一期**：簡體全鏈（OpenCC 機轉語系包＋字級表＋HZK16，`-lang zh-hans`）＋英文骨架（[`docs/spec/84`](docs/spec/84-multilanguage.md)）| 缺兩版並排的畫面對照這最後一格；簡體是機轉初稿未逐則校訂、英文 talk 未譯 |
