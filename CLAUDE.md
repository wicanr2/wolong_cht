...
- **簡體、日文、英文都已接通**（使用者裁定 2026-08-26，見 §6 與
  [`docs/spec/84`](docs/spec/84-multilanguage.md)）。
...
## 6. 多語系：四個語系（繁中母本／簡體／日文／英文）

**四個語系都已接通**（`-lang zh-hant|zh-hans|ja|en`，
規格與踩過的坑在 [`docs/spec/84`](docs/spec/84-multilanguage.md)）。

- ⭐ **日文不是翻譯，是原版**：PC-98 版與松崗版逐則、逐槽對應，
...
- **語系檔在 `translations/`，由 `tools/langpack.py`／`tools/namepack.py` 產生**，
  產出進版控、可重生；字型一律使用者自備（`tools/pcf2raw.py` 可把 X11 的
  `jiskan16`／`gb16st` 轉成本專案的區位格式）。
...
- **排版層必須先抽離。** 原版版面是照全形字排的；
  英文已靠「斷詞折行 ＋ 翻頁」吃下去，**固定寬的欄位還沒為半形語系重算**
  （`docs/spec/84` §6）。
...
16. **拓樸一樣不等於路一樣。** 兩個演算法可以產生**完全相同的圖**
    而每條路都不同（行軍：254 條邊裡 253 條兩邊都有，其中 109 條路徑長度差 > 2 格）。
...
34. **不要改一支正在跑的 shell 腳本。** bash 是邊跑邊讀的，它記的是**位元組位移**——
    在腳本執行期間插入幾行，之後讀到的就是位移過的位置，症狀是
    「第 227 行：指令找不到」而該行語法完全正確。`bash -n` 也驗不出來，
    因為檔案本身沒問題。要改就等它跑完，或先複製一份再改。
    這一條對「驗收腳本」特別致命：**跑完了、exit 0、而新加的那道檢查一次都沒執行**。
...
### `[HARD]` 動工前先寫 spec——polish 也算

**任何會改到 `internal/` 或 `cmd/` 的工作，動手之前先在
[`docs/spec/`](docs/spec/00-index.md) 寫一份規格。** 使用者裁定 2026-08-14，
講過兩次：**不管是 polish 還是新功能，都要先有規劃再動工。**

「這只是調一個常數」不是例外——指令列的節距就是一個常數，
而它錯了是因為當初照影片猜、沒有先去讀 `cs:6181h`。
**規格的第一格是「出處」，光是要填那一格就會逼你先去找原版怎麼寫的。**

順序是：

1. 查三張表（下一節），確認這件事解了沒。
2. 寫 spec：出處 → 算式 → 推論等級 → 打算改哪幾支 Go → 怎麼驗。
   **出處填不出來就回去做 RE**，不要憑現況反寫規格——
   那只會把既有的偏差固定下來。
3. 動手。改完回填「remake 實作」與「驗證」兩欄，狀態升 `CONFORMED`。

> ⚠ **「結果對了」不代表流程可以省。** 事後補的規格擋不住任何東西——
> 規格要擋的是「照影片猜」那一步，而那一步發生在動手的當下。
...
「這一層根本沒在比」——兩者輸出長得一樣。
⭐ **而自我測試自己也會漂。** `stale_scan` 的雜湊層逐行配對「檔名 … SHA-256 …」，
而推廣片與發行產物的標準寫法是條列式、檔名與雜湊分屬不同行——那一層因此
**結構上看不到那幾支影片**，綠了幾個月一次都沒比過（2026-08-29 用突變測試
才發現：把主預告的雜湊改錯一個字元，掃描照樣通過）。成因是自我測試自己
重寫了一份比對邏輯，於是它驗的是一個本體早就不是這樣做的東西。
**正對照要呼叫本體的那一支，而且要用真實文件的版面當樣本。**`stale_scan`、`parity_diff`、
