...
逐拍對拍（`workplace/parity/drift/`）把攻城的第一個分歧釘在**拍 2**：原版拍 0／1／2 一格都沒動、拍 3 才走，remake 拍 2 就走了。**把 remake 的取樣往後錯一拍，拍 0–2 的 96 槽完全相同、拍 3–5 只差 2 個兵**（`docs/spec/200`）。

移動那一側的閘已經讀出來：`sub_1ADC8` 的 `0001ADF1`–`0001ADFE`，兵記錄 `+0x01` 非零就遞減並**跳過移動**。remake 已經有這個欄位（`Soldier.Stun`，`docs/spec/63`）、移動端也接上了——**缺的只是開場那一次的設定**。

⚠ **不要照現象把它設成 1**。擺位段唯一碰 `+1` 位移的兩行是軍團記錄不是兵記錄，所以設定端還沒定位。下一個要看的是 `funcs_1A7E1` 跳表選中的命令 handler（跳表 offset 要正確的段基底，第一次用 0x10000 湊，`[0]` 落在一段 `db 0` 上）。

拍 3 之後仍有 2 個兵對不上，最大位移隨拍數增加，那是另一件事。

**怎樣算做完**：找到原版開場設 `+0x01` 的那一行，remake 接上之後逐拍 96 槽全等到至少拍 6。

**verify**：`present` `/狀態：DRAFT/` 在 `docs/spec/200-opening-move-delay.md`
...
- [x] 新增「經典再現」原版／remake 比較片：
  `wolong-remake-classic-revival.mp4`（檔案已不在，見 `docs/promo/classic-revival.md`），60 秒、
