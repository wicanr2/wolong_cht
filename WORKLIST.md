...
#### 同局面對拍的第一個分歧：拍 307 據點 93 少取一個亂數

逐子刻對拍現在是 329 / 5,480（6.0%），第一個分歧落在拍 307：據點 93（後來還有 118）每 192 拍少取一個亂數。`governor.Tick` 的第三個取數是徵兵，只在城兵**未達上限**時發生，所以那兩座城的城兵在 remake 的世界裡提早滿了。與對峙、退卻兩條線都無關。

**怎樣算做完**：找出城兵提早到上限的來源並修掉；第一個分歧往後推，或說明為什麼那一拍的差異是原版行為。

**verify**：`present` `/成因還沒追/` 在 `docs/playtest/119-rng-pace-comparison.md`
