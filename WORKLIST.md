...
#### 拍 3,415：據點 `+0x18` 是佔用圖抄來的，不是座標數出來的
...
根因是語意不同：原版的 `+0x18` 是 `sub_13EFD` 從**佔用圖**抄的（`al = es:[bx] & 7Fh`，`docs/re/44` §1），而那張圖是增量維護的（`sub_12662` 出發 `dec`、`loc_126F5` 到站 `inc`）；remake 的 `occupancyAt(x, y)` 是每次掃座標重算。停在目標上的軍團每輪走 `loc_1266A` 直接 `retn`，**不 dec 也不 inc**，所以圖上的值來自它最後一次「走進來」——在那一格**編成**的軍團就從來沒進過圖。

**怎樣算做完**：建模佔用圖（或找出等價的判準），拍 3,415 之後仍是四張表 0；逐拍取數的第一個分歧往後推。

**verify**：`absent` `/occupancyMap|佔用圖.*增量/` 在 `internal/state/strategy.go`、`internal/state/corps.go`
