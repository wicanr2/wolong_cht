...
#### ⭐ 對拍在 5 月 13 日停住：君主召見軍師（要決策）

同局面逐子刻對拍（4/16 16 時起跑）在 **5,787 拍 ＝ 196 年 5 月 13 日 11 時**停住，原版畫面是「孫乾　大人，主公有事召見。」

玩家扮的正是軍師（`CLAUDE.md` §3.2），所以原版**正確地**停下來等玩家。把 `steps:` 從 1.47 億加到 3 億，時鐘一步都不動、據點游標定在 144。

⇒ 這一局的無人值守上限是 **27 天**（4/16 → 5/13）。目標是 90 天，所以要先決定「召見」這類玩家事件在對拍裡怎麼處理：

1. **按掉**（dosgolem 送 click／key），兩邊都當成「玩家什麼都不做」；
2. **兩邊走同一條玩家路徑**——那要先把 remake 的玩家互動接進對拍腳本；
3. **換一個不會被召見的局面**（別的勢力／劇本）當長跑基準。

⚠ 三條路的工作量與意義都不同，**這是決策不是實作細節**。

**怎樣算做完**：使用者裁定走哪一條，或對拍能跨過 5 月 13 日 11 時。

**verify**：`manual` — 要人判：三條路各自的代價與「對拍還算不算純 AI 軌跡」由使用者定。
