...
#### 拍 5,176：remake 打了一場而原版沒打

三十二個檢查點（拍 200 … 5,058）軍團表全 0、逐拍取數不一致降到 3／5,480 之後的第一個分歧。

拍 5,176（據點 162）：原版 3 次取數（只有 `141D8`／`141F1`／`14216`），remake 18 次（一整場自動判定）。⇒ remake 多打了一場。

同一拍的檢查點：據點 14 個 byte／4 座、勢力 8 個 byte／2 個、軍團 48 個 byte／4 支（其中 1 支只在單邊存在）。

另外還剩一筆：據點 106 的 `+0x15`（原版 06、remake 00）——**那個欄位在 `docs/formats/08` 沒有記錄**，語意未解。

**怎樣算做完**：`tools/parity_ck.sh` 在拍 5,200 的五張表都是 0。

**verify**：`present` `/remake 打了一場而原版沒打/` 在 `docs/playtest/119-rng-pace-comparison.md`
