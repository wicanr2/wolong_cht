...
- 軍團 73 的 `+0x0C`（路徑指標）差 4 ＝ 一步。原版踏上端點走的是 `loc_126EA` 的 `sub_127F6` ＋ `sub_127A2`（跨節點），而加 `+0x0C` 的是 `sub_126FF`——兩條互斥。⚠ 試過在 `step` 裡用 `len(w.routes[i]) > 0` 當判準，**會全面退步**（2 支變 7 支）：那個條件對所有反向段的最後一步都成立。
- 軍團 89 的 `+0x08`（朝向）差 1。
- 據點 106 的 `+0x15` 差（欄位語意未解）。

逐拍取數的下一個分歧在拍 5,104（第 14 場戰鬥 remake 多取一次亂數）。

**怎樣算做完**：`tools/parity_ck.sh` 在拍 5,040、5,080 的五張表都是 0。

**verify**：`present` `/路徑指標那一筆試過/` 在 `docs/playtest/119-rng-pace-comparison.md`
