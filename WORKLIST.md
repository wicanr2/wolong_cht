...
#### 據點鄰接遮罩：原版增量維護、remake 由現況算

`threat.EnemyMask` 的註解說「這裡直接由現況算，結果一樣而且不會漂」——**機制上不等價**。

原版 `sub_1890A` 在據點換手那一刻**雙向**更新：找出鄰居的哪一槽指向這座城（`cmp ah, [di]`，四槽掃描得到位遮罩 `dh`），然後同時改**鄰居的** `+0x00`／`+0x1B` 與**自己的** `+0x00`／`+0x1B`；新主與鄰居同勢力就清位、不同就設位。

remake 在據點游標掃到時才重算（192 拍一圈），所以換手之後到游標掃到之前理論上會不同。

⚠ **目前的對拍區間（5,480 拍）沒有暴露這個差異**——拍 5,097 一度差過（據點 85／88），但那是據點 `+0x15` 沒讀回造成的連鎖，修掉之後就沒了。留這一條是因為機制差異本身有證據（`sub_1890A` 逐條讀出），**不是因為有未解的觀測**。要動它得先有能分開兩種讀法的樣本。

**怎樣算做完**：找到一個能分開兩種讀法的對拍樣本，或把 `Adjacency`／`EnemyNeighbours` 改成由 `capture` 照 `sub_1890A` 維護的狀態。

**verify**：`present` `/直接由現況算/` 在 `internal/rules/threat/threat.go`
...
對拍在 **5,787 拍 ＝ 196 年 5 月 13 日 11 時**撞到「孫乾　大人，主公有事召見。」停住。⭐ **推得過去**——照 `docs/playtest/118` §3 的序列（右鍵推進 → `move` 選列 → `press` → 右鍵推完），時鐘走到 5/14 12 時。

還要做的：

1. 把回應序列包成 `parity_ck_orig.sh` 能重複套用的東西（召見會發生**很多次**，每次的 `steps:` 偏移都不同）。
2. remake 側要走同一條——「選第 3 列＝絕」是**玩家決策**，會改變狀態（`playtest/118` §3：軍師回答「呂布　令人顧慮…」）。目前 remake 的對拍腳本 `rng_pace.go` 沒有玩家互動。
3. 確認 90 天內還有沒有別的停點（`playtest/118` 說劉備那一局只有召見）。

⚠ 這一項的前提已經解掉了（推得過去、序列已知），剩下的是接線。

**怎樣算做完**：`tools/parity_ck.sh` 能取到 196 年 7 月 16 日（第 90 天）的檢查點並比對。

**verify**：`absent` `/rclick/` 在 `tools/parity_ck_orig.sh`
