...
#### 同局面對拍的第一個分歧：拍 19 多取一個出擊計時

對齊法訂正之後（`-city-cursor 171`，不是 `-skip 27`，`docs/playtest/119` §34），第一個分歧從拍 307 提前到**拍 19**：那一拍 remake 多取一個 `aimarch.go:87`——`sub_143AF` 的「出擊前隨機等 1–8 拍」。\n\n那一支的四道閘與 `sub_143AF` 逐條對得上，裝探針照抄 `[di+18h]` 那個未初始化讀取也**一個字都沒變**，所以觸發者是條件的前半（`cityThreatened`）或更前面的 Stage 判斷。\n\n⚠ 亂數流一岔開，**所有吃亂數的欄位就跟著漂**——拍 200 的據點表還有 141 座對不上，形狀是對稱雙向的小幅偏移（上昇值 ±5），那是亂數岔開的指紋不是規則錯。

**卡在**：要往下追得取原版拍 19 附近的檢查點（`corps` ＋ 據點表），現在只有拍 200／700／1,200／1,600／1,900。

**怎樣算做完**：第一個分歧往後推；拍 200 的據點表差異大幅下降（現在 220 byte／141 座）。

**verify**：`present` `/要往下追得取原版拍 19 附近的檢查點/` 在 `docs/playtest/119-rng-pace-comparison.md`
...
#### 拍 1,900 軍團 50 的 `+0x00` 位元 0 多設了

五個檢查點裡唯一剩下的 byte：軍團 50 的旗標原版 `C4`、remake `C5`，差的是**位元 0（已經走上路徑）**。其他 63 個 byte 全部相同——同一條路徑指標（`+0x0C` ＝ `57FCh`）、同一個連結記錄、同一組座標。

它在拍 1,600 時還停在據點 129（兩邊都是 `C4`），1,600–1,900 之間走上路徑。位元 0 **沒有清除端**（`docs/spec/173` §1.1），所以原版走了一格卻沒設它 ⇒ 那一步不是經過 `sub_126FF`／`sub_147BB` 這兩個已知設定端進來的，還有第三條路徑。

這個位元是 `sub_12708` 第二個入口的閘（`docs/spec/175` §3.2），會影響對峙判定，不是純顯示欄位。

**怎樣算做完**：`tools/parity_ck.sh` 五個檢查點全部 0 個 byte。

**verify**：`present` `/還有第三條路徑|不是.*經過 `sub_126FF`／`sub_147BB`/` 在 `docs/playtest/119-rng-pace-comparison.md`
...
### 2026-09-08 桌面發行與影片收尾

- 依使用者要求整理 Release、完整版及推廣片；套用已指定的 RRSAL-1.0，重建 v.1.0.18 三平台完整版，保留所有舊版。
- 另建不含原版資產的公開引擎包，改用文字圖示並附第三方條款；公開 AppImage 用自備資料參數啟動成功。
- 本版完整 AppImage 正常重啟與兩場自然遭遇通過，結果頁倒數區間 27.27–30.80 秒。遊戲來源相對 v.1.0.17 未變，只更換授權檔。
- 重剪 56 秒桌面推廣片，區分歷史實錄與新版實拍；原版音訊來源、字幕、音畫長度與媒體檢查保存於本版 promo/，不隨公開包上傳。
...
- 使用者提醒委任應由呂布勝；原版委任位元與戰術放置分開驗。真正委任兩邊皆呂布勝，兩側各隊兵力、士氣及 RNG 前綴相同，但完整退路／壞滅出口尚有差異。
- IDA 9.4 證實 `0001A5BD jnb`／`0001A5C6 jbe` 是無號 ≥／≤，舊文件與程式誤作 <／>，把兵力充足的呂布送入退卻。僅改此兩處的隔離對照與正式重播均恢復呂布勝，不改能力或 seed。
- 補原始 opcode 邊界回歸；舊野戰終局 fixture 漏接正常畫面的單挑輸入，修正後正常結束。單挑左右位置候選未進正式程式，不以勝方相同冒稱完整逐拍一致。
- 新桌面修正版 v.1.0.17；AppImage 正常操作與清除快取後完整檢查通過。首次 GUI 計時觀察區間未達門檻，保留失敗並以同一命令乾淨重跑；第二輪 29.44–32.26 秒通過，未放寬斷言。自評維持 83／100；Windows／macOS 人工驗收，Android 與全局通關排除。
- 三平台封包與 349 個建置來源核對通過；111 個交付檔案與證據壓縮包內 6103 個成員 SHA-256 吻合，輸出皆 UID 1000。原版 EXE 與 IDA 來源雜湊未變。本輪容器已退出並自動移除，未 commit 或 push。


### 2026-09-08 快速切焦與完整同狀態戰況

- 快速切焦的重定位誤差已重現為 103,752 像素差異，改為跨 Draw 後重建捕捉基準；正式 AppImage 半秒切焦與水平／垂直外推共 67 項比較通過。
- 擺兵取亂數次序及空槽消費已修，攻城／野戰各 1056 個已對應初始欄位及 RNG 一致；正式 AppImage 各 96 槽座標／體力一致。首次 Draw 未證實同拍，不宣稱圖像完全一致。
- dosgolem 完整攻城退卻與野戰自然結算均已重生；同拍指令的共用規則重播第 1 拍即分歧。野戰原版呂布勝，remake 夏侯惇勝，完整戰況驗證失敗；原版 312／324 拍指令在 remake 仍遭單挑開場阻擋，沒有延後重送。
- 暫停更新期間 50 毫秒切焦，水平／垂直各兩次，共 51 項比較通過；正式包與來源 349 個檔案相符，三平台封包通過，最終 311 個檔案 SHA-256 吻合。戰況壓縮包內 2598 個成員也逐一核對。
- 自評下修 83／100（加權 82.85）。Windows／macOS 原生操作由人工檢驗，Android 與全局通關排除。v.1.0.15 保留候選；本輪修正版 v.1.0.16 不覆寫舊包。冷 Go 測試及完整檢查通過；本輪容器皆已退出並自動移除，未 commit 或 push。

### 2026-09-08 桌面打磨與人工驗收分工

- 現行交付 v.1.0.14-20260908；設定記憶、原版整格繪圖與一般地圖格框已接上，系統視窗的外推穿透與事件提示遮擋已修。
- 正式 AppImage 設定重啟、連續兩次自然遭遇的結果頁、37 項地圖比較及行軍選陝／存第二槽通過。349 個建置來源一致，三平台封包查核通過；細節見 playtest/113。
- Windows／macOS 原生操作由人工檢驗；低更新率半秒快速切焦曾跳圖，兩秒回焦通過不代表所有快速切焦皆修好。完整同狀態戰況與全劇本通關仍未證實。
- v.1.0.13 因系統視窗遮擋未完成驗收，保留候選，不覆寫或當成現行交付。
- 自評維持 85／100（加權 84.85）；Android 不改，未 commit 或 push。冷測試與完整檢查通過，最終 262 個檔案 SHA-256 全數吻合。Docker 工作皆使用 --rm，本輪容器已全部退出並自動移除；未清理其他專案資源。


### 2026-09-08 戰後結果頁與自評更新

- 使用者採用結果頁選項、預設關閉、設定秒數後自動關閉。系統第九列已接關／3／5／10／15／30 秒，不改原版六列位置；正式 AppImage 的預設關閉與三秒自動返回通過，收據見 playtest/112。
- README 自評為加權 84.7 四捨五入 85／100，新增桌面操作軸；舊自評及舊限制清單保留於本檔歷史快照。
- 自動審查曾誤認 check.sh 在主機跑測試；讀取 check.sh、go.sh、py.sh 證明工作負載均在 Docker 後，重試獲准並通過。


### 2026-09-08 已採用原版滑鼠外推

- 使用者採用持續外推才捲動、停手即停，保留方向鍵。桌面相對輸入、像素餘數、原版分別取整選點及失焦釋放已實作。
- 正式 AppImage 一般地圖、行軍選點、停手與焦點回復通過；正常選陝並退出後存第 2 槽通過。證據與限制見 [playtest/111](docs/playtest/111-desktop-map-scroll.md)。
- v.1.0.10 曾抓到失焦跳動，未通過且不作交付；修正版為 v.1.0.11-20260908。非整格原版畫面仍有水平 1 像素殘差，成因未證實。
- 格式整理碰到十個原先乾淨的測試檔；自動核准審查拒絕撤回純格式差異，因此保留。未 commit 或 push。


### 2026-09-08 地圖捲動與戰後返回實跑

- 地圖：`workplace/map-scroll-20260908/original/oracle.log` 記錄原版正常讀檔後，滑鼠向右推使捲動原點由 `(2976,1632)` 變成 `(3105,1632)`；停止移動後原點保持不變。AppImage 滑鼠移到邊緣不捲動，方向鍵可捲動。操作方式已提出共同決策，尚待回答，未改正式程式。
- 戰後：`workplace/battle-return-20260908/original/timeline.txt` 從讀檔、成軍、自然遭遇走到退卻；其後只執行指令、不增加確認輸入，即返回戰略地圖。AppImage `app/probe.py` 走正常遭遇與退卻，`post-7.png` 停在結果頁，完成觀察後點擊，`returned.png` 回到戰略畫面。這證明額外確認步驟，不證明完整戰況或亂數同狀態。
- 輸入與收據身分保存在 `workplace/battle-return-20260908/receipt.json`：dosgolem `fd74640`、AppImage `v.1.0.9-20260908`，共 78 個檔案的 SHA-256。既有 spec/89 §6 已明列結果頁本身為 remake 差異；目前尚未修改結果頁，也未重打包。
- Windows／macOS 原生驗證仍缺可用環境；Android 本輪不在範圍。上述容器均已退出並自動移除，未 commit 或 push。
...
#### 主君編成：系統選單裡的開關（[`docs/spec/76`](docs/spec/76-lord-not-in-formation.md)）

`formCandidates()` 沒排除君主，所以玩家可以拿曹操去帶兵。
**先做成硬擋，使用者當場改成「可切換的選項」**：開關放系統選單
（調速度那個畫面）、遊戲中可隨時切、**預設「可」**——
也就是預設行為與原版不同，是明確標記的 remake 差異。
...
⭐ **新列加在原版六列的後面，不是插在「遊戲結束」之前。**
插進去的話原版最後那一列會被往下推一格，**六列沒有一列還在原座標**；
加在後面只多出下緣 24 px。代價是「遊戲結束」不再是視覺上的最後一列
——換到的是原版版面不被動到。

⚠ 既有的版面契約測試當場擋下這個改動（釘著「視窗高 192、六列」），
那正是它該做的事。改成把**原版值與 remake 差異都釘住**：
高度寫成 `192 + sysRowStep`、原版六列的標籤與順序另外驗一次。
[`docs/playtest/39`](docs/playtest/39-system-window-parity.md) 的
「系統選單 0 px」已如實改寫成缺口——**沒有重量就不拿舊數字宣稱現況**。
...
### 2026-08-23（三）重新盤點斷言：七處錯的或衝突的

今天六輪改動之後回頭核 markdown ＋ 原始碼註解。`check.sh` 全綠，
所以找的是**它抓不到的那一類**。

| # | 問題 | 修法 |
|---|---|---|
| 1 | ⭐ **`docs/re/67` 早就記過顯示表**，而我在 `re/72` 又推導一次 | `re/72` 開頭寫明是誰先發現的，並記下「沒查到不是因為查不到，是查錯關鍵字」 |
| 2 | ⭐ **槽數衝突**：`re/67` 說五個疊圖槽、`re/72` 說四層 | 回機器碼核：**槽位五個（`+3`–`+7`）、推入端上限 4**。兩件不同的事，三份文件都補上 |
| 3 | `re/30` 仍標 `sub_1D4C7` 未讀 | 已解，劃掉並指向 `re/72` |
| 4 | ⭐ **第八個模態畫面漏掉右鍵**：離開確認的 ＹＥＳ／ＮＯ | 接上 `g.cancelled()`；`spec/73` 補一句「規則是模態畫面，不是面板」 |
| 5 | `spec/24` 說軍團情報「只能按 ESC」 | 已經是右鍵或 ESC |
| 6 | 原始碼註解說「系統選單的版面**全部出自原版**」 | 第七列是 remake 加的，註解改成「只有前六列」 |
| 7 | ⭐ **`spec/72` 引用的驗收截圖不存在** | `dist-all/verification/` **每次重打都被 `promote` 清掉**。改成寫批次號 |

順帶把 README 的數字重量：規格 59→**64 份**（63 CONFORMED）、
未解 432→**453 列**。並把 `release/04` 已被 `05` 取代的四列劃掉，
不再重複計數。

#### ⭐ 兩條可以帶走的判準

**一、「會被清掉的路徑不能當證據」。** `verification/` 是發行管線的產物，
每次 `promote` 整個換掉；拿它的路徑寫進規格，下一批一產出就指向空氣。
`phantom_scan.py` 抓不到——那是 inline code 不是 markdown 連結，
而且 `dist-all/` 在 gitignore 裡。**要留證據就寫批次號與重跑指令。**

**二、用實作上的分類去掃，會漏掉不屬於那個分類但同樣適用的東西。**
右鍵取消掃的是「面板」，而離開確認長得不像面板，於是漏掉。
規則講的是**模態畫面**——掃的時候要照規則的用詞，不是照程式的檔案分類。

### 2026-08-23（四）未解數字為什麼越來越多——量出來的答案

使用者問。先前給的說法是「解出新東西會看見更多不清楚的地方」——
聽起來合理，但**沒有量過**。量了之後答案更難聽也更有用。

#### 量法與結果

把 08-21 稽核收尾那版（431 列）與現在逐列 diff，再按來源文件分組：

| 來源 | 新增列數 |
|---|---:|
| `re/71`（熱區分派，新文件）| 4 |
| `release/05`（新文件）| 4 |
| `promo/dosv-realmachine`（新文件）| 3 |
| `spec/72`／`74`／`75`／`76`（各新文件）| 各 3 |
| `re/72`（新文件）| 3 |
| `playtest/41`（新文件）| 2 |
| 既有文件新增 | 8 |

⭐ **幾乎全部來自新寫的文件帶進自己的未解表。**
現況是 **449 列分布在 169 份文件，平均每份 2.7 列**。

#### 結論：它比較接近「文件有多少份」

而且是**制度性的**——`check.sh --strict` 會**要求**每份文件要嘛有未解小節、
要嘛明講 `<!-- 缺口：無 -->`。所以「解出新東西 → 寫一份文件 → 總數上升」
是這個指標的常態。

⚠ 這是 `docs/re/21` §3.1 那條教訓的**鏡像**：那次是寫一份登記表讓覆蓋率
從三位數掉到 8，中間一支函式都沒多讀；這次是寫文件讓未解數上升，
中間一個問題都沒多出來。**兩次都是量測工具把自己的輸出算進去了。**

處置：把這件事寫進 `re/43` 的 §0（生成的，每次重跑都會重算平均），
以及 README。**不改指標的定義**——它回答的是「還有什麼沒解」，那個問題
本來就不該有「還剩多少」的形狀。要看進度看規格的 CONFORMED 份數、
逐像素數字與覆蓋地圖。

#### ⭐ DOS／BIOS 平台層不再計入（使用者裁定）

`INT 61h` 的音效 TSR 服務號、VRAM 寫入迴圈、`YNFONT.EXE` 怎麼畫字——
**remake 不跟 DOS TSR 講話**，知道 `ah=4` 是什麼不會改變任何一行 Go。
四列分流到 `re/43` §9。

⚠ **用標記（`[DOS/BIOS]`）不用關鍵字。** 關鍵字掃過一次，`port` 中了
`spec/64-capital-relocation-report` 的**檔名**——語意判斷交給寫的人，
工具只認明確標記。
⚠ **分開數不是不數**：那些仍然是原版的未解之處，只是不擋 remake。

### 2026-08-24（四）稽核：五處錯的斷言，其中一處是「同一段文字放兩個地方」

打包完之後把 markdown 與註解掃一輪。

| 位置 | 曾經寫過 | 實際 |
|---|---|---|
| `docs/re/24` ×4 | `sub_1D615`／`sub_18E5A`／`sub_1B15D`／`sub_1B186` 仍列在未讀目錄 | 這四支今天都解掉了，改成已解並指到出處 |
| `MEMORY.md`、`RESEARCH-LOG.md`、`CONTEXT.md` | `00017DEC` ＝「還原初值」 | **設成上限**。呼叫端傳的那一個值就是上限（`si` 開場一律 0，沒有初值可傳）|
| `cmd/wlgame/amountpanel.go`、`internal/state/state_test.go` | 同上，寫在註解裡 | 同上 |
| `CLAUDE.md` M4 | 「規格已到 `docs/spec/71`」 | `79` |
| `docs/release/README-RELEASE.md` | 日期 `2026-08-12`、版本 `20260812`、界線寫「原版資料不隨包散布」 | **那是包裡那份說明的手抄複本**，而真正的來源是 `packaging/release/README-RELEASE.md`（帶 `@PKG_BOUNDARY@` 佔位符，依批次填）|

⭐ **最後一條的形狀最值得記**：它不是「忘了更新」，是**同一段文字放了兩個地方**。
`dist-all` 從 2026-08-22 起就是內含遊戲檔案的完整版，包裡那份說明跟著模板換了，
docs 那份手抄複本停在 12 天前——**而兩份都存在的時候，過期的那一份不會有人發現**。
處置是把 docs 那份改成指標，不再複製內容。

⚠ 順帶一提，這五處**沒有一處**是 `check.sh` 能擋的：它擋的是狀態行自相矛盾、
連結指不到、幽靈檔案。「內容過期但格式正確」目前只有人工掃得到。

### 2026-08-27（六，第三輪）三件 polish：攻城、大地圖、系統選單

上一則量到的三個「取樣點不等價」，逐一追下去有**四個是真的缺口**，
全部修掉。共通點：**每一個都通過了既有的驗收，因為既有的驗收只問「有沒有變」。**

**攻城（`field` 4.32% → 0.86%）。**

- **城壁一撞就垮**（[`docs/spec/93`](docs/spec/93-siege-wall-instant-break-facing.md)）。
  `hitStructure` 的「一撞歸零」捷徑，原版寫成兩個分支：玩家攻城時比面向 0
  （West）、玩家守城時比 2（East）。玩家守城時戰場轉 180 度，攻方改從 X 58
  朝西走——**兩個分支指的都是「背對城」**。remake 寫死成 West，於是攻方
  朝城走就被判成背對城，城壁在第 148 幀一幀之內從 1,660 掉到 0。
- **門強度那一塊是視窗，remake 沒畫外框**（[`docs/spec/32`](docs/spec/32-gate-strength-bar.md) §2.2）。
  顯示格與熱區都是 (256,0,224×32)，而清空的 (264,8)-(471,23) 每一邊內縮 8 px
  ＝ 外框圖塊的邊長。那一塊 24.57% → 0.13%。

**大地圖的軍團**（[`docs/playtest/50`](docs/playtest/50-corps-on-map.md)）。
`spec/74` 的驗證欄寫「首都那一格要多出軍團圖塊」，而那一格真的多了 122 px。
拿原版同狀態對過才知道其中 171 px 是錯的：

- **剛編成的軍團朝向是 Go 的零值 0**。靜止是 4，0 是「朝 X 減走」——
  畫成側面行進的圖塊。
- **軍團站在首都中心格時，原版不畫首都疊圖**。窮舉那一格 512 種組合，
  只有「不疊首都」是 0 px。首都疊圖本身沒問題（開局主畫面重驗仍 0 px），
  錯的是**同一格同時有兩者**時的取捨。

**系統選單那 272 px**（[`docs/spec/29`](docs/spec/29-audio.md) §5.1）。
`-shot` 直接把音檔目錄清空，`Available()` 因此是 false，
「音　效」那一格印 remake 才有的「未接入」。改成 `Bank.SetSilent(true)`：
只讓 `PlayMusic`／`PlayEffect` 返回，狀態一律照舊。
系統視窗上緣起 **176 px 逐像素相同**。

> **教訓一**：**「有變」不等於「畫對了」。** 三個缺口裡有兩個的既有驗收
> 寫的是「那一格要多出東西」「條要亮起來」——**變化量是自己跟自己比，
> 對不對要跟原版比**。驗收欄寫「有變」的，等於沒驗。
>
> **教訓二**：**計數為零要分兩種世界**——沒有人來打，與打了但被別的機制
> 吃掉。`spec/32` §4.1 量到「呼叫 10,020 次、傷害 0 次」就停在「地形擋住」，
> 而真正的成因是打了之後被「一撞歸零」吃掉。前者看呼叫端，
> 後者要看被呼叫的那一支怎麼提早返回。
>
> **教訓三**：**驗收捷徑會改到被驗收的畫面。** 關掉的要是**輸出**
> 不是**狀態**——兩者在程式裡常常是同一個開關。

### 2026-08-27（六，下半）把記錄過的對拍全部重跑一次

`docs/playtest/37`／`39`／`40`／`43` 的數字量於 08-17 到 08-24，而之後規則層
與呈現層都動過。**沒有任何檢查會在對拍漂掉的時候變紅**，只能重量
（[`docs/playtest/49`](docs/playtest/49-parity-retest-20260827.md)）。
原版側沿用當時的受控擷取，remake 側用現在的原始碼重截。

- **開局主畫面五區全 0 px、野戰 `field` 0.05%／小地圖 0.16%** ——兩組完全重現。
- **系統選單**：原版那六列裡五列 0 px，「音效」那一格差 272 px——
  ⭐ **是驗收路徑自己造成的**：`-shot` 會一律關音訊（`release/05` §3），
  於是那一格印「未接入」而不是原版的 `TYPE 1`。帶 `-audio` 也沒用，
  旗標在那一步之前就被清掉了。**驗收捷徑會改到被驗收的畫面。**
- **⭐ 攻城沒有重現**（兩個成因後來查出都是真的缺口，見上一則）：`field` 從記錄的 0.17% 變成 4.32%。先排除繪圖層
  （退回兩個版本的 `internal/ui/isoview/` 都逐位元組相同），看側欄小地圖
  再拆開量出兩個獨立成因：**門強度量表從沒畫出來**（第 61／240／600 步
  比那一塊都差 45%，地板 2.8 pp），以及**取樣點被作廢**——`spec/59`
  ＋ `spec/80` 把攻方的出發時間往後推了 60 幾步（直接量規則層座標：
  frame 61 還在 X 58..60，frame 300 已越過城門）。

> **教訓寫成規則**：**同狀態對拍的「取樣點」是規則層的函數，不是常數。**
> 第 61 步在 08-18 是「兩軍都到位、對白框剛過期」那一刻；規則層改過之後
> 同一個步數落在完全不同的局面，而**對拍數字會安靜地爛掉**——那些數字
> 只寫在文件裡，沒有測試會紅。取樣點要用**局面條件**描述
> （攻方已抵城門、對白框已過期），不是寫死一個步數。
...
順帶收掉一個舊缺口：**只有 macOS 的執行檔帶著 VCS 戳記**（osxcross 那顆
只 COPY GOROOT，沒有 `docker/go` 的 `GOFLAGS=-buildvcs=false`），
所以 macOS 執行檔的雜湊會跟著工作區乾不乾淨而變、同一個 commit 也重現不出來。
補上那一行之後四平台一致，批次跟著重打了一次——**Linux 的執行檔逐位元組沒變**
（`wlsim` 前後都是 `74508e5f…`），確認只有 macOS 端改變。

### 2026-08-26（五）看片回報的四件事：對話框、小兵、月結框、配樂

使用者看推廣片回報，逐項比過實機錄影之後全部成立
（[`docs/spec/88`](docs/spec/88-display-polish-parity.md)、
[`docs/playtest/48`](docs/playtest/48-display-polish.md)）：

- **TALK 對話框的底是黑的**，不是藍底龍紋。實機量到框內 6,298 px 全是
  (0,0,0)。系統選單確實是藍底龍紋，沒有一起改。
- **⭐ 小兵只畫得出一半**：`makeDisplayInfo` 的高度只算 lane 0（地形），
  平地上 `z1=0`，兵的下半永遠不畫。而 `spec/58` §1 早就寫著 `+1` 是
  「含物件」的高度。**規格寫對了不等於實作照著做**——58 是 CONFORMED，
  欄位語意一字不差，實作只取了兩個欄位裡的一個。
  ⚠ 對拍沒抓到：量過的兩個局面都在城壁邊，鄰格高度本來就把範圍撐開了。
- **月結框不消失**：`lastEvent` 沒有人清掉。改成六秒後自己消失。
- 「字壓在肖像上」出自推廣片裡 2026-08-10 的靜態圖——**推廣片的靜態圖會
  凍結當時的 UI**，重剪要連靜態圖一起重拍。

另外把**戰後的損害報告改成可關的選項、預設關閉**（原版沒有這個報告），
結果畫面也認滑鼠（[`docs/spec/89`](docs/spec/89-siege-damage-report-toggle.md)）。

推廣片重錄重剪：**配樂改用原版的曲子**（remake 自己的 OPL3 合成），
動態段全部重錄（第一版錄在修正之前，片子裡看得到那兩個問題），
實機對照片改成八段裡六段並排。
...
＋ 兩支 smoke。六個產物全部換新，[`06`](docs/release/06-appimage-20260824.md)
留下的混日期批次收掉了。紀錄在 [`docs/release/07`](docs/release/07-full-20260824.md)。

#### ⚠ 「全平台重建」是兩支腳本，不是一支

`release_all.sh` **不會重建 APK**——Android 是另一條管線，而
`android_apk_name` 刻意用 APK 自己的 mtime 取日期（不掛一個沒發生過的建置日）。
只跑前者的結果是：五個產物是新的、APK 還是舊的，**而 manifest 看起來完全正常**
（它只記路徑，不比日期）。這次是靠人工看檔名才發現。

#### ⚠ 狀態行裡不要放相對連結

把 `06` 標成「已被 [`07`](…) 取代」之後 `check.sh` 立刻擋下來：
`docs/INDEX.md` 會把狀態行**原樣抄過去**，而那一份的基底目錄不同，
`07-full-20260824.md` 在 `docs/` 底下解析不到。改成純文字檔名就過了。
⭐ **會被別的檔案原樣引用的欄位，不能放相對路徑。**

### 2026-08-24（下半）新遊戲的背景是大地圖；AppImage 同日重打兩次

使用者接著回報兩件事：**背景應該是整張大地圖（城池空白）**、
**滑鼠上下滑動清單沒有跟著捲**。

#### 背景：`sub_11A6E` 在進選單前就把地圖畫好了

```asm
mov al, 0 / call sub_10241        ; 調色盤組 0
mov dx, 0AAh / mov bx, 62h        ; ★ 鏡頭 (170, 98) 格
call sub_1D615 / call sub_1D66A   ; 建 40×23 顯示清單並畫出來
call sub_11AC3                    ; ← 四層選單
```

`sub_1D615` 的內外迴圈 `cx=28h`／`dx=17h` ＝ 40×23、每列 `add di, 158h`
（344 ＝ 384 − 40），所以 `dx`／`bx` 是**格座標**。

⭐ **「城池空白」不是缺陷，是這條路徑的性質**：`sub_1D615` 只複製圖塊編號，
歸屬換圖塊（自勢力 −2／他勢力 −1）與軍團疊圖都不在裡面。remake 因此改用
`RenderWorld`（不帶 marks），與原版走同一條。

⚠ 換上地圖之後，劇本名與操作提示這幾行**在草地上完全讀不出來**——
那三行是 remake 加的，原版沒有，所以各自鋪一條黑底。

#### 滾輪

加了。捲的是**視野**不是選取列（原版的 `top` 與選取列本來就是兩個獨立
狀態）；`↑↓` 那一邊才把視野拉回游標。

⚠ **一度連「滑鼠移過去就反白」也加了，然後拿掉**——那與同一位使用者
前一則回報的「滑鼠移動時主君也會跟著移動」直接衝突。順帶量到一件事：
加上 hover 之後，`tools/shot.sh` 拍到的選取列會**跟著 Xvfb 的游標初始位置跑**
（螢幕中心換算回邏輯座標剛好落在第 5 列）——**輸入行為會污染截圖的決定性**。

#### AppImage 同日重打兩次

`20260824` 這個檔名重打過兩次，而且**兩顆大小一模一樣**（squashfs 區塊對齊），
只有雜湊分得出來。兩個雜湊與各自含哪些 commit 記在 `docs/release/06`。

⚠ 驗「新畫面在包裡跑得起來」差點驗成假的：給 `-shot` 會走
`directStartFlagWasPassed` 的白名單**直接進遊戲、跳過殼層**；不給就會開音訊，
而容器裡沒有音效裝置 → 當場結束。第一次照第一條路跑，拍到大地圖第 600 幀，
**腳本照樣印了 ✓**。⭐ **拍到圖不等於拍到要驗的東西**——那張圖本身很正常，
只有對照「該出現什麼」才看得出來拍錯。

### 2026-08-24 選君主那一頁：看不見的熱區，與補上的原版清單

使用者玩出來的：**滑鼠移過去就換君主**。

#### 成因不是「滑鼠感應到地圖」

那一頁畫的是原版版面的卡片（`spec/27`），命中測試卻沿用啟動殼層的清單列
——而那些列**一列都沒畫出來**、範圍（x 128–512、y 88–280）又幾乎蓋滿整張
卡片。所以滑鼠一移過去就換君主、一點下去就直接決定，連點「自定」都會把
君主定案。⭐ **看不見的熱區是最難發現的一種**：畫面上沒有任何東西指向成因。

#### 正對照先做

用 `xdotool mousemove` 掃過卡片六個點再截圖：修正後兩張逐 byte 相同，
同一個腳本跑修正前的建置則從「曹操（1／22）」變成「孫策（2／22）」。
**量到「沒有變」之前先確認這個測法抓得到變化。**

#### 順著讀出整條新遊戲流程（`re/73`）

```
ＹＥＳ／ＮＯ → 劇本 → 勢力清單(sub_17B3C) → 君主卡(sub_18E5A)
每一層 CF=1 退一層
```

**卡片上根本沒有換勢力的熱區**——`re/53` §5 與 `spec/27` §5 掛著的
「未讀」因此結案。清單那一步也做了（`spec/79`）：視窗 (136,104) 384×176、
五欄、一頁 10 列、候選條件只有「勢力存在」。

⭐ **兩份獨立證據對上了**：欄位表 `cs:7B12` 的 X（24／88／168／216／280）
與標題字串「勢力名　軍師名　　武將　據點　　首都」逐字量出來的落點完全相同，
所以標題與列共用同一個列首——這一格不必另外找出處。

#### 順手刪掉的

啟動殼層自己的「選君主」清單（`visiblePlayers`、`launcherPlayerLabel`、
那一段畫不到的 `case` 與它的版面測試）整組移除。**留著就是下一個幽靈熱區。**

#### ⚠ 這一輪量出來的一件事：無頭環境送不進滑鼠**點擊**

`xdotool click`／`mousedown`＋`mouseup` 對 Xvfb 送，畫面一格都沒動——
建置 image 裡沒有視窗管理員，GLFW 的視窗拿不到輸入焦點。
滑鼠**移動**送得進去（上面那個正對照就是靠它）。

⭐ **所以這個專案至今沒有任何一條滑鼠點擊路徑被無頭驗過**——數值輸入器的
3×6 格、一覽表的列與捲軸、戰場的熱區都一樣，靠的是單測把「座標 → 動作」
的對應釘住，加上使用者實際玩。這也解釋了為什麼那個幽靈熱區能活這麼久。

### 2026-08-23（五）清 `re/43`：先查 Go 原始碼，再判斷缺口是不是真的

使用者的提醒（「也許已經實做」）值一整輪：**25 條「remake 沒做」裡，
查下去有三條是文件沒跟上，不是程式沒做。**
...
| 缺口 | 實際 |
|---|---|
| `spec/32`「右鍵提前收掉」 | `battleRightClickHotspots` 早就有；同一份的**狀態行**還寫著「已實作並有單測」，只有未解表沒跟上 |
| `spec/24`「remake 沒有那三個選項」 | `cmd/wlgame/marchmode.go` 三個都在，差的是入口位置 |
| `spec/35` 的 `Faction.Name` | 那個欄位**不存在**；實際走 `g.diplomacyFactionName(n)` |
| `playtest/23` 四張 24×16 圖形 | `re/48` §6 早就解成天秤／馬／弓／步，`DOSVResourceIcon` 用的就是原版素材 |

#### 真的漏掉的

- `poolBack` 退兵回池沒夾上限 → `economy.ClampReserve` ＋ 單測。
- 結局整段放 `overbgm-0`（那是 `D7OVER.EXE` 的遊戲結束曲）→ 補 `endbgm-0`。
  ⚠ 新的 case 要放在 `world == nil` 之前，`-open-ending` 那條沒有 world。
- 敗走的兩段訊息從來沒接（[`spec/77`](docs/spec/77-rout-talk-messages.md)）。
- 爬升／下降的通行判定只驗「目標平面有地面」，**未破的門也爬得上去**。
- 財政的數值輸入器沒接（[`spec/78`](docs/spec/78-amount-input-editor.md)）。

#### ⭐ 讀機器碼時翻出三條反了的斷言

三條都進了 `CONTEXT.md` §6：

1. `sub_1B15D`／`sub_1B186` 的 `cmp dl, 0F8h / jb` 跳到的是 `stc`——
   **≥ `0xF8` 才過得去**，不是才擋。合上呼叫端的 `≥ 0xF0`，語意是
   「只有打破的門上得去下得來」。
2. `sub_17C6E` 的 `ax` 是**上限**不是初值，所以 `5Fh` 是「最大」鍵。
   圖庫上那顆鍵的字樣（`re/48` §6 解出來的）本來就寫著「最大」——
   **兩份文件各自都對，沒有人把它們擺在一起看。**
3. 敗走走不到 TALK `#20`；`#198` 是組編號不是索引（第三次漏套
   「≥ `0x196` 是八格一組」那條規則）。

順帶解掉 `spec/14` 掛了很久的「徵兵數上限」：三支 handler 傳的
`ax = 2710h` ＝ **10,000 人**，寫回前 `div 10`。remake 原本的暫代上界是
60,000 **點**，差兩個數量級——**暫代值沒有症狀，所以不會有人去查它**。

#### 沒動的

- 行軍途中的**中間節點編號**（192–255）：`army.KindOf` 有分段判定但沒有
  寫入端。這一條要先解原版怎麼配號，`re/08` §7.1 的 `byte_1E47E`
  是從 192 **往下**數的據點索引，與「192–255 是路上」對不起來——
  那是真的 RE 缺口，不是查一下 Go 就能收的。
- 其餘 20 條「remake 差異」多半是已知的呈現層取捨（液晶調色盤、
  音源 TYPE 1–4、逐句節拍），或缺原版同狀態存檔的驗收項。
...
以下是更新前的評分與當時理由，不代表目前狀態；現行分數以 README 為準。
