...
5,480 拍的逐拍取數已經零不一致。檢查點的五張表在拍 200 … 5,080 逐 byte 相同，拍 5,176 剩四個 byte：

- **據點 106 的 `+0x15`**（原版 06、remake 00）。`tools/ida_disp_users.py` 掃出的寫入端只有 `sub_1237E`（暴風雨）與 `sub_134B1`，**欄位語意未解**，`docs/formats/08` 沒有它。
- **軍團 39 的 `+0x16`／`+0x18`**（目標 X／Y）：原版留著舊目標（據點 88）的座標，remake 換成新目標（據點 85）的。`sub_1474A` 的 `.retreat` 只寫 `+0x14`／`+0x20`，所以是別的路徑改的——`sub_14DA4`（據點失守後軍團調頭）是最可能的落點。
- **軍團 73 的 `+0x0B`**（移動計時）差 2。

**怎樣算做完**：`tools/parity_ck.sh` 在拍 5,200 的五張表都是 0。

**verify**：`present` `/拍 5,176 剩四個 byte|還剩什麼/` 在 `docs/playtest/119-rng-pace-comparison.md`
