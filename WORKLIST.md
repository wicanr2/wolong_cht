...
#### 快照沒有帶佔用圖：對拍的可信範圍停在拍 3,415

同局面對拍的第一個分歧。據點 129 的求援調兵，原版取一次 `14171`（跳過判定）、remake 取兩次 `strategy.go:642`。

兩邊的參數量出來了——原版的 `-watch` log 直接有暫存器（`sub_14155` 進來時 `dx` ＝ 佔用數:want，`sub dh, dl` 之後 `DX=0201` ⇒ skip 2、want 1）：

| | 佔用數(129) | 威脅量(129) | want | skip |
|---|---:|---:|---:|---:|
| 原版 | 3 | **2** | 1 | 2 |
| remake | 3 | **3** | 2 | 1 |

⇒ 只有威脅量差 1。威脅量是 Σ 敵方鄰居的佔用數，而據點 129 的敵方鄰居只有 122 （139 中立、118 和平），所以**據點 122 的佔用數原版 2、remake 3**。

⭐ 那一格（座標 `(304,158)`）實際站著**三支**軍團（34／72／114，`+0x1A`／`+0x1C` 都指向同一格）。原版算出 2 ⇒ **有一支從來沒被 `inc` 進佔用圖**。

根因追到底了（`docs/spec/183` §3）：**不在維護，在起點**。五個維護點全部照抄之後還是差一——原版側是**同一次執行繼續往下跑**，圖是從開新遊戲以來累積的歷史；remake 是**從快照重建**，得到的是「每一支活著的軍團都在自己那一格」這個理想值。兩者在快照當下就可能不同，只是要等到那一格被 `sub_13EFD` 重算（每 192 拍一次）才顯現。

⇒ 要修就得讓快照帶著這張圖：`tools/orig_snapshot.py` 多 `peek` 軍團所在的那幾格（段基底從軍團 `+0x1C` 反推，`Y × 24 + 段`、偏移 `X`），`tools/rng_pace.go` 用它初始化 `World.occupancy`。**夾具層的工作，不是規則層的。**

**怎樣算做完**：快照帶佔用圖之後，`tools/parity_ck.sh` 在拍 3,415 之後仍是四張表 0；逐拍取數的第一個分歧往後推。

**verify**：`absent` `/occupancy|佔用圖/` 在 `tools/orig_snapshot.py`、`tools/rng_pace.go`
