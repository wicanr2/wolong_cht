...
#### 五個檢查點剩下的差異：整備值與移動計時器各差 1

事件佇列游標接上之後（docs/spec/176），五個檢查點剩下的差異只有兩種形狀：

1. **據點 `+0x10`（上昇值）／`+0x11`（防災值）差 1**，而且落在游標剛掃過的那兩三座（103／104、27–29、43／44、151–154）。
2. **軍團 `+0x0B`（移動計時器）差 1–2**。

兩者都像「整備／移動各差一拍」的相位差，但**據點游標 `+0x2E` 兩邊相同**，所以不能直接歸給取樣時刻。候選之一是 `byte_131AD`（每 10 次一筆的節拍計數）——它在程式碼段，不在存檔區塊裡，remake 只能用初值 7。現況數字在 `docs/playtest/119` §37.3／§37.4。

**怎樣算做完**：五個檢查點（200／700／1,200／1,600／1,900）的據點表與軍團表都逐 byte 一致。

**verify**：`present` `/所以不能直接歸給取樣相位/` 在 `docs/playtest/119-rng-pace-comparison.md`
