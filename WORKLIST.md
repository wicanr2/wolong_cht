...
逐拍對拍（`workplace/parity/drift/`）把攻城的第一個分歧釘在**拍 2**：原版拍 0／1／2 一格都沒動、拍 3 才走，remake 拍 2 就走了。**錯開一拍後拍 0–2 的 96 槽完全相同、拍 3–5 只差 2 個兵**（`docs/spec/200`）。

⛔ **「開場設了一個計時器」的假說已經否證**：`ida_disp_users.py` 掃全庫「位移 1 的寫入」，兵記錄那一段每一個寫入端都是命中或發射（`sub_1AD2D`／`sub_1B618`／`sub_1B97E`…），**沒有一支在開場設**。

⭐ `docs/spec/159` 早就寫過另一半：原版**首拍有呼叫移動常式**（體力 128→127）**只是沒走成**。⇒ 不是被擋住，是前兩拍算不出可以走的一步。

下一個要讀的是移動常式 `sub_1AF69` 與陣形 handler 尾端的 `sub_1AA2C`（中繼目標）；`docs/spec/159` 的「中繼目標追查補記」已列了四支相關函式。
