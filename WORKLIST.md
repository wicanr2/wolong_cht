...
#### 拍 5,016：求援那一支的分岔走了不同邊

二十五個檢查點（拍 200 … 5,200）裡拍 200–4,950 五張表全 0 之後的第一個分歧。

據點 2：原版取了 `sub_14155` 的調兵亂數（`14171`）而 remake 沒有，也就是 `sub_14057` 的 `cmp byte ptr [si+858h], 1 / ja loc_14099`（佔用數 > 1 才調兵）兩邊判到不同邊。

⚠ 佔用數是**佔用圖抄來的**（`docs/playtest/119` §42），不是座標數出來的——先確認拍 5,016 前後兩邊的佔用圖一致。

**怎樣算做完**：`tools/parity_ck.sh` 在拍 5,200 的五張表都是 0。

**verify**：`present` `/求援那一支的分岔走了不同邊/` 在 `docs/playtest/119-rng-pace-comparison.md`
