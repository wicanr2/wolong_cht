...
#### 拍 5,000：戰後退卻選的方向與原版不同

十四場戰鬥的前十三場（拍數、雙方、模式）全部對上，拍 4,999 五張表全 0。分歧是拍 5,000 那一場野戰（軍團 44 vs 軍團 73）的**戰後行為**。

兵力、士氣、勝負都一致（軍團 44 沒有任何差異），差的是軍團 73 往哪走：原版退進據點 2（`+0x0E` ＝ `0x0010`、步進 `FC` 反向、`+0x00` ＝ `C5`），remake 留在 (246,16) 又撞上軍團 44 開始新的對峙（`+0x00` ＝ `E5`、`+0x03` ＝ 9、步進 `04` 正向）。

⇒ `retreatOrPerish` 的 `nextHopHome` 選的方向與 `sub_1487B` 不同。

連帶：第 14 場戰鬥 remake 早 6 拍（5,098 vs 5,104）。

**怎樣算做完**：`tools/parity_ck.sh` 在拍 5,040 與 5,080 的五張表都是 0。

**verify**：`present` `/戰後退卻的方向/` 在 `docs/playtest/119-rng-pace-comparison.md`
