...
5,480 拍的逐拍取數零不一致、五張表在拍 200 … 5,080 逐 byte 相同。之後剩三類：

- **據點 `+0x00` 低 4 位與 `+0x1b`**（拍 5,097 的據點 85／88）：鄰接敵方據點的遮罩與計數。remake 由現況算（`threat.EnemyMask`），原版由 `sub_1890A` 在換手時增量維護——**更新時機不同**，下一個檢查點就自己收斂。要對齊得照原版在換手那一刻改。
- **軍團 `+0x0B`（移動計時）差 1–2**（拍 5,137 的軍團 50／61／73）。
- **據點 106 的 `+0x15`**（原版 06、remake 00）。寫入端只有 `sub_1237E`（暴風雨）與 `sub_134B1`，**欄位語意未解**，`docs/formats/08` 沒有它。

**怎樣算做完**：`tools/parity_ck.sh` 在拍 5,120、5,160、5,200 的五張表都是 0。

**verify**：`present` `/欄位語意未解/` 在 `docs/playtest/119-rng-pace-comparison.md`
