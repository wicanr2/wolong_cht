...
攻城那一組（`tools/parity_screens.sh siege`）走精確初始化之後：**開場 96 槽逐槽全等**，跑 6 個戰術節拍之後 8 個兵裡 2 個完全相同、6 個差 1 格，而且**方向不一致**（+1／−1 混著）——不是取樣相位。

`sb-minimap` 的殘差對得上算術：縮圖每格 2 px，一個兵錯 1 格貢獻 8 px，48 兵 × 8 ≈ 384，與 `ticks:6` 那一輪量到的 388 相符（`docs/playtest/121` §4.3）。

⚠ **這一格連錯三次歸因**（亂數不同源／擺位結果不同／縮圖換算），三次都是拿一個沒對齊的比較去解釋差異。現在的說法有逐兵數字支撐。

下一步：規則層的逐拍比對——像 `tools/parity_ck.sh` 對六張表做的那樣，找出**第幾拍開始差**。畫面對拍只告訴你「有差」。
