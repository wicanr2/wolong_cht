...
#### 軍團 `+0x0B`（移動計時）的相位差 1–2

同局面拍 5,137：軍團 50（原版 1、remake 3）、61（3→2）、73（3→2）。

`+0x0B` 由 `sub_16FD2`（`recalcCorps`）寫 1、`sub_14DA4` 也寫 1，其餘每個移動週期倒數。差 1–2 表示某條路徑的重設時機不同。

⚠ 軍團 50 那時已經到站（`+0x0E` ＝ `+0x14` ＝ 據點 129、Stage 0），所以不是行軍中的倒數——先看到站之後誰還在寫它。

**怎樣算做完**：`tools/parity_ck.sh` 在拍 5,160 的軍團表是 0。
