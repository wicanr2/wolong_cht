...
type World struct {
	...
	// occupancy 是原版那張單位佔用圖（`cs:word_19872`，384 × 256 的計數器）
	// 的建模：**座標 → 停在那一格的軍團數**。
	//
	// ⭐ **它是增量維護的，不是每次數出來的**（docs/spec/183）。原版
	// `sub_12662` 在軍團「輪到移動」時先 `dec` 舊格、走完再 `inc` 新格；
	// 停在目標上的軍團走 `loc_1266A` 直接 `retn`，**兩邊都不做**。
	// 所以**在某一格編成、從來沒走進來過的軍團不在圖上**——
	// 據點 `+0x18` 因此可能小於「站在那一格的軍團數」。
	//
	// 拿座標重數會得到一個「看起來比較合理」的值，而那個值與原版分岔
	// （同局面拍 3,415：那一格站著三支，原版的圖是 2）。
